...
Apply all migrations through 153 with the migration owner, then apply `infra/postgres/roles.sql` according to the deployment procedure. API and worker startup require schema version 153, the policy functions/tables and the fee-term columns. The role template covers policy authorization and sequence permissions.
