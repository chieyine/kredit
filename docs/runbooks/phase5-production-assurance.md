...
Migration 086 introduced the metrics reader. For the current application, apply every current migration (through 128 for this repair candidate), then apply `infra/postgres/roles.sql`. The helper returns fourteen fixed **aggregate gauges**, not customer records. It is SECURITY DEFINER with fixed search path, no caller-supplied SQL and no PUBLIC execute grant. `row_security=off` makes insufficient owner visibility fail visibly rather than silently filtering the counts. It does not grant table-wide runtime access or weaken financial RLS.
