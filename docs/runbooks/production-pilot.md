...
4. Configure a private scrape of `/api/v1/ops/metrics/prometheus` with the dedicated read-only `METRICS_SCRAPE_TOKEN` credential, then run the load smoke profile against staging and review p95 latency, error rate, provider call volume, and database connections.
