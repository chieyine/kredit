...
psql "${RESTORE_ROLE_ADMIN_URL:-$RESTORE_DATABASE_URL}" -X -v ON_ERROR_STOP=1 -f infra/postgres/roles.sql
...
DATABASE_URL="$RESTORE_DATABASE_URL" python3 scripts/recovery_fingerprint.py capture "$actual_dir/restored.json" --quiesced
python3 scripts/recovery_fingerprint.py compare "$RESTORE_EXPECTED_FINGERPRINT" "$actual_dir/restored.json"
