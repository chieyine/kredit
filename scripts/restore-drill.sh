...
# Verify the original compressed bytes above; pg_restore needs the inner archive.
case "$backup_file" in
  *.gz)
    restore_archive="$actual_dir/backup.dump"
    gzip -dc -- "$backup_file" > "$restore_archive"
    ;;
esac
...
pg_restore --exit-on-error --no-owner --dbname="$RESTORE_DATABASE_URL" "$restore_archive"
