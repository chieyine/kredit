-- +goose Up
-- Named Mono accounts retain the same bank debit safeguards.
...
BEGIN
 ...
 IF mid IS NOT NULL THEN
   ...
   IF provider_name='mono-sweep' OR EXISTS(SELECT 1 FROM app.payment_mandates WHERE id=mid AND metadata->>'provider_adapter'='mono') THEN
...
-- +goose Down
-- +goose StatementBegin
DO $$ BEGIN RAISE EXCEPTION 'Provider boundaries require forward recovery'; END $$;
-- +goose StatementEnd
