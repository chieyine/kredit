-- +goose Up
ALTER POLICY bank_debit_operator_read ON app.bank_debit_enrollments
 USING(app.has_admin_role(app.current_user_id(),ARRAY['platform_owner','platform_admin','compliance_reviewer']));
-- +goose StatementBegin
CREATE OR REPLACE FUNCTION app.pilot_metric(timestamptz,timestamptz,text,text) RETURNS double precision
...
IF NOT app.has_admin_role(app.current_user_id(),ARRAY['platform_owner','platform_admin','compliance_reviewer']) THEN RAISE EXCEPTION 'scorecard authority required'; END IF;
 ...
 WHEN 'days_to_payment' THEN SELECT * INTO result FROM (WITH paid AS (
...
) SELECT COALESCE(avg(extract(epoch FROM (paid_at-activated_at))/86400),0)::float8 FROM paid) metric_value;
 ...
 WHEN 'deemed_acceptance_share' THEN
 SELECT CASE WHEN count(*)=0 THEN 0 ELSE
  100.0*count(*) FILTER(WHERE EXISTS(SELECT 1 FROM app.system_acceptances sa WHERE sa.credit_request_id=o.credit_request_id)) / count(*) END::float8
 INTO result FROM app.obligations o
 WHERE o.activated_at >= $1 AND o.activated_at < $2
 AND ($3='' OR o.supplier_organization_id=NULLIF($3,'')::uuid);
...
-- +goose Down
-- +goose StatementBegin
DO $$ BEGIN RAISE EXCEPTION 'Restore reporting and recovery behavior through a forward migration'; END $$;
-- +goose StatementEnd
