...
func TestAuditProviderRequiresCompleteJSON(t *testing.T) {
	const limit = 1 << 20
	adapters := []string{"native", "connector"}
		...
		t.Run(adapter, func(t *testing.T) {
			...
			for _, tc := range []struct {
				...
				t.Run(tc.name, func(t *testing.T) {
					...
					var err error
					if adapter == "native" {
						provider := &NativeLookup{endpoint: "https://identity.example", token: "synthetic", client: client}
						err = provider.request(context.Background(), http.MethodGet, "/synthetic", nil, &result)
					} else {
						provider := &WebhookProvider{endpoint: "https://identity.example", token: "synthetic", client: client}
						err = provider.request(context.Background(), http.MethodGet, "/synthetic", nil, &result)
					}
					if (err == nil) != tc.ok {
