...
func (s *Server) createCreditRequest(w http.ResponseWriter, r *http.Request) {
	...
	if !exists {
		writeProblem(w, 404, "organization_not_found", "We could not find that business.")
		return
	}
...
func (s *Server) sendCreditRequest(w http.ResponseWriter, r *http.Request) {
	...
	if !s.requireSaleRiskClear(w, r, orgID, existing.Request.BuyerUserID, "credit") {
		return
	}
...
func (s *Server) releaseCreditRequest(w http.ResponseWriter, r *http.Request) {
	...
	if !s.requireSaleRiskClear(w, r, orgID, existing.Request.BuyerUserID, "release") {
		return
	}
...
func (s *Server) getBuyerAgreement(w http.ResponseWriter, r *http.Request) {
	...
	document, err := credit.PrintableAgreement(v)
	if err != nil {
		writeProblem(w, http.StatusConflict, "agreement_document_unavailable", "The agreed terms could not be verified. Please try again or contact support.")
		return
	}
...
func (s *Server) authorizeCreditMandate(w http.ResponseWriter, r *http.Request) {
	...
	var options mandates.AuthorizationOptions
	if r.ContentLength != 0 {
		if err := decodeJSON(w, r, &options); err != nil {
			writeProblem(w, 400, "invalid_mandate_options", "Those bank debit settings are not valid.")
			return
		}
	}
	v, err := s.runtime.Credit.AuthorizeMandate(r.Context(), id, user.ID, options)
	if err != nil {
		writeProblem(w, 409, "mandate_authorization_failed", err.Error())
		return
	}
	s.auditCredit(user.ID, "", "mandate.authorization_requested", id)
	writeJSON(w, 200, v)
}
func (s *Server) acceptCreditRequest(w http.ResponseWriter, r *http.Request) {
	...
	request, err := s.runtime.Credit.GetForBuyer(id, user.ID)
	if err != nil {
		writeProblem(w, http.StatusNotFound, "credit_request_not_found", "credit request was not found")
		return
	}
	...
	if portal.Business.ID != request.Request.BuyerBusinessID || portal.Representative.BusinessID != request.Request.BuyerBusinessID || portal.Representative.PersonID != portal.Person.ID {
...
func (s *Server) recordPayment(w http.ResponseWriter, r *http.Request) {
	...
	if in.SourceType == payments.SourceCollected {
	...
	recorder, ok := s.runtime.Payments.(interface {
...
func (s *Server) reconcilePayments(w http.ResponseWriter, r *http.Request) {
	...
	rebuilder, ok := s.runtime.Payments.(interface {
		RebuildContext(context.Context, string) (ledger.Money, error)
	})
	if !ok {
		writeProblem(w, 503, "payment_unavailable", "Payment reconciliation is temporarily unavailable. Please try again.")
		return
	}
	rebuilt, err := rebuilder.RebuildContext(r.Context(), v.Obligation.ID)
	if err != nil {
	...
	for _, payment := range paymentsForObligation {
		...
		if err != nil {
			writeProblem(w, 503, "ledger_unavailable", "Your money history is not available right now. Please try again shortly.")
			return
		}
...
func (s *Server) reversePayment(w http.ResponseWriter, r *http.Request) {
	...
	contextPayments, ok := s.runtime.Payments.(interface {
		GetContext(context.Context, string) (payments.Payment, error)
		ReverseContext(context.Context, string, string, string) (payments.Payment, error)
	})
	if !ok {
		writeProblem(w, 503, "payment_unavailable", "Payment reversal is temporarily unavailable. Please try again.")
		return
	}
	p, err := contextPayments.GetContext(r.Context(), paymentID)
	if err != nil || p.ObligationID != v.Obligation.ID {
		writeProblem(w, 404, "payment_not_found", "We could not find that payment.")
		return
	}
	p, err = contextPayments.ReverseContext(r.Context(), paymentID, user.ID, in.Reason)
	if err != nil {
...
func (s *Server) createSchedule(w http.ResponseWriter, r *http.Request) {
	...
	// Activation derives the schedule from the accepted agreement. Replacing it
	// here would bypass the buyer's acceptance and could accelerate a bank debit.
	writeProblem(w, 409, "accepted_schedule_locked", "Payment days you already agreed cannot be changed here. Somebody at Kredit must review it, and your customer must agree to it. Contact support to ask.")
}
...
func (s *Server) createTradeLine(w http.ResponseWriter, r *http.Request) {
	...
	mandate, err := resolver.ResolveTradeLineMandate(r.Context(), in.MandateID, in.BuyerUserID, in.BuyerBusinessID, orgID)
...
func (s *Server) reserveDrawdown(w http.ResponseWriter, r *http.Request) {
	...
	if !s.requireSaleRiskClear(w, r, orgID, line.BuyerUserID, "credit") {
		return
	}
...
func (s *Server) requireDrawdownScope(w http.ResponseWriter, lineID, drawdownID, buyerID, organizationID string) bool {
	...
	for _, drawdown := range statement.Drawdowns {
		if drawdown.ID == drawdownID && drawdown.TradeLineID == lineID {
			return true
		}
	}
	writeProblem(w, http.StatusNotFound, "drawdown_not_found", "We could not find that purchase under this customer limit.")
	return false
}
...
func (s *Server) retryCollection(w http.ResponseWriter, r *http.Request) {
	...
	if !exists || !s.runtime.Credit.ObligationBelongsToOrganization(attempt.ObligationID, orgID) {
		writeProblem(w, 404, "collection_not_found", "We could not find that bank debit.")
		return
	}
...
func (s *Server) collectionWebhook(w http.ResponseWriter, r *http.Request) {
	...
	attempt, err := s.runtime.Collections.SignalWebhook(r.Context(), event)
	if err != nil {
		writeProblem(w, 400, "collection_webhook_rejected", err.Error())
		return
	}
	writeJSON(w, 202, map[string]any{"attempt": attempt, "reconciliation_required": true})
}
...
func (s *Server) decideDispute(w http.ResponseWriter, r *http.Request) {
	orgID, _ := pathID(r, "organizationID")
	_, _, _, ok := s.requireOrganizationAccess(w, r, orgID, access.PermissionManageDisputes)
	if !ok {
		return
	}
	if !s.requireCSRF(w, r) {
	...
	writeProblem(w, http.StatusForbidden, "independent_review_required", "buyer disputes can only be decided by an independent dispute reviewer")
}
...
func (s *Server) writeOffObligation(w http.ResponseWriter, r *http.Request) {
	...
	if ledger.Money(in.AmountKobo) > v.Obligation.OutstandingKobo {
		writeProblem(w, 422, "operation_invalid", "You cannot write off more than the customer still owes.")
		return
	}
...
func (s *Server) listOperationActions(w http.ResponseWriter, r *http.Request) {
	...
	actions, err := s.runtime.Operations.ListForOrganization(r.Context(), orgID)
	if err != nil {
		writeProblem(w, http.StatusServiceUnavailable, "operation_history_unavailable", "We could not load your business activity. Please try again.")
		return
	}
...
// Credit and release holds apply to both parties. Reporting payments, declining
// offers and recording delivery problems remain available while a hold is active.
func (s *Server) requireSaleRiskClear(w http.ResponseWriter, r *http.Request, organizationID, buyerID, scope string) bool {
	if s.runtime.PlatformOps == nil {
		return true
	}
	ctx, cancel := context.WithTimeout(r.Context(), 5*time.Second)
	defer cancel()
	for _, target := range []struct{ kind, id string }{{"supplier", organizationID}, {"buyer", buyerID}} {
		if target.id == "" {
			continue
		}
		blocked, err := s.runtime.PlatformOps.ActiveHold(ctx, target.kind, target.id, scope)
		if err != nil {
			writeProblem(w, http.StatusServiceUnavailable, "risk_hold_unavailable", "Risk hold status could not be verified. Try again.")
			return false
		}
		if blocked {
			writeProblem(w, http.StatusLocked, "risk_hold_active", "This action is blocked by an active risk hold.")
			return false
		}
	}
	return true
