...
func TestAuditCompleteProviderResponse(t *testing.T) {
	const limit = 1048576
	const prefix = `{"banks":[{"code":"058","name":"Synthetic bank"}]}`
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			connector, err := NewConnector("synthetic", "https://connector.example", strings.Repeat("x", 32))
			if err != nil {
				t.Fatal(err)
			}
			connector.client.Transport = auditResponseTransport(func(*http.Request) (*http.Response, error) { return response, nil })
			banks, err := connector.Banks(context.Background())
			if (err == nil) != tc.ok || (err != nil && banks != nil) {
				t.Fatalf("banks=%v error=%v; want success=%v", banks, err, tc.ok)
			}
			if !body.closed {
