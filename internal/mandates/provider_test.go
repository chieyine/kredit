...
func TestMockProviderResolvesTradeLineMandateByOwner(t *testing.T) {
	provider := NewMockProvider()
	mandate, err := provider.CreateAuthorizationSession(context.Background(), AuthorizationInput{UserID: "buyer-1", BusinessID: "business-1", AmountCeiling: 100000, Purpose: "recurring"})
	if err != nil {
		t.Fatal(err)
	}
	resolved, err := provider.ResolveTradeLineMandate(context.Background(), mandate.ID, "buyer-1", "business-1")
	if err != nil || resolved.ID != mandate.ID || resolved.Status != Active {
		t.Fatalf("resolved=%#v err=%v", resolved, err)
	}
	if _, err := provider.ResolveTradeLineMandate(context.Background(), mandate.ID, "other-buyer", "business-1"); err == nil {
