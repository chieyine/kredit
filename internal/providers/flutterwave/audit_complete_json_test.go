...
func TestAuditProviderRequiresCompleteJSON(t *testing.T) {
	const limit = 1 << 20
	adapters := []string{"flutterwave"}
	for _, adapter := range adapters {
		t.Run(adapter, func(t *testing.T) {
			const prefix = `{"status":"success","data":{"accepted":true}}`
			...
			for _, tc := range []struct {
				...
				t.Run(tc.name, func(t *testing.T) {
					...
					provider := &Client{endpoint: "https://provider.example", secret: "synthetic", http: client}
					err := provider.call(context.Background(), http.MethodGet, "/synthetic", nil, &result)
					if (err == nil) != tc.ok {
