...
func TestAuditProviderRequiresCompleteJSON(t *testing.T) {
	const limit = 1 << 20
	adapters := []string{"monnify"}
	for _, adapter := range adapters {
		t.Run(adapter, func(t *testing.T) {
			const prefix = `{"requestSuccessful":true,"responseCode":"0","responseBody":{"accepted":true}}`
			...
			for _, tc := range []struct {
				...
				t.Run(tc.name, func(t *testing.T) {
					...
					provider := &Client{endpoint: "https://provider.example", http: client}
					err := provider.request(context.Background(), http.MethodGet, "/synthetic", "synthetic", nil, &result)
					if (err == nil) != tc.ok {
