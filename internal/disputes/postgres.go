...
func NewPostgresStore(pool *pgxpool.Pool, invalidate func(string)) *PostgresStore {
	return &PostgresStore{pool: &db.ScopedDatabase{Pool: pool}, ctx: context.Background(), invalidate: invalidate}
}
...
func (s *PostgresStore) Get(id string) (Dispute, []Evidence, []Decision, error) {
	d, err := scanDispute(s.pool.QueryRow(s.ctx, `SELECT id::text,obligation_id::text,supplier_organization_id::text,buyer_user_id::text,opened_by::text,total_disputed_kobo,remaining_disputed_kobo,reason,COALESCE(explanation,''),state,collection_effect,COALESCE(assigned_reviewer::text,''),opened_at,resolved_at FROM app.disputes WHERE id=$1::uuid`, id))
	if errors.Is(err, pgx.ErrNoRows) {
...
func (s *PostgresStore) list(where, id string) []Dispute {
	rows, err := s.pool.Query(s.ctx, `SELECT id::text,obligation_id::text,supplier_organization_id::text,buyer_user_id::text,opened_by::text,total_disputed_kobo,remaining_disputed_kobo,reason,COALESCE(explanation,''),state,collection_effect,COALESCE(assigned_reviewer::text,''),opened_at,resolved_at FROM app.disputes WHERE `+where+` ORDER BY opened_at DESC`, id)
	if err != nil {
		return []Dispute{}
	}
	defer rows.Close()
...
func (s *PostgresStore) evidence(id string) ([]Evidence, error) {
	rows, err := s.pool.Query(s.ctx, `SELECT id::text,dispute_id::text,submitted_by::text,COALESCE(document_id::text,''),COALESCE(statement,''),submitted_at FROM app.dispute_evidence WHERE dispute_id=$1::uuid ORDER BY submitted_at`, id)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *PostgresStore) decisions(id string) ([]Decision, error) {
	rows, err := s.pool.Query(s.ctx, `SELECT id::text,dispute_id::text,reviewer_id::text,outcome,valid_principal_kobo,adjustment_kobo,remaining_disputed_kobo,reason,decided_at FROM app.dispute_decisions WHERE dispute_id=$1::uuid ORDER BY decided_at`, id)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *PostgresStore) ForContext(ctx context.Context) Service {
	return &PostgresStore{pool: s.pool, ctx: ctx, invalidate: s.invalidate}
}
