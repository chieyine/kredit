...
async function signedIn(page: Page, context: BrowserContext, baseURL?: string) {
	await context.addCookies([
		{ name: 'kredit_session', value: 'synthetic-completion-session', url: baseURL ?? 'http://127.0.0.1:5173' }
	]);
	await page.route('**/api/v1/**', (route) => {
		...
		if (path === '/api/v1/organizations')
			return route.fulfill({
				json: {
					organizations: [
						{ id: 'org-a', legal_name: 'Example supplier A' },
						{ id: 'org-b', legal_name: 'Example supplier B' }
					]
				}
			});
...
for (const [kind, path, goodsLabel] of [
	...
	test(`${kind} draft storage requires consent, restores only opted-in fields and can be revoked`, async ({
		page,
		context,
		baseURL
	}) => {
		...
		expect(saved).toMatchObject({
			...
			goods: 'Synthetic oil cartons',
			principal: '127,500.49'
		});
...
test('legacy default-on drafts are discarded and runtime-only fields are never persisted', () => {
	...
	const draft = {
		goods: 'Synthetic stock',
		principal: '100.49',
		dueDate: '',
		customer: 'DO-NOT-RETAIN',
		invoice: 'DO-NOT-RETAIN'
	};
	storage.setItem(
		'kredit.quick-sale.v2:u1:org-a',
		JSON.stringify({ ...draft, userID: 'u1', organizationID: 'org-a', expiresAt: Date.now() + 10000 })
	);
...
test('feedback recovers from a dropped response and success survives unavailable storage', async ({
	page,
	context,
	baseURL
}) => {
	...
	await page.addInitScript(() => {
		Object.defineProperty(window, 'localStorage', {
			get() {
				throw new Error('Storage unavailable in this synthetic test');
			}
		});
	});
	const keys: string[] = [];
	await page.route('**/api/v1/me/product-feedback', (route) => {
		...
		return keys.length === 1
			? route.abort('failed')
			: route.fulfill({ status: 201, json: { feedback: { answer: 'yes' } } });
	});
...
async function saleDetail(page: Page) {
	await page.route('**/organizations/org-a/credit-requests/retry-sale', (route) =>
		route.fulfill({
			json: {
				request: {
					id: 'retry-sale',
					version: 1,
					state: 'ACTIVE',
					buyer_legal_name: 'Example buyer',
					principal_kobo: 100049,
					goods_description: 'Synthetic stock',
					due_date: '2026-10-01',
					collection_at: '2026-10-02T12:00:00Z',
					grace_hours: 24
				},
	...
	await page.route('**/credit-requests/retry-sale/schedule', (route) =>
		route.fulfill({ json: { schedule: { id: 'schedule-1' }, items: [] } })
	);
	await page.route('**/credit-requests/retry-sale/collection/eligibility', (route) =>
		route.fulfill({ json: { eligible: false, reasons: [] } })
	);
	await page.route('**/credit-requests/retry-sale/collections', (route) => route.fulfill({ json: { attempts: [] } }));
}

test('sale-detail payment retry preserves its key, amount and paid-at timestamp after reload', async ({
	page,
	context,
	baseURL
}) => {
	await signedIn(page, context, baseURL);
	await saleDetail(page);
	const keys: string[] = [],
		bodies: unknown[] = [];
	await page.route('**/credit-requests/retry-sale/payments', (route) => {
		...
		return keys.length === 1
			? route.abort('failed')
			: route.fulfill({ status: 201, json: { payment: { id: 'payment-1' } } });
	});
	const enter = async () => {
...
test('sale-detail failed payment and schedule reads never become empty successful histories', async ({
	page,
	context,
	baseURL
}) => {
	...
	for (const path of ['payments', 'schedule', 'payment-claims', 'collections', 'collection/eligibility']) {
		await page.route(`**/credit-requests/retry-sale/${path}`, (route) =>
...
test('sale-detail transfer confirmation cannot bypass the bank-check dialog', async ({ page, context, baseURL }) => {
	...
	const claim = {
		...
		state: 'pending',
		amount_kobo: 10049,
		transfer_reference: 'SYNTHETIC-CLAIM',
		paid_at: '2026-09-07T12:00:00Z'
	};
	await page.route('**/credit-requests/retry-sale/payment-claims', (route) =>
		route.fulfill({ json: { payment_claims: [claim] } })
	);
	let submitted = 0;
	await page.route('**/payment-claims/claim-1/decide', (route) => {
		submitted++;
		return route.fulfill({ json: { payment_claim: { ...claim, state: 'confirmed' } } });
	});
	await page.goto('/workspace/sales/retry-sale?organization=org-a');
