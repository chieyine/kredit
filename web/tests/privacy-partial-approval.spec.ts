...
test('privacy approval acknowledges retained records without reporting an uncertain failure', async ({
	page,
	context,
	baseURL
}) => {
	...
	let item = {
		...
		details: 'Remove optional profile information',
		decided_by: ''
	};
	...
	await page.route('**/api/v1/ops/privacy-requests/privacy-request/decide', async (route) => {
		...
		await route.fulfill({
			json: { request: { ...item, legal_hold_applies: true, retention_outcome: 'Required records retained' } }
		});
	});
