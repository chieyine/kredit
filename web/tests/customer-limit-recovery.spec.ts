...
const line = (id: string) => ({
	id,
	supplier_organization_id: 'org',
	buyer_user_id: 'buyer',
	buyer_business_id: 'business',
	state: 'ACTIVE',
	approved_limit_kobo: 100000,
	current_exposure_kobo: 0,
	reserved_pending_kobo: 0,
	available_limit_kobo: 100000,
	version: 4
});
test.beforeEach(async ({ page, context, baseURL }) => {
	await context.addCookies([
		{ name: 'kredit_session', value: 'limit-review', url: baseURL ?? 'http://127.0.0.1:5173' }
	]);
	await page.route('**/api/v1/me', (r) =>
		r.fulfill({ json: { user: { id: 'seller' }, session: { authentication_level: 'AAL2' }, organizations: [] } })
	);
	await page.route('**/api/v1/organizations', (r) =>
		r.fulfill({ json: { organizations: [{ id: 'org', legal_name: 'Seller' }] } })
	);
	await page.route('**/api/v1/organizations/org/trade-lines/*/statement', (r) =>
		r.fulfill({ json: { line: line(r.request().url().split('/').at(-2)!), drawdowns: [] } })
	);
});
test('uncertain customer-limit changes retry the original amount and version', async ({ page }) => {
	...
	await page.route('**/api/v1/organizations/org/trade-lines/first', (r) => {
		sent.push({ body: r.request().postDataJSON(), key: r.request().headers()['idempotency-key'] });
		return r.fulfill(
			sent.length === 1
				? { status: 503, json: { detail: 'Result unknown' } }
				: {
						json: {
							trade_line: { ...line('first'), version: 5, approved_limit_kobo: 80000, available_limit_kobo: 80000 }
						}
					}
		);
	});
...
test('customer-limit route changes clear prior input and capability outages remain retryable', async ({ page }) => {
	let calls = 0;
	await page.route('**/api/v1/platform/capabilities', (r) =>
		r.fulfill(
			++calls === 1 ? { status: 503, json: { detail: 'Unavailable' } } : { json: { features: { drawdowns: true } } }
		)
	);
	await page.goto('/workspace/sales/limits/first?organization=org');
	...
	await page.evaluate(() => {
		...
		a.href = '/workspace/sales/limits/second?organization=org';
		a.textContent = 'Open second limit';
