...
for (const area of ['app', 'buyer']) {
	test(`${area} message history recovers from a dropped response without claiming an empty history`, async ({
		page,
		context,
		baseURL
	}) => {
		...
		await page.route('**/api/v1/me', (route) =>
			route.fulfill({ json: { user: { id: 'history-user' }, session: { id: 'session' } } })
		);
		let ready = false;
							...
							notifications: [
								{
									id: 'message-1',
									channel: 'email',
									template: 'PaymentRecorded',
									state: 'sent',
									body: 'Synthetic payment notice',
