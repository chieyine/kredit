...
async function prepare(page: Page, context: BrowserContext, baseURL?: string) {
	await context.addCookies([
		{ name: 'kredit_session', value: 'synthetic-audit', url: baseURL ?? 'http://127.0.0.1:5173' },
	...
	await page.route('**/api/v1/**', (route) => {
		...
		if (path === '/api/v1/me')
			return send(route, {
				...
				organizations: [{ id: 'org-a', legal_name: 'Kora Wholesale' }]
			});
		if (path === '/api/v1/organizations')
			return send(route, { organizations: [{ id: 'org-a', legal_name: 'Kora Wholesale', trading_name: '' }] });
		...
		if (path.endsWith('/credit-terms/preview')) {
			...
			return send(route, {
				...
				cutoff: '23:59',
				timing_mode: input.collection_local ? 'lagos_explicit' : 'lagos_end_of_day'
			});
		}
...
test('invoice upload retries preserve one operation before sale creation', async ({ page, context, baseURL }) => {
	...
	const hash = await page.evaluate(async () => {
		...
		return Array.from(new Uint8Array(await crypto.subtle.digest('SHA-256', await response.arrayBuffer())), (byte) =>
	...
	await page.route('**/api/v1/organizations/org-a/credit-requests', (route) => {
		...
		return send(route, { request: { id: 'created' } }, 201);
	});
...
test('custom instalments must reconcile exactly before any write', async ({ page, context, baseURL }) => {
	...
	await page.route('**/api/v1/organizations/org-a/credit-requests', (route) => {
		writes++;
		return send(route, {}, 500);
	});
	await page.getByLabel('How will they pay?').selectOption('custom');
	await page
		.getByRole('textbox', { name: 'Write out each payment' })
		.fill('60,000.00, 2026-09-18\n60,000.00, 2026-10-18');
	await page.getByRole('button', { name: 'Check terms', exact: true }).click();
