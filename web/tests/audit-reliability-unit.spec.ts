...
import { LatestRequest, rows, record, readResource, normalizeNigerianPhone, safeNext } from '../src/lib/api/reliable';
...
test('failed, malformed and missing financial responses stay unavailable, not empty', async () => {
	...
	expect(await readResource('org-a', '/payments', rows('payments', record))).toMatchObject({
		state: 'ready',
		data: []
	});
});
...
test('lost response reuses exactly one mutation identity even after navigation', async () => {
	...
	await expect(first.run({ amount_kobo: 10049, reference: 'TEST-ONLY' }, record)).rejects.toMatchObject({
		outcome: 'unknown'
	});
...
test('unknown identities are never silently expired and replaced', async () => {
	...
	saved.createdAt = new Date(Date.now() - 16 * 60000).toISOString();
	store.setItem(key, JSON.stringify(saved));
	await expect(new MutationIntent('user', '/payments', store).run({ amount: 100 }, record)).rejects.toMatchObject({
		outcome: 'unknown'
	});
	expect(calls).toBe(1);
});
...
test('offline, malformed saved identity and unavailable storage fail before a write', async () => {
	...
	await expect(new MutationIntent('u', '/payments', new MemoryStorage()).run({}, record)).rejects.toMatchObject({
		outcome: 'not_sent'
	});
	Object.defineProperty(globalThis, 'navigator', { configurable: true, value: { onLine: true } });
	const store = new MemoryStorage();
	store.setItem('kredit.intent.v1:u:%2Fpayments', '{broken');
	await expect(new MutationIntent('u', '/payments', store).run({}, record)).rejects.toMatchObject({
		outcome: 'unknown'
	});
	...
	await expect(new MutationIntent('u', '/payments', unavailable).run({}, record)).rejects.toMatchObject({
		outcome: 'not_sent'
	});
	expect(calls).toBe(0);
});
...
test('malformed success and server errors retain the unresolved identity', async () => {
	for (const response of [
		...
		json({ code: 'idempotency_in_progress' }, 409),
		json({}, 500)
	]) {
		const store = new MemoryStorage();
		globalThis.fetch = async () => response;
...
test('request storage contains a digest, not personal or financial form fields', async () => {
	...
	await expect(
		new MutationIntent('user-a', '/payment', store).run(
			{ secretEvidence: 'DO-NOT-RETAIN', amount_kobo: 9876543 },
			record
		)
	).rejects.toThrow();
	const saved = store.getItem(store.key(0)!)!;
	expect(saved).not.toContain('DO-NOT-RETAIN');
	expect(saved).not.toContain('9876543');
...
test('drafts are isolated by account and business and expire', () => {
	const store = new MemoryStorage(),
		now = Date.now(),
		draft = { goods: '40 cartons', principal: '127,500.49', dueDate: '2026-09-18' };
...
test('drafts reject tampering and discard the legacy unscoped record', () => {
	...
	const key = store.key(1)!;
	const value = JSON.parse(store.getItem(key)!);
	value.userID = 'b';
	store.setItem(key, JSON.stringify(value));
	expect(readDraft('a', 'one', store)).toBeNull();
	expect(store.getItem('kredit.quick-sale.draft.v1')).toBeNull();
});

for (const phone of ['0803 123 4567', '2348031234567', '+234 (803) 123-4567'])
	test(`normalises Nigerian phone ${phone}`, () => {
		expect(normalizeNigerianPhone(phone)).toBe('+2348031234567');
	});
for (const phone of ['123', '+12025551234', '080312345678', '80letters'])
	test(`rejects invalid Nigerian phone ${phone}`, () => {
		expect(() => normalizeNigerianPhone(phone)).toThrow();
	});

test('post-login destinations stay on the same origin', () => {
	for (const path of ['//evil.test', '/\\evil.test', '/signin', 'https://evil.test', '/\u0000evil'])
		expect(safeNext(path, 'https://kredit.ng')).toBe('/start');
	expect(safeNext('/workspace/purchases/orders/abc?review=1', 'https://kredit.ng')).toBe(
		'/workspace/purchases/orders/abc?review=1'
	);
});
...
test('hosted permission only links to the approved HTTPS provider origin', () => {
	...
	for (const url of [
		'javascript:alert(1)',
		'http://authorise.mono.co/a',
		'https://authorise.mono.co.evil.test/a',
		'https://evil.test/a',
		'https://user@authorise.mono.co/a',
		'https://authorise.mono.co:8443/a',
		'https://authorise.mono.co/'
	])
		expect(hostedAuthorizationURL(url, 'mono-sweep')).toBeNull();
});
...
test('prices retain exact kobo, fee floors and caps', () => {
	...
	expect(baseFeeForKobo(10049, { policy_revision: 1, base_bps: 50, collection_bps: 50, min_fee_kobo: 1000 })).toBe(
		1000n
	);
...
test('financial summaries reject missing, negative and unsafe amounts', () => {
	for (const summary of [
		{},
		{ obligation_count: 1, outstanding_kobo: -1, overdue_kobo: 0 },
		{ obligation_count: 1, outstanding_kobo: Number.MAX_SAFE_INTEGER + 1, overdue_kobo: 0 }
	])
		expect(() => receivables({ summary })).toThrow();
	expect(
		receivables({ summary: { obligation_count: 1, outstanding_kobo: 10049, overdue_kobo: 0 } }).outstanding_kobo
	).toBe(10049);
});
...
test('the entire attention queue counts disputes before ordinary drafts', () => {
	...
	expect(items[0].id).toBe('dispute-dispute-1');
	expect(items[0].href).toContain('organization=org-a');
	expect(attentionItems('org-a', sales, [], [], [{ ...dispute, state: 'PARTIALLY_RESOLVED' }])[0].id).toBe(
		'dispute-dispute-1'
	);
});
