...
test('post-login destinations stay on the same origin', () => {
  for (const path of ['//evil.test', '/\\evil.test', '/signin', 'https://evil.test', '/\u0000evil']) expect(safeNext(path, 'https://kredit.ng')).toBe('/start');
  expect(safeNext('/workspace/purchases/orders/abc?review=1', 'https://kredit.ng')).toBe('/workspace/purchases/orders/abc?review=1');
});
