...
test.beforeEach(async ({ page, context, baseURL }) => {
	...
	await page.route('**/api/v1/me', (route) =>
		route.fulfill({ json: { user: { id: 'audit-user' }, organizations: [], mfa_enrolled: true } })
	);
});
...
test('failed permission reads never claim reminders are stopped', async ({ page }) => {
	...
	await page.route('**/api/v1/buyer/credit-requests', (route) =>
		route.fulfill({
			json: { requests: [{ request: { supplier_organization_id: 'seller-a', supplier_legal_name: 'Audit seller' } }] }
		})
	);
	await page.route('**/api/v1/buyer/relationships/consents', (route) =>
		ready
			? route.fulfill({
					json: {
						suppliers: [{ id: 'seller-a', legal_name: 'Audit seller', trading_name: '' }],
						consents: [
							{
								id: 'consent-a',
								supplier_organization_id: 'seller-a',
	...
	await page.goto('/account/suppliers');
...
test('admin team list failure is recoverable without claiming no administrators exist', async ({ page }) => {
	let ready = false;
	await page.route('**/api/v1/ops/team', (route) =>
		ready ? route.fulfill({ json: { members: [] } }) : route.abort('failed')
	);
...
test('multiple collection attempts for one sale render without duplicate row keys', async ({ page }) => {
	...
	await page.route('**/api/v1/organizations', (route) =>
		route.fulfill({ json: { organizations: [{ id: 'org-a', legal_name: 'Audit seller' }] } })
	);
				...
				collections: [
					{
						...
						buyer_legal_name: 'Test customer',
						amount_kobo: 100000,
						state: 'FAILED',
...
for (const path of ['billing', 'settlement', 'credit-policy']) {
	test(`${path} settings explain an outage and recover before allowing edits`, async ({ page }) => {
		...
		await page.route('**/api/v1/organizations/org-a/onboarding', (route) =>
			route.fulfill({
				json: { profile: { version: 1 }, permissions: { billing: true, settlement: true, credit_policy: true } }
			})
		);
		await page.route('**/api/v1/organizations/org-a/onboarding/settlement/banks', (route) =>
			route.fulfill({ json: { banks: [{ code: '001', name: 'Synthetic bank' }] } })
		);
		await page.route('**/api/v1/organizations/org-a/seller-settlements', (route) =>
			route.fulfill({ json: { settlements: [] } })
		);
		if (path === 'billing') {
			// Billing also mounts these independent read surfaces after onboarding recovers.
			await page.route('**/api/v1/organizations/org-a/fee-operations', (route) =>
				route.fulfill({ json: { banks: [], debits: [] } })
			);
			await page.route('**/api/v1/organizations/org-a/fee-invoices', (route) =>
...
test('buyer can manage reminders for a seller before any sale activates', async ({ page }) => {
	...
	await page.route('**/api/v1/buyer/relationships/consents', async (route) => {
		...
		if (route.request().method() === 'POST') {
			...
			expect(body.supplier_organization_id).toBe(supplier.id);
	...
	await page.goto('/account/suppliers');
	await expect(page.getByText('Limit supplier', { exact: true })).toBeVisible();
