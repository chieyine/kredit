...
test('analytics rejects incomplete metrics and recovers without a page crash', async ({ page }) => {
	...
	await page.route('**/api/v1/ops/analytics/scorecard?*', (route) =>
		route.fulfill(
			json({
				scorecard:
					state === 'missing-feedback'
						? { ...card, feedback: null }
...
test('admin lists reject malformed entries before rendering', async ({ page }) => {
	const errors: string[] = [];
	page.on('pageerror', (error) => errors.push(error.message));
	for (const [path, endpoint, key] of [
		['cases', 'cases?state=', 'cases'],
		['disputes', 'disputes?state=', 'disputes'],
		['audit', 'audit', 'events']
	]) {
...
for (const [path, endpoint, key, empty] of [
	...
	test(`${path}: an outage or malformed response cannot become an empty queue`, async ({ page }) => {
		let state = 'outage';
		await page.route(`**/api/v1/ops/${endpoint}`, (route) =>
			state === 'outage'
...
test('connector configuration sends an atomic replacement and clears credentials after closing', async ({ page }) => {
	const setting = {
		key: 'integrations.notifications.sms',
		category: 'integrations',
		description: 'SMS delivery connector',
		is_secret: true,
		value: '',
		version: 0
	};
	...
	await page.route('**/api/v1/ops/platform-settings', (route) => {
		if (route.request().method() === 'POST') {
			...
			return route.fulfill(json({ setting: { ...setting, version: 1 } }));
		}
	...
	expect(JSON.parse(saved.value)).toMatchObject({
		adapter: 'connector',
		enabled: true,
		endpoint: 'https://connector.example/send',
...
test('editing a protected action invalidates its impact preview', async ({ page }) => {
	await page.route('**/api/v1/ops/commands/preview', (route) =>
		route.fulfill(
			json({
				command: {
					current_version: 1,
					impact_preview: { effect: 'Suspend this user', will_notify: true, audit: 'Required' }
				}
			})
		)
	);
...
test('business and suspended-account links select the matching protected action', async ({ page }) => {
	await page.goto(
		'/admin/controls?target_type=organization&target_id=business&organization_id=business&version=7&status=active'
	);
...
test('Mono settings are editable without revealing credentials and clearly require a restart', async ({ page }) => {
	...
	const setting: any = {
		...
		category: 'integrations',
		description: 'Mono bank collections',
		is_secret: true,
		value: '••••••••',
		version: 3,
		requires_restart: true,
		connection_state: 'applied_unverified',
		applied_version: 3,
		connection_fields: [
	...
	await page.route('**/api/v1/ops/platform-settings', (route) => {
		if (route.request().method() === 'POST') {
			saved = route.request().postDataJSON();
			setting.version = 4;
			setting.connection_state = 'restart_required';
	...
	expect(JSON.parse(saved.value)).toEqual({
		MonoSweepEnabled: false,
		MonoSecretKey: '',
		MonoRedirectURL: 'https://app.kredit.test/return'
	});
});
