...
for (const area of ['app', 'buyer']) {
	test(`${area} dispute evidence retries preserve the submitted request`, async ({ page, context, baseURL }) => {
		...
		const endpoint =
			area === 'buyer' ? '/api/v1/buyer/disputes/problem-1' : '/api/v1/organizations/org-a/disputes/problem-1';
		await page.route(
				...
				route.fulfill({
					json: {
						dispute: {
							...
							state: 'OPEN',
							total_disputed_kobo: 10000,
							remaining_disputed_kobo: 10000,
							collection_effect: 'CONTESTED_ONLY',
							reason: 'Missing cartons',
		...
		await page.route(`**${endpoint}/evidence`, (route) => {
			...
			return sent.length === 1
				? route.abort('failed')
				: route.fulfill({ status: 201, json: { evidence: { id: 'evidence-1' } } });
		});
		await page.route('**/api/v1/buyer/businesses', (route) =>
			route.fulfill({
				json: { businesses: [{ id: 'business-1', workspace_id: 'org-a', legal_name: 'Synthetic buyer' }] }
			})
		);
		await page.route('**/api/v1/buyer/obligations/obligation-1', (route) =>
		...
		await page
			...
			.fill('Two cartons were missing from the delivery.');
...
test('admin dispute decision keeps the same identity after an unknown result', async ({ page, context, baseURL }) => {
	...
	await page.route(`**${endpoint}/decide`, (route) => {
		...
		return sent.length === 1
			? route.abort('failed')
			: route.fulfill({ json: { dispute: { id: 'problem-1' }, decision: { id: 'decision-1' } } });
	});
	await page.goto('/admin/disputes/problem-1');
	await page
		.getByRole('textbox', { name: 'Reason', exact: true })
		.fill('The buyer and supplier evidence confirms this amount.');
