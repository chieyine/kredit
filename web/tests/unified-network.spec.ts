...
const organizations = [
	{ id: 'org-a', legal_name: 'Factory Limited' },
	{ id: 'org-b', legal_name: 'Distributor Limited' }
];
const businesses = organizations.map((org, i) => ({
	id: `profile-${i}`,
	workspace_id: org.id,
...
async function account(page: any, context: any, baseURL: string) {
	...
	await page.route('**/api/v1/**', (route: any) => {
		...
		else if (path.endsWith('/reports/receivables'))
			json = { summary: { obligation_count: 0, outstanding_kobo: 0, overdue_kobo: 0 } };
...
test('a foreign purchasing business fails closed instead of showing another business', async ({
	page,
	context,
	baseURL
}) => {
	await account(page, context, baseURL!);
	await page.goto('/workspace/purchases?organization=org-a&business_id=profile-1');
...
test('signed-out private entry preserves the intended destination', async ({ page }) => {
	for (const path of ['/start', '/account', '/workspace/today', '/personal/purchases']) {
		await page.goto(path);
		await expect(page).toHaveURL(new RegExp('/signin\\?next=' + encodeURIComponent(path)));
	}
});

test('a direct purchase link resolves the actual business and rejects a conflicting context', async ({
	page,
	context,
	baseURL
}) => {
	await account(page, context, baseURL!);
	await page.route('**/api/v1/buyer/obligations/debt-2', (route) =>
		route.fulfill({
			json: {
				view: {
					request: { id: 'sale-2', buyer_business_id: 'profile-1', goods_description: 'Goods for Distributor Limited' },
