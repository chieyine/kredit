...
for (const selected of ['home', 'terms', 'contact', 'guide-how-to-sell-goods-on-credit-in-nigeria'] as const)
	test(`owner saves, reviews and publishes ${selected} without interpreting editor text as HTML`, async ({
		page,
		context,
		baseURL
	}) => {
		...
		await page.route('**/api/v1/me', (route) =>
			route.fulfill({ json: { user: { id: 'owner' }, organizations: [], mfa_enrolled: true } })
		);
		let version = 0;
		...
		await page.route('**/api/v1/ops/website/*', async (route) => {
			if (!route.request().url().endsWith(`/${selected}`)) {
				await route.fulfill({ json: { version: 0, content: null } });
				return;
			}
			...
			if (body.action === 'save') content = { draft: body.copy };
				...
				expect(body.copy).toEqual(content.draft);
				content.published = {
					copy: body.copy,
					version,
					published_at: '2026-09-09T09:00:00Z',
					content_hash: 'a'.repeat(64),
					...(selected === 'terms' ? { document_version: `legal-terms-v${version}`, effective_date: '2026-09-09' } : {})
				};
			}
			await route.fulfill({ json: { version, content } });
		});
		...
		if (selected !== 'home') {
			await page.getByRole('combobox', { name: 'Page', exact: true }).selectOption(selected);
			await expect(page.getByLabel('Title', { exact: true })).toHaveValue(websiteDefaults[selected].title);
			if (selected !== 'contact')
				await expect(page.getByLabel('Section text').first()).toHaveValue(websiteDefaults[selected].sections[0].body);
		}
		...
		if (selected === 'terms')
			await expect(page.getByRole('link', { name: /Read published document/ })).toHaveAttribute(
				'href',
				'/legal/terms?version=legal-terms-v2'
			);
	});
