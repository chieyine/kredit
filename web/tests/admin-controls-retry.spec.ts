...
test('unknown protected command survives reload and cannot be replaced with changed details', async ({
	page,
	context,
	baseURL
}) => {
	...
	await page.route('**/api/v1/me', (route) =>
		route.fulfill({ json: { user: { id: 'admin' }, organizations: [], mfa_enrolled: true } })
	);
		...
		route.fulfill({
			json: {
				command: {
					...
					current_version: 1,
					impact_preview: { effect: 'Suspend this synthetic user', will_notify: true, audit: 'immutable' }
				}
			}
		})
	);
	...
	await page.route('**/api/v1/ops/commands', async (route) => {
		...
		await route.fulfill({
			json: { command: { ...route.request().postDataJSON(), id: 'saved-command', state: 'APPLIED' } }
		});
	});
...
test('document recovery uses the version returned by the protected preview', async ({ page, context, baseURL }) => {
		...
		route.fulfill({
			json: {
				command: {
					...route.request().postDataJSON(),
					current_version: 7,
					impact_preview: { effect: 'Queue a fresh scan; file stays blocked.', will_notify: false, audit: 'immutable' }
				}
			}
		})
	);
	let submitted: any;
	await page.route('**/api/v1/ops/commands', (route) => {
		submitted = route.request().postDataJSON();
		return route.fulfill({ json: { command: { ...submitted, id: 'scan-review', state: 'APPLIED' } } });
	});
	...
	expect(submitted).toMatchObject({
		command_type: 'retry_document_scan',
		target_type: 'document',
		target_id: 'scan-document',
		expected_version: 7
	});
});
