...
test('switching business resets pagination and ignores a late response', async ({ page }) => {
	...
	await page.route('**/api/v1/organizations/org-a/collections', route => route.fulfill({ json: { collections: Array.from({ length: 25 }, (_, i) => ({ id: `sale-${i}`, buyer_legal_name: `First buyer ${i}`, amount_kobo: 10000 })) } }));
