...
test('an invalid saved session shows only the account check before sign-in', async ({ page, context, baseURL }) => {
	await context.addCookies([
		{ name: 'kredit_session', value: 'expired-session', url: baseURL ?? 'http://127.0.0.1:5173' }
	]);
...
test('an expired session cannot flash the next page during an account navigation', async ({
	page,
	context,
	baseURL
}) => {
	await context.addCookies([
		{ name: 'kredit_session', value: 'expires-during-use', url: baseURL ?? 'http://127.0.0.1:5173' }
	]);
	let checks = 0;
	await page.route('**/api/v1/me', async (route) => {
		checks++;
		if (checks === 1) {
			await route.fulfill({
				...
				body: JSON.stringify({ user: { id: 'seller-1' }, session: { id: 'session-1' }, organizations: [] })
			});
			return;
		}
		await new Promise((resolve) => setTimeout(resolve, 500));
...
test('the secure payment link is public but never shows the seller account', async ({ page }) => {
	await page.route('**/api/v1/public/payment-intents/example', async (route) =>
		route.fulfill({
			status: 200,
			contentType: 'application/json',
			body: JSON.stringify({
				payment_intent: {
					reference: 'sale-example',
					supplier_name: 'Adebayo Supplies',
					description: 'Twenty bags of rice',
					amount_kobo: 25000000,
					payment_status: 'ready',
					provider_action: 'Continue to your approved payment provider.'
				}
			})
		})
	);
...
test('the signed-in payments page prioritizes money and items needing an answer', async ({
	...
	await page.route('**/api/v1/organizations/org-1/payments', async (route) =>
		route.fulfill({
			status: 200,
			contentType: 'application/json',
			body: JSON.stringify({
				payments: [
					{
						id: 'sale-1',
						buyer_legal_name: 'Kano Retail',
						description: 'Twenty bags of rice',
						amount_kobo: 40000000,
						source_type: 'integrated_voluntary',
						state: 'recognized',
						paid_at: '2026-08-29T09:00:00Z',
						reference: 'PAY-100'
					}
				]
			})
	...
	await page.route('**/api/v1/organizations/org-1/payment-claims', async (route) =>
		route.fulfill({
			status: 200,
			contentType: 'application/json',
			body: JSON.stringify({
				payment_claims: [
					{
						...
						transfer_reference: 'TRF-100',
						state: 'pending',
						paid_at: '2026-08-30T09:00:00Z',
						hold_expires_at: '2026-08-31T09:00:00Z'
					}
				]
			})
		})
...
test('payment review keeps the selected business fixed until dismissed', async ({ page, context, baseURL }) => {
	...
	await page.route('**/api/v1/organizations', (route) =>
		route.fulfill({
			json: {
				organizations: [
					{ id: 'org-first', legal_name: 'First business' },
					{ id: 'org-second', legal_name: 'Second business' }
				]
			}
		})
	);
	await page.route('**/api/v1/organizations/*/payments', (route) => route.fulfill({ json: { payments: [] } }));
	await page.route('**/api/v1/organizations/*/payment-claims', (route) =>
		route.fulfill({
			json: {
				payment_claims: [
					{
						...
						state: 'pending',
						paid_at: '2026-09-09T09:00:00Z',
						hold_expires_at: '2026-09-10T09:00:00Z'
					}
				]
			}
		})
