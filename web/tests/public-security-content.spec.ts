...
test('public security page uses service language instead of an internal launch checklist', async ({ page }) => {
	...
	await expect(page.getByRole('heading', { name: 'What we do to protect you.' })).toBeVisible();
	const content = await page.locator('main.trust-page').innerText();
	expect(content).not.toMatch(
		/pre[- ]launch|before we (?:go live|launch)|coming soon|outside reviewers check us before|tested before launch/i
	);
	expect(content).not.toMatch(/we never debit twice|nobody signs in as you|certified by Mono|independently certified/i);
	await expect(page.getByRole('heading', { name: 'We ask before we debit' })).toBeVisible();
	await expect(page.getByText(/A debit already sent to the bank may still finish/)).toBeVisible();
