...
test('a reversed receipt cannot be mistaken for an active payment', async ({ page }) => {
	await page.route('**/api/v1/public/receipts/audit', (route) =>
		route.fulfill({
			json: {
				receipt: {
					reference: 'payment-audit',
					amount_kobo: 150000,
					state: 'reversed',
	...
	await expect(page.getByRole('link', { name: 'Email Kredit support' })).toHaveAttribute(
		'href',
		/^mailto:hello@kredit\.ng/
	);
});
...
test('a paid-off payment link offers no further payment action', async ({ page }) => {
	await page.route('**/api/v1/public/payment-intents/audit', (route) =>
		route.fulfill({
			json: {
				payment_intent: {
					reference: 'sale-audit',
					supplier_name: 'Test seller',
					description: 'Goods',
					amount_kobo: 0,
					provider_action: 'Sign in to review.'
				}
			}
		})
	);
