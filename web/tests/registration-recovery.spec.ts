...
test('admin recovers an uncertain registration and sees a truthful empty state', async ({ page, context, baseURL }) => {
	...
	let done = false;
	await page.route('**/api/v1/ops/customer-registrations', (route) =>
		route.fulfill({
			json: {
				registrations: done
					? []
					: [{ id: 'attempt', business_id: 'business-reference', created_at: '2026-09-10T08:00:00Z' }]
			}
		})
	);
	await page.route('**/api/v1/ops/customer-registrations/attempt', (route) => {
		expect(route.request().postDataJSON()).toMatchObject({
			action: 'not_created',
			reason: 'Provider dashboard confirms this customer was not created.'
		});
		done = true;
	...
	await page
		.getByLabel('Evidence checked and reason')
		.fill('Provider dashboard confirms this customer was not created.');
