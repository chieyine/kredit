...
const profiles = organizations.map((org, i) => ({ id: `profile-${i}`, workspace_id: org.id, legal_name: org.legal_name }));
const sale = (business: string, amount: number) => ({ request: {
  id: `sale-${business}`, state: 'ACTIVE', buyer_business_id: business, buyer_legal_name: 'Distributor',
  principal_kobo: amount, due_date: '2026-10-01'
...
test.beforeEach(async ({ page, context, baseURL }) => {
  ...
  await page.route('**/api/v1/**', route => {
    ...
    if (path === '/api/v1/buyer/credit-requests') return route.fulfill({ json: { requests: [sale('profile-0', 10000), sale('profile-1', 25000)] } });
    ...
    const key = ({ 'distributor-imports': 'batches', 'credit-requests': 'requests', payments: 'payments', overdue: 'overdue', 'payment-claims': 'payment_claims', disputes: 'disputes', due: 'due' } as Record<string, string>)[path.split('/').at(-1)!];
...
test('distributor import preserves the originating business', async ({ page }) => {
  ...
  await expect(page.getByRole('link', { name: 'Track invitations' })).toHaveAttribute('href', '/workspace/partners/invitations?organization=org-b');
});
