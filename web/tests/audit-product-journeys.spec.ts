...
async function signedIn(page: Page, context: BrowserContext, baseURL?: string) {
 ...
 await page.route('**/api/v1/**', async route => {
  ...
  if (path === '/api/v1/buyer/businesses') return send(route,{businesses:[{id:'business-1',workspace_id:'buyer-workspace',legal_name:'Amina Stores'}]});
...
test('OTP has an expiry and resend countdown without exposing the full target', async ({ page }) => {
 ...
 await page.goto('/signin'); await page.getByRole('radio', { name: 'WhatsApp', exact: true }).check(); await page.getByRole('textbox', { name: 'Phone number' }).fill('08031234567'); await page.getByRole('button', { name: 'Send me a code' }).click();
...
test('payment decisions require a bank check and preserve their request identity', async ({ page, context, baseURL }) => {
 ...
 await page.goto('/workspace/money/received'); await page.getByRole('button', { name: 'Yes, I got the money' }).click();
...
test('native account menu traps focus and restores it on Escape', async ({ page, context, baseURL }) => {
 ...
 await page.goto('/workspace/settings');
 const trigger = page.getByRole('navigation', { name: 'Business workspace', exact: true }).getByRole('button', { name: 'Menu', exact: true });
 await trigger.click(); const dialog = page.getByRole('dialog', { name: /menu$/i });
...
for (const [label, path] of [['home', '/'], ['pricing', '/pricing'], ['supplier', '/workspace/today'], ['quick-sale', '/workspace/sales/quick'], ['full-sale', '/workspace/sales/new?advanced=1'], ['buyer', '/workspace/purchases/orders/sale-1']] as const) {
 test(`visual and accessibility evidence: ${label}`, async ({ page, context, baseURL }, testInfo) => {
  ...
  for (const width of [390, 1440]) {
   ...
   if (path.startsWith('/signin/') || path.startsWith('/workspace/purchases/')) await expect(page.locator('.account-gate')).toHaveCount(0);
