...
const orgs = [
	...
	{ id: 'org-b', legal_name: 'Borno Supplies', trading_name: 'Borno Supplies' }
];
const sale = (id = 'sale-1', state = 'BUYER_REVIEWING') => ({
	request: {
		id,
		state,
		supplier_legal_name: 'Kora Wholesale',
		buyer_legal_name: 'Amina Stores',
		buyer_user_id: 'user-1',
		buyer_business_id: 'business-1',
		principal_kobo: 12750049,
		goods_description: '40 cartons of cooking oil',
		due_date: '2026-09-18',
	...
	obligation: null as unknown,
	purchasing_actions: ['view', 'accept', 'review', 'receive', 'bank', 'payments', 'disputes']
});
...
async function signedIn(page: Page, context: BrowserContext, baseURL?: string) {
	await context.addCookies([
		{ name: 'kredit_session', value: 'synthetic-audit-session', url: baseURL ?? 'http://127.0.0.1:5173' },
		{ name: 'kredit_csrf', value: 'synthetic-csrf', url: baseURL ?? 'http://127.0.0.1:5173' }
	]);
	await page.route('**/api/v1/**', async (route) => {
		const path = new URL(route.request().url()).pathname;
		if (path === '/api/v1/buyer/businesses')
			return send(route, {
				businesses: [{ id: 'business-1', workspace_id: 'buyer-workspace', legal_name: 'Amina Stores' }]
			});
		...
		if (path === '/api/v1/pricing')
			return send(route, { policy_revision: 1, base_bps: 50, collection_bps: 50, min_fee_kobo: 0 });
				...
				customers: [
					{
						...
						buyer_business_id: 'business-1',
						legal_name: 'Amina Stores',
						trading_name: 'Amina Stores',
						state: 'verified'
					}
				]
			});
		...
		if (path.endsWith('/reports/receivables'))
			return send(route, { summary: { obligation_count: 1, outstanding_kobo: 12750049, overdue_kobo: 0 } });
...
test('unavailable balance, payments and disputes never produce an all-clear', async ({ page, context, baseURL }) => {
	...
	for (const suffix of ['reports/receivables', 'payments', 'disputes'])
		await page.route(`**/organizations/org-a/${suffix}`, (route) =>
...
test('switching businesses cannot show an earlier balance under the new name', async ({ page, context, baseURL }) => {
	...
	await page.route('**/organizations/org-a/reports/receivables', async (route) => {
		await new Promise<void>((resolve) => {
			finishA = resolve;
		});
		await send(route, { summary: { obligation_count: 1, outstanding_kobo: 12345600, overdue_kobo: 0 } }).catch(
			() => {}
		);
	});
	await page.route('**/organizations/org-b/reports/receivables', (route) =>
		send(route, { summary: { obligation_count: 2, outstanding_kobo: 9876549, overdue_kobo: 0 } })
	);
	await page.goto('/workspace/today');
...
test('accepting a sale does not automatically grant bank permission or announce release readiness', async ({
	page,
	context,
	baseURL
}) => {
	await signedIn(page, context, baseURL);
	let state = 'BUYER_REVIEWING';
	let mandateCalls = 0;
	await page.route('**/api/v1/buyer/credit-requests/sale-1', (route) => send(route, sale('sale-1', state)));
	await page.route('**/api/v1/buyer/credit-requests/sale-1/accept', (route) => {
		state = 'BUYER_ACCEPTED';
		return send(route, sale('sale-1', state));
	});
	await page.route('**/api/v1/buyer/credit-requests/sale-1/mandate', (route) => {
		mandateCalls++;
		return send(route, { code: 'unexpected' }, 500);
	});
...
test('pending hosted permission stays pending after return parameters', async ({ page, context, baseURL }) => {
	await signedIn(page, context, baseURL);
	const current = sale('sale-1', 'BUYER_ACCEPTED');
	current.mandate = {
		id: 'mandate-1',
		provider_id: 'provider-1',
		provider: 'mono-sweep',
		status: 'PENDING',
		authorization_url: 'https://authorise.mono.co/synthetic-example'
	};
	await page.route('**/api/v1/buyer/credit-requests/sale-1', (route) => send(route, current));
	await page.goto('/workspace/purchases/orders/sale-1?success=true&status=approved');
	await expect(page.getByRole('link', { name: /Continue to bank authorization/ })).toHaveAttribute(
		'href',
		'https://authorise.mono.co/synthetic-example'
	);
...
test('untrusted hosted permission links are never rendered', async ({ page, context, baseURL }) => {
	...
	current.mandate = {
		...
		provider: 'mono-sweep',
		status: 'PENDING',
		authorization_url: 'https://bank-permission.evil.test/steal'
	};
	await page.route('**/api/v1/buyer/credit-requests/sale-1', (route) => send(route, current));
	await page.goto('/workspace/purchases/orders/sale-1');
...
test('OTP has an expiry and resend countdown without exposing the full target', async ({ page }) => {
	await page.route('**/api/v1/me', (route) => send(route, { code: 'authentication_required' }, 401));
	await page.route('**/api/v1/auth/otp/challenges', (route) =>
		send(
			route,
			{ challenge_id: 'challenge-1', expires_at: new Date(Date.now() + 300000).toISOString(), channel: 'sms' },
			202
		)
	);
	await page.goto('/signin');
...
test('quick sale uses the server timing preview and exact kobo', async ({ page, context, baseURL }) => {
	...
	await page.route('**/organizations/org-a/credit-requests', async (route) => {
		if (route.request().method() === 'POST') {
			...
			return send(route, { request: { id: 'created-sale' } }, 201);
		}
...
test('exact typed pricing supports voluntary and partial repayment examples', async ({ page }) => {
	await page.route('**/api/v1/pricing', (route) =>
		send(route, { policy_revision: 1, base_bps: 50, collection_bps: 50 })
	);
	await page.goto('/pricing');
	await page.getByRole('textbox', { name: 'Sale amount (₦)', exact: true }).fill('127,500.49');
	const example = page.getByLabel('Fee example');
	await expect(example).toContainText('₦127,500.49');
	await expect(example).toContainText('₦637.50');
	await page.getByRole('textbox', { name: 'Amount Kredit successfully collects (₦)' }).fill('40,000.25');
	await expect(example).toContainText('₦200.00');
	await expect(example).toContainText('₦837.50');
});

test('payment decisions require a bank check and preserve their request identity', async ({
	page,
	context,
	baseURL
}) => {
	...
	const claim = {
		...
		state: 'pending',
		amount_kobo: 10049,
		transfer_reference: 'TEST-TRANSFER-1',
		paid_at: '2026-09-06T12:00:00Z',
		hold_expires_at: '2026-09-07T12:00:00Z'
	};
	await page.route('**/organizations/org-a/payment-claims', (route) => send(route, { payment_claims: [claim] }));
	const keys: string[] = [];
	await page.route('**/payment-claims/claim-1/decide', (route) => {
		keys.push(route.request().headers()['idempotency-key']);
		return keys.length === 1 ? route.abort('failed') : send(route, { payment_claim: { ...claim, state: 'confirmed' } });
	});
	...
	await expect(dialog).toContainText('₦100.49');
	await expect(dialog.getByRole('button', { name: 'Confirm received' })).toBeDisabled();
...
for (const [label, path] of [
	['home', '/'],
	['pricing', '/pricing'],
	['supplier', '/workspace/today'],
	['quick-sale', '/workspace/sales/quick'],
	['full-sale', '/workspace/sales/new?advanced=1'],
	['buyer', '/workspace/purchases/orders/sale-1']
] as const) {
	test(`visual and accessibility evidence: ${label}`, async ({ page, context, baseURL }, testInfo) => {
		...
		for (const width of [390, 1440]) {
			...
			if (path.startsWith('/signin/') || path.startsWith('/workspace/purchases/'))
			...
			expect(
				...
				JSON.stringify(
					results.violations.map((item) => ({ id: item.id, nodes: item.nodes.map((node) => node.target) }))
				)
			).toEqual([]);
...
test('full sale keeps business identity and server-reviewed timing on the invoice path', async ({
	page,
	context,
	baseURL
}) => {
			...
			customers: [
				{
					buyer_user_id: 'buyer-1',
					buyer_business_id: 'business-1',
					legal_name: 'First shop',
					trading_name: 'First shop',
					state: 'verified'
				},
				{
					buyer_user_id: 'buyer-1',
					buyer_business_id: 'business-2',
					legal_name: 'Second shop',
					trading_name: 'Second shop',
					state: 'verified'
				}
			]
		})
	);
	await page.route('**/api/v1/organizations/org-a/credit-terms/preview', (route) =>
		send(route, {
			due_date: '2026-09-18',
			grace_hours: 24,
			collection_at: '2026-09-19T22:59:00Z',
	...
	await page.route('**/api/v1/organizations/org-a/credit-requests', (route) => {
		if (route.request().method() === 'POST') {
	...
	await page.goto('/workspace/sales/new?advanced=1&organization=org-a');
