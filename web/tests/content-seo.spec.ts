...
test('guide renders useful content without invented publication or research claims', async ({ page }) => {
	...
	await expect(page.locator('link[rel="canonical"]')).toHaveAttribute(
		'href',
		'https://kredit.ng/blog/how-to-sell-goods-on-credit-in-nigeria'
	);
	...
	await expect(
		page.getByRole('navigation', { name: 'Breadcrumb' }).getByRole('link', { name: 'Credit sales' })
	).toHaveAttribute('href', '/blog/topic/credit-sales');
...
test('topic hubs give every article a crawlable route into its guide cluster', async ({ page, request }) => {
	...
	await expect(page.locator('link[rel="canonical"]')).toHaveAttribute(
		'href',
		'https://kredit.ng/blog/topic/customer-checks'
	);
...
test('sitemap and RSS publish the guide library for discovery', async ({ request }) => {
	...
	expect(sitemapText).not.toContain('/blog/cash-sale-vs-credit-sale');
	expect(sitemapText).toContain('/blog/how-to-sell-goods-on-credit-in-nigeria');
	const rss = await request.get('/blog/rss.xml');
	expect(rss.ok()).toBeTruthy();
	const rssText = await rss.text();
	expect(rssText).toContain('<rss version="2.0"');
	expect(rssText).toContain('rel="self"');
	expect(rssText).toContain('<lastBuildDate>');
	expect(rssText).not.toContain('<pubDate>');
});
...
test('terms explain the complete sale, payment and complaint journey', async ({ page }) => {
	...
	await expect(
		page.getByText('Kredit is not a bank, wallet, credit bureau, insurance company, marketplace or debt buyer.')
	).toBeVisible();
	await expect(page.locator('meta[name="robots"]')).toHaveAttribute(
		'content',
		'index,follow,max-image-preview:large,max-snippet:-1'
