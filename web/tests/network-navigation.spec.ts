...
test('business context stays attached without contaminating a different explicit workspace', () => {
	...
	expect(workspaceHref('/workspace/purchases/orders', current)).toBe(
		'/workspace/purchases/orders?organization=org-a&business_id=buyer-a'
	);
	expect(workspaceHref('/workspace/sales', current)).toBe('/workspace/sales?organization=org-a');
	expect(workspaceHref('/workspace/purchases?organization=org-b', current)).toBe(
		'/workspace/purchases?organization=org-b'
	);
	expect(workspaceHref('/workspace/purchases?business_id=buyer-b', current)).toBe(
		'/workspace/purchases?business_id=buyer-b'
	);
...
test('bank authorization accepts only the current internal route and approved HTTPS hosts', () => {
	...
	expect(hostedAuthorizationURL(`/workspace/purchases/bank-authorization/${reference}`, 'native')).toBe(
		`/workspace/purchases/bank-authorization/${reference}`
	);
