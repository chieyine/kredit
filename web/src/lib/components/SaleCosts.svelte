...
<style>.sale-costs{border:1px solid var(--color-border,var(--color-border));border-radius:.75rem;padding:1.25rem;margin:1rem 0}h2{font-size:1.25rem}dl{display:grid;grid-template-columns:repeat(auto-fit,minmax(14rem,1fr));gap:1rem}dt{color:var(--color-muted,var(--color-primary));font-size:.9rem}dd{margin:.35rem 0;font-weight:650}p{line-height:1.6}</style>
