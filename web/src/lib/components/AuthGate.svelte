<script lang="ts">
	...
	setContext<AccountContext>(ACCOUNT_CONTEXT, {
		get userID() {
			return userID;
		}
	});
	async function verify() {
		...
		userID = '';
		try {
			const id = await checkedJSON('/api/v1/me', (value) => text(record(record(value).user).id), {
	...
	<main class="account-gate" aria-live="polite">
		...
		<section>
			...
			{#if gateStatus === 'checking'}
				...
				<p>Your private records will open after this check.</p>
			{:else}
				<h1>We cannot open your account.</h1>
				<p>The account check did not finish. Check your connection and try again.</p>
				<div class="gate-actions">
					<button type="button" onclick={verify}>Try again</button><a href="/">Go to the home page</a>
				</div>
			{/if}
		</section>
	</main>
