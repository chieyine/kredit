...
<style>section{margin:2rem 0}section>div{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:1rem}a{display:grid;gap:.6rem;border:1px solid var(--color-border);background:var(--color-surface);padding:1rem;color:var(--color-primary);text-decoration:none}strong{font:2rem var(--font-serif)}span{font-size:.85rem}</style>
