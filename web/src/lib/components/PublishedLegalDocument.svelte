<script lang="ts">
	...
	const sections = $derived(
		publication.copy.sections.map((section, index) => ({ id: `section-${index + 1}`, title: section.heading }))
	);
	function pieces(text: string) {
		return text
			...
			.map((value) => ({
				text: value,
				href: /^https?:\/\//i.test(value) ? value : /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value) ? `mailto:${value}` : null
			}));
	}
</script>

<DocumentLayout
	title={publication.copy.title}
	description={publication.copy.introduction}
