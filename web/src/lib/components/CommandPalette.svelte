<script lang="ts">
	...
	function trapFocus(event: KeyboardEvent) {
		...
		const focusable = Array.from(
			dialog.querySelectorAll<HTMLElement>('input, button, a[href], select, textarea, [tabindex]:not([tabindex="-1"])')
			...
			{#each results as [label, href], index (href)}
				<li>
					<button
						...
						onclick={() => pick(href)}>{label}<span>{href}</span></button
					>
				</li>
			{:else}
