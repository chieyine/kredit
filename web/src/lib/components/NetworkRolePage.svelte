...
<main class="network-page shell">
  ...
  <section class="network-section" aria-labelledby="role-steps">
    ...
    <ol class="steps">
      {#each stage.steps as [title, body]}
        <li><div><h3>{title}</h3><span>{body}</span></div></li>
      {/each}
    </ol>
  </section>
  ...
  <section class="network-cta">
    <div>
      <h2>{role === 'consumers' ? 'Already received a purchase link?' : 'Start with your next trading relationship.'}</h2>
      <p>{role === 'consumers' ? 'Open that link to go straight to the purchase. Your account keeps the accepted record available afterwards.' : 'One business account for the credit you give and receive. Each relationship keeps its own terms and balance.'}</p>
    </div>
