...
{/if}
<style>
  .resource-loading{color:var(--color-muted);padding:.75rem 0;line-height:1.5}.resource-error{display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:.75rem;padding:1rem;background:var(--color-background);border:1px solid var(--color-warning);border-radius:.4rem;color:var(--color-warning);margin:1rem 0}.resource-error p{margin:.25rem 0 0;font-size:.9rem;line-height:1.5}.resource-error button{background:transparent;color:inherit;border:1px solid currentColor;min-height:2.75rem;padding:.55rem .8rem;border-radius:.3rem;cursor:pointer}
</style>
