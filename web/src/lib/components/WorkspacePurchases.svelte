<script lang="ts">
	...
	const outstanding = $derived(
		sumKobo(
			purchases.map((item) => item.obligation?.outstanding_kobo ?? (beforeDebt.has(item.request.state) ? 0 : null))
		)
	);
	...
	async function load(scope: string) {
		...
		purchases = [];
		try {
			const profiles = await checkedJSON(
			...
			if (!profile) return;
			const sales = await checkedJSON(
				`/api/v1/buyer/credit-requests?business_id=${encodeURIComponent(profile.id)}&organization=${encodeURIComponent(scope)}`,
...
<section class="purchasing-summary" aria-label="Supplier balances" aria-busy={loading}>
	...
	{:else if error}
		...
		<button onclick={() => load(organizationID)}>Try purchases again</button>
	{:else if !businessID}
		<p>
			No purchasing profile is available to your account for this business. Its owner can connect it when accepting a
			supplier invitation.
		</p>
	{:else}
		<strong class="amount"
			>{#if outstanding === null}Not confirmed{:else}<Money amountKobo={outstanding} />{/if}</strong
		>
		<p>
			{outstanding === null
				? 'One or more obligations could not be confirmed. Review the individual purchases.'
				: 'Supplier obligations for this business. Customer receivables do not reduce this amount.'}
		</p>
