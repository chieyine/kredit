...
<details><summary>Prove it is really you before this change</summary><p>Open your authenticator app and type the six digits it shows. <a href="/account/security">Account safety settings</a>.</p><form onsubmit={(e)=>{e.preventDefault();verify()}}><label>Authenticator code<input disabled={busy} bind:value={code} inputmode="numeric" autocomplete="one-time-code" pattern={'[0-9]{6}'} maxlength="6" required/></label><button disabled={busy||!/^\d{6}$/.test(code)}>Confirm it is me</button></form>{#if message}<p role="status">{message}</p>{/if}</details>
<style>details{padding:1rem;border:1px solid var(--color-border);margin:1rem 0}input,button{padding:.7rem;margin:.5rem}input{width:8rem}summary{cursor:pointer}</style>
