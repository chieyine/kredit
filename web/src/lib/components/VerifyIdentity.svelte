...
<details><summary>Prove it is really you before this change</summary><p>Open your authenticator app and type the six digits it shows. <a href="/app/settings/security">Account safety settings</a>.</p><form onsubmit={(e)=>{e.preventDefault();verify()}}><label>Authenticator code<input bind:value={code} inputmode="numeric" autocomplete="one-time-code" pattern="[0-9]{6}" maxlength="6" required/></label><button disabled={busy}>Confirm it is me</button></form>{#if message}<p role="status">{message}</p>{/if}</details>
