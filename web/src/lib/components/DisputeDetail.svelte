<script lang="ts">
  ...
  let files=$state<FileList>();let uploaded=$state<{id:string;file_name:string;scan_state:string}|null>(null);
 let statement = $state(''), documentID = $state(''), outcome = $state('UPHELD'), valid = $state('0'), adjustment = $state('0'), remaining = $state('0'), reason = $state('');
  ...
  async function uploadFile(){
    ...
    if(file.size===0||file.size>2*1024*1024||!['application/pdf','image/jpeg','image/png'].includes(file.type)){error='Choose a PDF, JPG or PNG up to 2 MB.';return}
    const scope=endpoint;busy=true;error='';
    try{
      const content=await new Promise<string>((resolve,reject)=>{const reader=new FileReader();reader.onerror=()=>reject(new Error('We could not read the file.'));reader.onload=()=>resolve(String(reader.result).split(',')[1]??'');reader.readAsDataURL(file)});
      const upload=new MutationIntent(account.userID,`${scope}/documents`);
      const doc=await upload.run({file_name:file.name,content_type:file.type,content_base64:content},value=>{const row=record(record(value).document);return{id:text(row.id),file_name:text(row.file_name),scan_state:text(row.scan_state)}});
      if(scope!==endpoint)return;uploaded=doc;documentID=doc.id;
    }catch(cause){if(scope===endpoint)error=cause instanceof Error?cause.message:'The upload could not be confirmed.'}finally{if(scope===endpoint)busy=false}
  }
  async function checkScan(){if(!uploaded||busy)return;const scope=endpoint;busy=true;error='';try{const doc=await checkedJSON(`${scope}/documents/${encodeURIComponent(uploaded.id)}`,value=>{const row=record(record(value).document);return{id:text(row.id),file_name:text(row.file_name),scan_state:text(row.scan_state)}});if(scope===endpoint)uploaded=doc}catch(cause){if(scope===endpoint)error=publicError(cause,'the file safety check')}finally{if(scope===endpoint)busy=false}}
  async function openDocument(id:string){try{const url=await checkedJSON(`${endpoint}/documents/${encodeURIComponent(id)}/download`,value=>{const url=new URL(text(record(value).url),location.origin);if(!['https:','http:'].includes(url.protocol)||url.username||url.password||(location.protocol==='https:'&&url.protocol!=='https:'))throw new Error('Invalid document link');return url.href});location.assign(url)}catch(cause){error=publicError(cause,'this document')}}
  async function submitEvidence(event: SubmitEvent) {
    ...
    try {
      ...
      statement = ''; documentID = ''; uploaded=null; files=undefined; notice = 'Information added.';
      await load();
    } catch (cause) {
  ...
  $effect(() => {
    endpoint;
    dispute = null; evidence = []; decisions = []; busy = false; notice = ''; statement = ''; documentID = ''; uploaded=null; files=undefined; reason = '';
    void load();
...
{#if loading}<p role="status">Opening this problem…</p>{:else if error && !dispute}<p class="error" role="alert">{error} <button onclick={load}>Try again</button></p>{:else if dispute}
	...
	<div class="columns"><section class="card"><h2>What has been added</h2>{#if evidence.length}<ol>{#each evidence as item}<li><p>{item.statement||'A document was added.'}</p>{#if item.document_id}<button type="button" onclick={()=>openDocument(item.document_id)}>Open document</button>{/if}<small>{timeLabel(item.submitted_at)}</small></li>{/each}</ol>{:else}<p>Nothing has been added yet.</p>{/if}<form onsubmit={submitEvidence}><h3>Add more information</h3><label>What else should we know?<textarea bind:value={statement} required rows="4" disabled={busy}></textarea></label><label>Attach a PDF or image <small>optional, up to 2 MB</small><input type="file" accept="application/pdf,image/jpeg,image/png" bind:files disabled={busy||!!uploaded}/></label>{#if uploaded}<p>{uploaded.file_name}: {uploaded.scan_state==='CLEAN'?'Ready to attach':uploaded.scan_state==='PENDING'?'Safety check pending':'File is not ready to attach'}</p>{#if uploaded.scan_state!=='CLEAN'}<button type="button" disabled={busy} onclick={checkScan}>Check file status</button>{/if}<button type="button" disabled={busy} onclick={()=>{uploaded=null;documentID='';files=undefined}}>Remove attachment</button>{:else if files?.length}<button type="button" disabled={busy} onclick={uploadFile}>Upload file</button>{/if}<button class="primary" disabled={busy||!statement.trim()||!!files?.length&&!uploaded||!!uploaded&&uploaded.scan_state!=='CLEAN'}>{busy?'Adding…':'Add this information'}</button></form></section>
