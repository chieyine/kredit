...
export function decodeImportBatch(value: unknown): ImportBatch {
	...
	if (!Number.isInteger(v.row_count) || (v.row_count as number) < 1 || (v.row_count as number) > 200)
	...
	if (!/^[a-f0-9]{64}$/.test(hash) || !Number.isFinite(Date.parse(created)))
		throw new Error('Invalid import identity.');
	if (
		!Array.isArray(v.completed_rows) ||
		!v.completed_rows.every((n) => Number.isInteger(n) && n >= 1 && n <= size) ||
		new Set(v.completed_rows).size !== v.completed_rows.length
	)
		throw new Error('Invalid import progress.');
	...
	if (v.contacts !== undefined) {
		...
		contacts = v.contacts.map((value) => {
			...
			return {
				source_reference: text(c.source_reference),
				target: text(c.target),
				target_type: text(c.target_type),
				legal_name: text(c.legal_name),
				trading_name: typeof c.trading_name === 'string' ? c.trading_name : '',
				business_type: text(c.business_type),
				business_address: text(c.business_address),
				industry: text(c.industry)
			};
		});
		if (new Set(contacts.map((c) => c.source_reference)).size !== size)
			throw new Error('Duplicate saved contact identities.');
	}
	return {
		id: text(v.id),
		state: state as ImportBatch['state'],
		row_count: size,
		source_hash: hash,
		created_at: created,
		contacts,
		completed_rows: v.completed_rows as number[]
	};
}
