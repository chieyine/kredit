...
export class MutationError extends UserFacingError {
	...
	constructor(
		message: string,
		outcome: MutationError['outcome'],
		readonly recoveryReference = ''
	) {
		super(recoveryReference ? `${message} Support reference: ${recoveryReference}.` : message);
...
function canonical(value: unknown): unknown {
	...
	if (typeof value === 'number' && !Number.isFinite(value))
		throw new MutationError('Check the amount before continuing.', 'not_sent');
	return value;
}
...
export class MutationIntent {
		...
		try {
			...
			if (raw) {
				...
				if (
					...
					!/^kredit-[0-9a-f]{32}$/.test(item.key) ||
					...
					this.saved = item as StoredIntent;
					this.bornAt = item.createdAt;
				}
			}
		} catch {
			this.blocked = true;
	...
	get unresolved() {
		return this.blocked || this.saved !== null;
	}
	private retain(item: StoredIntent) {
		if (!this.storage)
			throw new MutationError(
				'Your browser cannot keep this request safe. Enable site storage or use another browser before submitting.',
				'not_sent'
			);
	...
	private clear() {
		try {
			...
			this.blocked = true;
			throw new MutationError(
				'The response arrived, but this browser could not clear its saved request. Check the record before making another change.',
				'unknown',
	...
	async run<T>(payload: unknown, decode: Decoder<T>, method = 'POST'): Promise<T> {
		...
		if (this.blocked)
			throw new MutationError(
				'An earlier request could not be verified. Check the record or contact support before submitting another.',
				'unknown',
				this.recoveryReference
			);
		if (typeof navigator !== 'undefined' && !navigator.onLine) {
			if (this.unresolved)
				throw new MutationError(
					'You are offline and an earlier request is still unconfirmed. Reconnect before checking its record or retrying the same details.',
					'unknown',
					this.recoveryReference
				);
		...
		try {
			...
			if (!globalThis.crypto?.subtle)
				throw new MutationError('Use an updated, secure browser to submit this request.', 'not_sent');
			const fingerprint = Array.from(
				new Uint8Array(
					await crypto.subtle.digest('SHA-256', new TextEncoder().encode(`${method}\n${this.url}\n${body ?? ''}`))
				),
				(byte) => byte.toString(16).padStart(2, '0')
			).join('');
			if (this.saved && this.saved.fingerprint !== fingerprint)
				throw new MutationError(
					'Your earlier request may already have completed. Check the record or retry with the same details before making a different request.',
					'unknown',
					this.recoveryReference
				);
			// Do not silently expire unknown requests and submit them under a fresh key.
			if (this.saved && Date.now() - Date.parse(this.saved.createdAt) > 15 * 60 * 1000)
				throw new MutationError(
					'This earlier request needs a record check. Do not submit it again; open the sale history or contact support.',
					'unknown',
					this.recoveryReference
				);
			const item: StoredIntent = this.saved ?? {
				version: 1,
				key: randomKey(),
				fingerprint,
			...
			this.retain(item);
			sent = true;
			const result = await checkedJSON(this.url, decode, {
				method,
				headers: {
					...csrfHeader(),
			...
			if (sent && error instanceof RequestError && error.status === 409 && error.code === 'credit_approval_required') {
				this.clear();
				throw new MutationError(
					'Request internal approval for these exact terms. Once approved, you can send the offer.',
					'rejected'
				);
			}
			...
			if (
				...
				error instanceof RequestError &&
				error.status === 409 &&
				error.code === 'financial_difference_unresolved'
			) {
				this.clear();
				throw new MutationError(
					'Financial discrepancy remains unresolved. Correct the underlying records before closing this review.',
					'rejected'
				);
			}
			if (
			...
			throw new MutationError(
				sent
					? 'We have not confirmed the result. Check the record or retry the same request; do not submit it as a new request.'
					: 'This request has not been sent. Check your browser and connection.',
				sent ? 'unknown' : 'not_sent',
				sent ? this.recoveryReference : ''
			);
		} finally {
			this.busy = false;
		}
