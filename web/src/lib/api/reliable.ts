...
export function record(value: unknown): Record<string, unknown> {
	if (!value || typeof value !== 'object' || Array.isArray(value))
		throw new RequestError('The response was incomplete.', 0, 'invalid_response');
...
export async function readResource<T>(
	...
	try {
		return {
			state: 'ready',
			scope,
			data: await checkedJSON(url, decode, { signal }),
		...
		return {
			state: 'error',
			scope,
			message: publicError(error, subject),
...
export function randomKey(): string {
	if (!globalThis.crypto?.getRandomValues)
		throw new RequestError('This browser cannot securely submit the request. Use an updated browser.');
...
export function safeNext(value: string | null, origin: string): string {
	...
	try {
		...
		return parsed.origin === origin && parsed.pathname !== '/signin'
...
export function normalizeNigerianPhone(value: string): string {
	...
	if (/^\+234\d{10}$/.test(compact)) return compact;
	throw new RequestError(
		'Enter an 11-digit Nigerian phone number, or a number starting with +234.',
		400,
		'invalid_phone'
	);
}
