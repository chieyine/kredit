...
export async function loadSaleDefaults(organizationID: string, signal: AbortSignal) {
 return checkedJSON(`/api/v1/organizations/${encodeURIComponent(organizationID)}/onboarding`, value => {
  const profile = record(record(value).profile);
  const configured = typeof profile.default_credit_policy_updated_at === 'string' && !profile.default_credit_policy_updated_at.startsWith('0001-');
  ...
  if (typeof paymentDays !== 'number' || !Number.isInteger(paymentDays) || paymentDays < 1 || paymentDays > 365 || typeof graceHours !== 'number' || !Number.isInteger(graceHours) || graceHours < 0 || graceHours > 720) throw new Error('Your saved payment settings could not be confirmed. Check your credit policy settings.');
