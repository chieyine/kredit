...
import {
	websiteDefaults as defaults,
	websitePages as pages,
	type WebsitePage,
	type WebsiteCopy
	...
	articles.map((a) => [
		'guide-' + a.slug,
		{
			...
			sections: a.sections.map((s) => ({
				...
				...(s.points?.length ? { points: s.points } : {})
			})),
			guide: {
				description: a.description,
				category: a.category,
				keyphrase: a.keyphrase,
				faq: a.faq.map((q) => ({ heading: q.question, body: q.answer })),
				sources: a.sources,
...
export const websitePages: Record<WebsitePage, string> = {
	...pages,
	...Object.fromEntries(articles.map((a) => ['guide-' + a.slug, 'Guide: ' + a.title]))
};
