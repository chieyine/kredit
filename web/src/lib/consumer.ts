...
export function kobo(v:string){const amount=parseNaira(v);if(amount<0)throw new Error('Enter a valid naira amount with no more than two decimal places.');return amount;}
...
export class Mutation {
 ...
 async retry(): Promise<unknown> {
  if (!this.pending) throw new Error('Re-enter the original details to recover this action, or check its saved record.');
 ...
 async send(url: string, body: unknown, decode: Decoder<unknown> = purchase): Promise<unknown> {
  ...
  if (this.pending && (this.pending.url !== url || this.pending.body !== encoded)) throw new Error('Retry the previous action with its original details before starting another.');
  let intent = this.intents.get(url);
  if (!intent) { intent = new MutationIntent('consumer', url); this.intents.set(url, intent); }
