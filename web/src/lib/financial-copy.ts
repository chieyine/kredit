...
export function disputeEffectCopy(effect: string, amount?: KoboValue): string {
	if (effect === 'FULL_BLOCK')
		return `New bank debit requests for this sale will be on hold once this report is saved. ${submittedDebitCaveat}`;
	if (effect === 'NO_AUTOMATIC_BLOCK')
		return `This report does not request an automatic hold. Eligible payments may continue. ${submittedDebitCaveat}`;
