...
export function validFeeTerms(value: unknown): value is FeeTerms {
	...
	if (item.min_fee_kobo !== undefined && (exactKobo(item.min_fee_kobo) === null || exactKobo(item.min_fee_kobo)! < 0n))
		return false;
	return (
		Number.isInteger(item.base_bps) &&
		Number.isInteger(item.collection_bps) &&
		Number(item.base_bps) >= 0 &&
		Number(item.base_bps) <= 1000 &&
		Number(item.collection_bps) >= 0 &&
		Number(item.collection_bps) <= 1000
	);
}
...
export function feeForKobo(value: KoboValue, basisPoints: number): bigint | null {
	...
	if (amount === null || amount < 0n || !Number.isInteger(basisPoints) || basisPoints < 0 || basisPoints > 1000)
		return null;
...
export function baseFeeForKobo(value: KoboValue, terms: FeeTerms): bigint | null {
	...
	const amount = exactKobo(value),
		percentage = feeForKobo(value, terms.base_bps),
		floor = exactKobo(terms.min_fee_kobo ?? 0);
