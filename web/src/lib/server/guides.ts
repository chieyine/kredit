...
export async function loadGuides(fetcher: typeof fetch): Promise<Article[]> {
	...
	try {
		const response = await fetcher('/api/v1/website-guides', {
			credentials: 'omit',
			cache: 'no-store',
		...
		if (!data.publications || typeof data.publications !== 'object' || Array.isArray(data.publications))
			throw new Error('Incomplete guide publications');
		result = result.map((article) => {
			const value = data.publications[article.slug];
			if (value === undefined) return article;
			const publication = decodeWebsitePublication(value),
			...
			const wordCount = [
				copy.introduction,
				...sections.flatMap((s) => [s.heading, ...s.paragraphs, ...(s.points ?? [])]),
			...
			return {
				...
				category: g.category as Article['category'],
				keyphrase: g.keyphrase,
				sections,
				faq,
				sources: g.sources,
				related: g.related.map((slug) => ({ slug, title: '' })),
				wordCount,
				readingMinutes: Math.max(1, Math.ceil(wordCount / 220)),
				modified: publication.published_at.slice(0, 10)
			};
		});
	} catch {
		/* Public educational copy remains available during a content outage. */
	}
	...
	return result.map((a) => ({
		...
		related: a.related.map((r) => ({
			...r,
			title: published.get(r.slug)?.title ?? r.title,
			category: published.get(r.slug)?.category
		}))
	}));
}
