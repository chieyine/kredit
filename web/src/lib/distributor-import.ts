...
export function parseDistributorCSV(source: string): DistributorRow[] {
	...
	function finishRow() {
		...
		row = [];
		if (records.length > 201) throw new Error('Import at most 200 distributors at a time.');
	}
	for (let i = 0; i < input.length; i++) {
		...
		if (c === '"') {
			if (cell || closed) throw new Error('A quoted field must start immediately after a comma.');
			quoted = true;
	...
	return records.map((values, index) => {
		...
		if (!/^[A-Za-z0-9][A-Za-z0-9._:-]{0,99}$/.test(entry.source_reference))
			throw new Error(
				`Row ${line}: use a stable customer reference, up to 100 letters, numbers, dots, colons or hyphens.`
			);
		...
		else if (entry.target_type === 'email' && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(entry.target))
			entry.target = entry.target.toLowerCase();
		...
		if (contacts.has(contact))
			throw new Error(`Row ${line}: duplicate contact; review the intended business before inviting.`);
		contacts.add(contact);
		if (!entry.legal_name || !entry.business_address || !entry.industry)
			throw new Error(`Row ${line}: business name, address and industry are required.`);
		if (
			!['unregistered_business', 'registered_business', 'sole_proprietor', 'limited_company', 'partnership'].includes(
				entry.business_type
			)
		)
			throw new Error(`Row ${line}: select a business type from the template instructions.`);
