...
export function attentionItems(organizationID: string, sales: SaleView[], claims: WorkRow[], overdue: WorkRow[], disputes: WorkRow[]): AttentionItem[] {
  ...
  for (const item of disputes.filter(item => ['OPEN', 'UNDER_REVIEW', 'PENDING'].includes(item.state.toUpperCase()))) items.push({ id: `dispute-${item.id}`, rank: 0, title: 'A sale needs review', detail: item.reason || 'Check the reported problem and the amount on hold.', href: `/app/disputes/${encodeURIComponent(item.id)}${query}`, action: 'Review problem' });
  for (const item of claims.filter(item => item.state.toUpperCase() === 'PENDING')) items.push({ id: `claim-${item.id}`, rank: 1, title: `${customerName(item)} reported a payment`, detail: `${formatKobo(item.amount_kobo)} · Check your bank account before confirming.`, href: `/app/payments${query}`, action: 'Check payment' });
