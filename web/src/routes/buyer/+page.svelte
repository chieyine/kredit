 ...
 async function load(){
  ...
  try{
   const [account,sales,days]=await Promise.allSettled([
    checkedJSON('/api/v1/buyer/me',value=>{const portal=record(record(value).portal);text(record(portal.person).full_name);text(record(portal.business).legal_name);return portal;},{signal:request.signal}),
...
<main class="shell buyer-home">
	{#if portal}
		...
		{#if error}<p role="alert">{error}</p><button onclick={load} disabled={loading}>Try again</button>{/if}{#if loading}<p role="status">Opening your balances…</p>{:else}
