<script lang="ts">
  ...
  async function claimPayment() {
    const amountKobo = Math.round(Number(claimAmount.replace(/,/g,''))*100);
  ...
  async function openDispute(event:SubmitEvent){event.preventDefault();const disputed_amount_kobo=Math.round(Number(disputeAmount.replace(/,/g,''))*100);if(disputed_amount_kobo<=0||!disputeReason.trim()){message='Enter the money affected and what went wrong.';return}const response=await fetch(`/api/v1/buyer/credit-requests/${requestID}/disputes`,{method:'POST',credentials:'include',headers:{'Content-Type':'application/json','Idempotency-Key':idempotencyKey(),...csrfHeaders()},body:JSON.stringify({disputed_amount_kobo,reason:disputeReason,explanation:disputeExplanation,collection_effect:disputeEffect})});const result=await response.json().catch(()=>({}));if(!response.ok){message=result.detail??'We could not report this problem.';return}message='Your problem was reported. Bank debit is paused for the money affected.';disputeAmount='';disputeReason='';disputeExplanation='';await load()}
  onMount(load);
</script>
...
<main>
  ...
  {#if view}
    <section class="terms"><h2>The sale</h2><dl><dt>Seller</dt><dd>{view.request.supplier_legal_name}</dd><dt>Customer</dt><dd>{view.request.buyer_legal_name}</dd><dt>Money to pay</dt><dd><Money amountKobo={view.request.principal_kobo} /></dd><dt>Goods</dt><dd>{view.request.goods_description}</dd><dt>First payment day</dt><dd>{view.request.due_date}</dd><dt>Bank debit after</dt><dd>{new Date(view.request.collection_at).toLocaleString('en-NG',{timeZone:'Africa/Lagos',timeZoneName:'short'})}</dd><dt>Extra time</dt><dd>{view.request.grace_hours} hours</dd><dt>How to pay</dt><dd>{view.request.schedule_type==='equal'?`${view.request.schedule_count} ${view.request.schedule_cadence} payments`:view.request.schedule_type==='custom'?'Different payment amounts':'One payment'}</dd><dt>Kredit fee</dt><dd>0.5% when you pay. 1% only when Kredit has to collect late money.</dd><dt>Sale number</dt><dd>{view.request.id}</dd></dl>{#if view.request.custom_schedule_items?.length}<h3>Payment days</h3><ol>{#each view.request.custom_schedule_items as item}<li><Money amountKobo={item.amount_kobo} /> by {item.due_date}</li>{/each}</ol>{/if}<details><summary>Technical record</summary><p>Sale record code<br/><code>{view.agreement.document_hash}</code></p></details></section>
    ...
    {#if view.request.state === 'RECEIPT_CONFIRMATION_PENDING'}<button onclick={async () => { if (!receiptKey) receiptKey = idempotencyKey(); const response = await fetch(`/api/v1/buyer/credit-requests/${requestID}/receipt`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': receiptKey, ...csrfHeaders() }, body: JSON.stringify({ state: 'confirmed' }) }); if (response.ok) receiptKey = ''; await load(); }}>Yes, I got the goods</button>{/if}
