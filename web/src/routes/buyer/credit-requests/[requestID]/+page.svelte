...
<main>
  ...
  {#if view}
    ...
    <h2>What you have paid so far</h2>
    {#if payments.length === 0}<p>Nothing paid on this sale yet.</p>{:else}<ul>{#each payments as payment}<li><Money amountKobo={payment.amount_kobo} /> · {productLabel(payment.source_type)} · {productLabel(payment.state)}</li>{/each}</ul>{/if}
