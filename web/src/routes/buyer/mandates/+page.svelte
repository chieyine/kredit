...
<main class="shell workspace"><p class="eyebrow">Bank debit</p><h1>You decide what leaves your bank.</h1><p class="lede">You can stop this permission at any time. After that, Kredit cannot debit you, and any new sale will have to ask you again.</p>
{#if error}<p class="error" role="alert">{error}</p>{/if}
{#if mandates.length}<section class="cards">{#each mandates as mandate}<article><div><strong>{mandate.provider}</strong><span>{productLabel(mandate.status)}</span></div><p>The most Kredit may ever take: <Money amountKobo={mandate.amount_ceiling_kobo} /></p><details class="reference"><summary>Permission number</summary>{mandate.id}</details>{#if mandate.status==='ACTIVE'}<button class="danger" disabled={busy===mandate.id} onclick={()=>command(mandate,'cancel')}>Stop bank debit</button>{:else if ['CANCELLED','EXPIRED','FAILED'].includes(mandate.status)}<button disabled={busy===mandate.id} onclick={()=>command(mandate,'restore')}>Allow bank debit again</button>{/if}</article>{/each}</section>{:else}<section class="empty"><h2>You have not given bank debit permission yet</h2><p>It will show here after you accept your first sale.</p></section>{/if}</main>
