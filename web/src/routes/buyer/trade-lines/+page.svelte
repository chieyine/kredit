<script lang="ts">
 import {feeDisclosure} from "$lib/fee-terms";
	...
	async function load() {
		...
		if (!response.ok) { error = 'We could not open your limits. Please try again.'; return; }
	...
	async function command(path: string, body: unknown, drawdownID: string) {
		...
		try {
			...
			notice = 'Saved. Thank you.'; await load();
...
<main class="shell workspace">
	...
	{#if statements.length}{#each statements as statement}<section class="line">
		...
		{#if statement.drawdowns.length}<div class="drawdowns">{#each statement.drawdowns as drawdown}<article>
			...
			<dl><dt>Pay before</dt><dd>{drawdown.due_date}</dd><dt>Kredit may debit your bank after</dt><dd>{new Date(drawdown.collection_at).toLocaleString('en-NG')}</dd><dt>Extra time before that</dt><dd>{drawdown.grace_hours} hours</dd><dt>Invoice</dt><dd>{drawdown.invoice_reference || 'No number'}</dd><dt>Kredit fee (paid by the seller)</dt><dd>{feeDisclosure(drawdown.fee_terms)}</dd></dl>
			<details class="hash"><summary>Technical record (for reference)</summary><code>{drawdown.agreement_hash}</code></details><a href={`/api/v1/buyer/trade-lines/${statement.line.id}/drawdowns/${drawdown.id}/agreement-document`} target="_blank" rel="noreferrer">Print or save a copy of this sale →</a>
			{#if drawdown.state === 'PENDING_BUYER_CONFIRMATION'}<p>Read the goods, the money and the dates. Nothing is owed yet.</p><button class="primary" disabled={busy === drawdown.id} onclick={() => command(`/api/v1/buyer/trade-lines/${statement.line.id}/drawdowns/${drawdown.id}/confirm`, { agreement_hash: drawdown.agreement_hash }, drawdown.id)}>Yes to this {money(drawdown.principal_kobo)} sale</button>{/if}
