...
<svelte:head><title>Pay safely — Kredit</title></svelte:head>
<main class="shell prose-page"><p class="eyebrow">Safe payment</p>{#if intent}<h1>{exactKobo(intent.amount_kobo) === 0n ? 'There is nothing left to pay.' : 'Check the amount before you pay.'}</h1><dl><div><dt>You are paying</dt><dd>{intent.supplier_name||'Seller'}</dd></div><div><dt>For</dt><dd>{intent.description}</dd></div><div><dt>Money left to pay</dt><dd><Money amountKobo={intent.amount_kobo} /></dd></div></dl><p>{intent.provider_action}</p>{#if exactKobo(intent.amount_kobo)! > 0n}<a class="primary" href={`/buyer/credit-requests/${encodeURIComponent(intent.reference)}`}>Continue to pay</a>{/if}<div class="support-card"><p><strong>Not sure about this payment?</strong> Talk to a real person before you pay.</p><a class="whatsapp-support" href="mailto:hello@kredit.com.ng?subject=Payment%20question">Email Kredit support</a></div><p class="privacy">This page never shows your bank details or your private account information.</p>{:else if error}<h1>This payment link is not working.</h1><p role="alert">{error}</p><button type="button" onclick={() => load(page.params.token!)}>Try again</button>{:else}<p>Opening your payment…</p>{/if}</main>
