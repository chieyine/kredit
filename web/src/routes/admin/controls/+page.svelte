<script lang="ts">
	...
	let targetType = $state('user'),
		targetId = $state(''),
		organizationId = $state('');
	let scope = $state('all_sensitive'),
		expiresAt = $state(''),
		version = $state(1),
		reason = $state('');
	...
	function payload() {
		...
		return {
			...
			scope,
			expires_at: expiresAt ? lagosISO(expiresAt) : undefined,
	...
	async function apply() {
		...
		message = '';
		try {
			const data = await intent.run(candidate, (value) => {
				const response = record(value),
					command = record(response.command);
				text(command.id);
				if (
					command.state !== 'APPLIED' ||
					command.command_type !== candidate.command_type ||
					command.target_id !== candidate.target_id
				)
					throw new Error('Result unavailable');
				return { id: text(command.id) };
			});
			...
			message =
				cause instanceof Error
					? cause.message
					: 'We have not confirmed the result. Check the record before applying this again.';
		} finally {
			busy = false;
		}
	}
...
<main class="shell workspace">
	...
	<p>
		...
		impact preview explains whether a user notification is expected. Every action is recorded permanently.
	</p>
	<VerifyIdentity />
	<form
		oninput={() => {
		...
		<fieldset disabled={busy}>
			<label
				>Action<select
					...
					onchange={() => {
						preview = null;
						if (command === 'retry_document_scan') targetType = 'document';
					}}
					><option value="retry_document_scan">Retry a quarantined document scan</option><option value="suspend_user"
						>Suspend a user</option
					><option value="restore_user">Restore a user</option><option value="suspend_organization"
						>Suspend a business</option
					><option value="restore_organization">Restore a business</option><option value="place_risk_hold"
						>Place a temporary hold</option
					><option value="lift_risk_hold">Remove a temporary hold</option><option value="request_reconciliation"
						>Ask for a money check</option
					><option value="resolve_unknown_submission">Resolve an unknown bank result</option><option
						value="retry_collection">Retry a bank debit</option
					><option value="cancel_collection">Cancel a bank debit</option></select
				></label
			>
			<label>Target type<input bind:value={targetType} required /></label><label
				>Target ID<input bind:value={targetId} required /></label
			>
			<label>Organization ID (when known)<input bind:value={organizationId} /></label><label
				>Current version<input type="number" min="1" bind:value={version} /></label
			>
			<label
				>Scope (risk holds only)<select bind:value={scope}
					><option>all_sensitive</option><option>credit</option><option>release</option><option>collection</option
					><option>settlement</option></select
				></label
			>
			<label>Expires (Lagos time; risk holds only)<input type="datetime-local" bind:value={expiresAt} /></label><label
				>Structured reason<textarea bind:value={reason} minlength="8" required></textarea></label
			>
	...
	{#if preview}<section>
			...
			<p>{preview.impact_preview.effect}</p>
			<dl>
				<dt>Current version</dt>
				<dd>{preview.current_version}</dd>
				<dt>User notification</dt>
				<dd>{preview.impact_preview.will_notify ? 'Required' : 'No'}</dd>
				<dt>Audit</dt>
				<dd>{preview.impact_preview.audit}</dd>
			</dl>
			<button type="button" class="primary" disabled={busy} onclick={apply}
				>{busy ? 'Applying…' : 'Apply this change'}</button
			>
		</section>{/if}
</main>
