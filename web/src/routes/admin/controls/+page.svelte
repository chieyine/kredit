<script lang="ts">
 ...
 let preview: any = $state(null), message = $state(''), ready = $state(false);
	...
	async function inspect() {
		...
		const data = await response.json(); preview = response.ok ? data.command : null; message = response.ok ? 'Read the impact below before you apply this.' : data.detail ?? 'The preview failed. Try again.';
	}
	async function apply() {
		...
		const data = await response.json(); message = response.ok ? `Applied. Command ${data.command.id}` : data.detail ?? 'The command failed. Nothing was applied.'; if (response.ok) preview = null;
	}
</script>
...
<main class="shell workspace">
	...
	<p>Before any action here goes through, all of this must be true: you signed in with MFA recently, you hold the exact permission, the record is on its current version, you gave a clear reason, you looked at the impact preview and the affected user gets told. Every action is recorded permanently.</p>
