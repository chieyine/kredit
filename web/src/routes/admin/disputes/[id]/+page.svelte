<script lang="ts">
 ...
 async function openDocument(id:string){try{const url=await checkedJSON(`/api/v1/ops/disputes/${encodeURIComponent(page.params.id!)}/documents/${encodeURIComponent(id)}/download`,value=>{const url=new URL(text(record(value).url),location.origin);if(!['https:','http:'].includes(url.protocol)||url.username||url.password||(location.protocol==='https:'&&url.protocol!=='https:'))throw new Error('Invalid document link');return url.href});location.assign(url)}catch(cause){actionError=publicError(cause,'this document')}}
