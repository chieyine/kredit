...
<main class="shell workspace">
	...
	{#if message}<p class="notice" role="status">{message}</p>{/if}{#if error}<section role="alert">
			...
			<button type="button" onclick={load}>Try again</button>
				...
				<div class="actions">
					<button onclick={() => begin(item, 'approve')}>Approve with cooling-off</button><button
						class="danger"
						onclick={() => begin(item, 'reject')}>Reject</button
					>
				</div>
			</article>{:else}<section class="empty-state">
				<h2>Nothing waiting for review</h2>
				<p>A request shows up here once independent proof has been collected.</p>
			</section>{/each}{/if}
</main>
{#if selected}<ProtectedActionDialog
		bind:open={dialogOpen}
		title={`${decision === 'approve' ? 'Approve' : 'Reject'} account recovery`}
		description={decision === 'approve'
			? 'A 24-hour cooling-off period will begin and sensitive financial changes remain blocked.'
			: 'Access will not be restored. Record the evidence-based reason for rejection.'}
		confirmLabel={decision === 'approve' ? 'Approve with cooling-off' : 'Reject recovery'}
		onconfirm={confirm}
	/>{/if}
