<script lang="ts">
 ...
 type Review = {id:string;kind:string;target_id:string;expected:string;actual:string;owner_id:string|null;first_seen_at:string;last_seen_at:string;history:{id:string;action:string;reason:string;occurred_at:string}[]};
 let actorID=$state(''),canTakeover=$state(false);
 let filter=$state('all');
 let cases:Review[]=$state([]), loading=$state(true),busy=$state(''),error=$state('');
 let reasons:Record<string,string>=$state({});
 const visible=$derived(cases.filter(item=>filter==='all'||(filter==='mine'?item.owner_id===actorID:item.owner_id===null)));
 ...
 function review(value:unknown):Review {
  ...
  for(const key of ['id','kind','target_id','expected','actual','first_seen_at','last_seen_at']) text(item[key]);
  if(!/^-?\d+$/.test(item.expected as string)||!(/^-?\d+$/.test(item.actual as string))) throw new Error('Financial amounts were incomplete');
  if(!Number.isFinite(Date.parse(text(item.first_seen_at)))||!Number.isFinite(Date.parse(text(item.last_seen_at))))throw new Error('Review dates were incomplete');
 ...
 async function load(){
  ...
  try{const result=await checkedJSON('/api/v1/ops/financial-reconciliation',value=>{const body=record(value);if(typeof body.can_takeover!=='boolean')throw new Error('Review permissions unavailable');return{cases:rows('cases',review)(body),actor:text(body.actor_id),canTakeover:body.can_takeover};},{signal:request.signal});if(request.current()){cases=result.cases;actorID=result.actor;canTakeover=result.canTakeover;}}
...
<main class="shell workspace">
 ...
 <VerifyIdentity />
 <label>Show reviews<select bind:value={filter}><option value="all">All open reviews</option><option value="mine">Assigned to me</option><option value="unassigned">Awaiting an owner</option></select></label>
 <button onclick={load} disabled={loading||!!busy}>Refresh reviews</button>
 {#if error}<p role="alert" class="error">{error}</p>{/if}
 {#if loading}<p>Loading financial reviews…</p>{:else if !error && cases.length===0}<p>No open financial reviews.</p>{:else if !error && !loading && visible.length===0}<p>No open reviews match this filter.</p>{/if}
 ...
 <p>{item.owner_id===actorID?'Assigned to you':item.owner_id?'Assigned to another reviewer':'Awaiting a reviewer'}</p>
 ...
 <div>{#if !item.owner_id}<button disabled={!!busy||(reasons[item.id]||'').trim().length<8} onclick={()=>decide(item,'claim')}>Take this review</button>{/if}
 {#if item.owner_id===actorID}<button disabled={!!busy||(reasons[item.id]||'').trim().length<8} onclick={()=>decide(item,'release')}>Release for handover</button><button disabled={!!busy||(reasons[item.id]||'').trim().length<8} onclick={()=>decide(item,'resolve')}>Close after records agree</button>
 {:else if item.owner_id&&canTakeover}<button disabled={!!busy||(reasons[item.id]||'').trim().length<8} onclick={()=>decide(item,'takeover')}>Take over as platform owner</button>{/if}</div>
