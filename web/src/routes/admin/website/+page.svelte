<script lang="ts">
	...
	function cloneCopy(value: WebsiteCopy): WebsiteCopy {
	...
	let loading = $state(true),
		...
		reason = $state(''),
		preview = $state(false);
	let history = $state<{ version: number; recorded_at: string; reason: string; copy: WebsiteCopy }[]>([]),
		historyError = $state('');
	...
	async function load() {
		...
		try {
			...
			try {
				...
				if (!signal.aborted) historyError = 'Saved history could not be opened. Reload to try again.';
			}
		} catch (e) {
			if (!signal.aborted) error = e instanceof Error ? e.message : 'The editor could not be opened.';
		} finally {
	...
	async function save(action: 'save' | 'publish') {
		...
		try {
			...
			message =
				action === 'publish'
					? 'Published. The public page now uses this copy.'
					: 'Draft saved. Preview it before publishing.';
		} catch (e) {
			error = e instanceof Error ? e.message : 'The result could not be confirmed. Reload to check the saved version.';
		} finally {
			busy = false;
		}
	}
	function restore(item: { version: number; copy: WebsiteCopy }) {
		copy = cloneCopy(item.copy);
		reason = `Restore content from saved version ${item.version}`;
		preview = false;
...
<main class="editor">
	...
	{#if current && !loading}
		{#if isLegalPage(selected) && current.content?.published?.document_version}<p>
				<a
					href={`/legal/${selected}?version=${encodeURIComponent(current.content.published.document_version)}`}
					target="_blank"
					rel="noreferrer">Read published document · {current.content.published.document_version}</a
				>
			</p>{/if}
		<p class="version">
			Draft version {current.version}{#if current.content?.published}
				· Published {localTime(current.content.published.published_at)} · Publication {current.content.published
					.version}{:else}
				· Public page uses the initial release copy{/if}
		</p>
		{#if isLegalPage(selected)}<p class="notice">
				Publishing takes effect today and creates a permanent document version. Earlier accepted records keep the
				document version they referenced.
			</p>{/if}
		<form
			onsubmit={(event) => {
		...
		>
			<fieldset disabled={busy}>
					...
					{#each copy.sections as section, i (i)}
						<section class="question">
							<label
								...
								{i + 1}<input bind:value={section.heading} required maxlength="200" /></label
							><label
								>{selected === 'faq' ? 'Answer' : 'Section text'}<textarea
									bind:value={section.body}
									required
									maxlength="6000"
									rows="5"
								></textarea></label
							>{#if isGuidePage(selected)}<label
									>Bullet points, one per line<textarea
										rows="4"
										value={section.points?.join('\n') ?? ''}
										oninput={(event) => {
											section.points = event.currentTarget.value.split('\n').filter((s) => s.trim());
										}}
									></textarea></label
								>{/if}<button
								type="button"
								onclick={() => {
									copy.sections = copy.sections.filter((_, index) => index !== i);
									preview = false;
								}}>Remove section {i + 1}</button
							>
						</section>
					{/each}
					<button
						type="button"
						disabled={copy.sections.length >= 40}
						onclick={() => {
							copy.sections = [...copy.sections, { heading: '', body: '' }];
							preview = false;
						}}>{selected === 'faq' ? 'Add a question' : 'Add a section'}</button
					>
				{/if}
				{#if copy.guide}
					<label
						>Search description<textarea bind:value={copy.guide.description} required maxlength="2000"
						></textarea></label
					>
					<label
					...
					{#each copy.guide.faq as q, i (i)}<section class="question">
							...
							><button
								type="button"
								onclick={() => {
									copy.guide!.faq = copy.guide!.faq.filter((_, j) => j !== i);
								}}>Remove question {i + 1}</button
							>
						</section>{/each}
					<button
						type="button"
						disabled={copy.guide.faq.length >= 40}
						onclick={() => {
							copy.guide!.faq = [...copy.guide!.faq, { heading: '', body: '' }];
					...
					{#each copy.guide.sources as source, i (i)}<section class="question">
							...
							><label>Source note<textarea bind:value={source.note} maxlength="2000"></textarea></label><button
								type="button"
								onclick={() => {
									copy.guide!.sources = copy.guide!.sources.filter((_, j) => j !== i);
								}}>Remove source {i + 1}</button
							>
						</section>{/each}
					<button
						type="button"
						disabled={copy.guide.sources.length >= 40}
						onclick={() => {
							copy.guide!.sources = [...copy.guide!.sources, { name: '', url: '', note: '' }];
						}}>Add source</button
					>
					<label
				...
				{#if copy.contact}
					<label
						>Help email<input type="email" bind:value={copy.contact.support_email} required maxlength="254" /></label
					>
					<label
						>Privacy email<input type="email" bind:value={copy.contact.privacy_email} required maxlength="254" /></label
					>
				...
				<label
					>Reason for this change<input
						...
						minlength="4"
						maxlength="500"
						placeholder="For example, clarify how delivery proof works"
					/></label
				>
				<div class="actions">
					<button type="submit" disabled={reason.trim().length < 4}>Save draft</button><button
						type="button"
						disabled={dirty || current.version === 0}
						onclick={() => (preview = !preview)}>Preview saved copy</button
					>
				</div>
			</fieldset>
		</form>
		...
		{#if preview && !dirty}
			<section class="preview" aria-label="Saved copy preview">
			...
			<p>
				Publishing replaces this page’s supported text. Its links, layout and fee calculations keep their existing
				behaviour.
			</p>
			<button class="publish" disabled={busy || reason.trim().length < 4} onclick={() => save('publish')}
				>Publish reviewed copy</button
			>
			<p class="hint">Enter a publication reason above before publishing.</p>
		{/if}
		<section class="history">
			...
			{#if historyError}<p role="alert">{historyError}</p>{/if}{#each history as item, i (i)}<article>
					...
					<button disabled={busy || dirty} onclick={() => restore(item)}>Use this copy</button>
				</article>{/each}{#if !history.length && !historyError}<p>No earlier saved copy.</p>{/if}
		</section>
	{/if}
</main>
