<script lang="ts">
	...
	const gateLabels: Record<string, string> = {
		...
		dpia: 'Data protection review reference',
		legal_approval: 'Legal approval reference',
		penetration_test: 'Security test evidence',
		backup_restore: 'Backup recovery evidence',
		provider_certification: 'Provider certification reference',
		support_training: 'Support training evidence',
		launch_approval: 'Launch approval reference',
		pilot_provider_accounts: 'Allowed provider accounts',
		pilot_industries: 'Allowed industries',
		real_identity_provider: 'Identity provider enabled',
		real_collection_provider: 'Collection provider enabled',
		pilot_max_supplier_organizations: 'Maximum seller businesses',
		pilot_max_buyer_businesses: 'Maximum buyer businesses',
		pilot_max_principal_kobo: 'Maximum sale amount',
		pilot_max_active_exposure_kobo: 'Maximum outstanding credit',
		pilot_max_drawdowns_per_line_day: 'Daily purchases per credit limit',
		pilot_max_collection_retries: 'Collection retry limit',
		pilot_enhanced_review_kobo: 'Extra review threshold'
	};
	const labels: Record<string, string> = {
		needs_setup: 'Needs setup',
		configured_unverified: 'Configured · delivery or operation still needs confirmation',
	...
	function applied(item: Process) {
		return (
			...
			Object.keys(item.versions).every((key) => versions[key] === item.versions[key])
		);
	}
	async function load() {
		if (busy) return;
		...
		try {
			...
			if (!Array.isArray(data.tasks) || !Array.isArray(data.processes) || typeof data.auto_apply !== 'boolean')
				throw new Error('Setup information could not be verified.');
			tasks = data.tasks.map((value) => {
				const item = record(value);
				const url = text(item.url);
				if (!url.startsWith('/admin/')) throw new Error('Invalid setup link');
				return { id: text(item.id), label: text(item.label), state: text(item.state), detail: text(item.detail), url };
			});
			const saved = record(data.saved_versions);
			...
			processNote = typeof data.process_status_note === 'string'
			...
			gates = data.configuration_gates.map((value) => {
				const item = record(value);
				if (typeof item.passed !== 'boolean') throw new Error('Invalid launch requirement');
				return { name: text(item.name), passed: item.passed, detail: text(item.detail) };
			});
			autoApply = data.auto_apply;
			checkedAt = text(data.checked_at);
			failures = Number(data.provider_initialization_failures);
			error = '';
		} catch (cause) {
			error = publicError(cause, 'super-admin setup');
		} finally {
			busy = false;
			loading = false;
		}
...
<main class="shell workspace">
	<h1>Launch setup</h1>
	<p>
		Complete your provider connections, business details and launch evidence here. Credentials are stored securely and
	...
	{#if loading}<p role="status">Loading setup…</p>{:else if !error}
		<section class="service-status">
			...
			<p>{processNote}</p>
			{#if truncated}<p role="alert">Only the 500 most recent instance records are shown. This list is incomplete.</p>{/if}
			{#each ['api', 'worker'] as name}
				...
				{#each instances as item (item.instance_id)}
					<p>
						...
						<br />Version {item.version} · revision {item.revision.slice(0, 12)} · last reported {localTime(item.updated_at)}
					</p>
				{:else}
			...
			<p>Applied settings do not prove that a provider has approved your account or completed a live operation.</p>
		</section>
		<section class="service-status">
			<h2>Launch requirements</h2>
			<p>These show which configuration values are recorded. Evidence still needs to be genuine and reviewed.</p>
			<ul>
				{#each gates as gate (gate.name)}<li>
						{gateLabels[gate.name] ?? gate.detail}: <strong>{gate.passed ? 'Recorded' : 'Missing'}</strong>
					</li>{/each}
			</ul>
			<a href="/admin/platform-settings?connection=integrations.runtime.launch">Update launch evidence and limits</a>
		</section>
		<div class="tasks">
			{#each tasks as task (task.id)}<article>
					<h2>{task.label}</h2>
					<strong>{labels[task.state] ?? task.state}</strong>
					<p>{task.detail}</p>
					<a href={task.url}>Open {task.label.toLowerCase()}</a>
				</article>{/each}
		</div>
		<p>
			Checked {localTime(checkedAt)}. Some approvals, domain verification and account activation must be completed with
			the provider; record their results here.
		</p>
		<p>
			<a href="/admin/platform-settings?connection=integrations.runtime.retained_collections"
				>Manage saved collection accounts</a
			>
			· <a href="/admin/billing">Manage fee billing</a>
		</p>
