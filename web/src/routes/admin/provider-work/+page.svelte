<script lang="ts">
	...
	let items = $state<Item[]>([]),
		connections = $state<{ provider: string; state: string; action: string }[]>([]),
		counts = $state<Record<string, unknown>>({}),
		error = $state(''),
		loading = $state(true),
		asOf = $state('');
	...
	async function load() {
		...
		error = '';
		try {
			const result = await checkedJSON('/api/v1/ops/provider-work', (value) => {
				const data = record(value),
					work = record(data.work);
				return {
					connections: rows('connections', (value) => {
						const r = record(value);
						return { provider: text(r.provider), state: text(r.state), action: text(r.action) };
					})(data),
					items: rows('items', (value) => {
						const r = record(value);
						for (const key of ['kind', 'id', 'provider', 'state', 'target_id', 'started_at', 'next_action'])
							text(r[key]);
						return r as Item;
					})(work),
					counts: record(work.counts),
					asOf: text(work.as_of)
				};
			});
			items = result.items;
			...
			error = publicError(cause, 'provider work');
		} finally {
			loading = false;
		}
	}
...
<main class="shell workspace">
	<h1>Provider work needing attention</h1>
	<p>
		Configuration, waiting requests and failed outcomes are shown separately. No recent activity does not prove that a
		provider is healthy.
	</p>
	<button disabled={loading} onclick={load}>Refresh records</button><a href="/admin/diagnostics">System diagnostics</a>
	{#if error}<p role="alert">{error}</p>{/if}{#if loading}<p role="status">Loading current records…</p>{:else if !error}
		...
		<section class="connections">
			{#each connections as item, i (i)}<article>
					...
					<p>{item.action}</p>
					<a href={item.provider === 'Mono' ? '/admin/mono' : '/admin/platform-settings'}>Open settings</a>
				</article>{/each}
		</section>
		<label
		...
		>
		<ul>
			{#each visible as item (item.kind + item.id)}<li>
					<h2>{item.provider} · {item.kind}</h2>
					<p>{item.state} · {timeLabel(item.started_at)}</p>
					<p>{item.next_action}</p>
					<p>Record <code>{item.id}</code></p>
					<p>Affected record <code>{item.target_id}</code></p>
					<a href={action(item)}>Review</a>
				</li>{:else}<li>No matching work in these records.</li>{/each}
		</ul>{/if}
</main>
