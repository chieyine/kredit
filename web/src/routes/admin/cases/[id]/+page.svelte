<script lang="ts">
	...
	async function update(e: SubmitEvent) {
		...
		try {
			await intent.run(
				{ state: nextState, note },
				(value) => {
					const body = record(value);
					if (text(record(body.case).id) !== id) throw new Error('Unconfirmed case update');
					return body;
				},
				'PATCH'
			);
			if (page.params.id !== id) return;
			notice = 'Case updated. The note was added to its permanent history.';
			note = '';
			await load(id);
		} catch (cause) {
			if (page.params.id !== id) return;
			notice = cause instanceof Error ? cause.message : 'The case update could not be confirmed.';
		} finally {
			busy = false;
		}
	}
	$effect(() => {
		...
		notice = '';
		note = '';
		intent = new MutationIntent('admin-case', `/api/v1/ops/cases/${encodeURIComponent(id)}`);
		void load(id);
		return () => reads.cancel();
	});
</script>
...
<main class="shell workspace">
	<a href="/admin/cases">← All support cases</a>
	<p class="eyebrow">Admin / Support case</p>
	<h1>Support case</h1>
	{#if notice}<p class="notice" role="status">{notice}</p>{/if}{#if loading}<p>Loading case…</p>{:else if error}<p
			class="error"
			role="alert"
		>
			{error}
		...
		<div class="columns">
			...
			{#if item.state !== 'CLOSED'}<section class="action">
					<h2>Move this case forward</h2>
					<form onsubmit={update}>
						<fieldset disabled={busy}>
							<label
								>New status<select bind:value={nextState}
									><option value="OPEN">Open</option><option value="IN_PROGRESS">Being handled</option><option
										value="RESOLVED">Resolved</option
									><option value="CLOSED">Closed</option></select
								></label
							><label>What happened?<textarea bind:value={note} minlength="4" rows="5" required></textarea></label
							><button class="primary" disabled={busy}>{busy ? 'Saving…' : 'Save case update'}</button>
						</fieldset>
					</form>
				</section>{/if}
		</div>{/if}
