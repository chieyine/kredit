...
<style>main{max-width:850px;padding-block:2rem}ul{list-style:none;padding:0}li,form{padding:1.25rem;border:1px solid var(--border,var(--color-border));border-radius:1rem;margin-top:1rem}fieldset{border:0;padding:0;display:grid;gap:1rem;min-width:0}label{display:grid;gap:.5rem}input,select,textarea{width:100%;box-sizing:border-box;padding:.75rem;font:inherit}li p{overflow-wrap:anywhere}button{padding:.75rem 1rem;cursor:pointer}textarea{min-height:7rem}</style>
