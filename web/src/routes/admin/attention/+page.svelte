<script lang="ts">
	...
	async function load() {
		...
		try {
			...
			if (!['mandates', 'debits', 'notices'].every((key) => Array.isArray(body[key])))
				throw new Error('Work needing attention could not be verified.');
			data = body;
		} catch (e) {
...
<main class="shell workspace">
	...
	<AdminAttention />{#if error}<p role="alert">{error} <button onclick={load}>Try again</button></p>{/if}{#if data}<p>
		...
		{#each data.mandates as item}<article>
				<h3>{item.buyer}</h3>
