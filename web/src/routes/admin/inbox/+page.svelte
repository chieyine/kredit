<script lang="ts">
	...
	async function assign(clear = false) {
		if (busy || !selected) return;
		busy = true;
		error = '';
		try {
			await adminPost('/api/v1/ops/review-assignments', {
				kind: selected.kind,
				id: selected.id,
				owner_id: clear ? '' : actor,
				due_at: lagosISO(due),
				reason
			});
			selected = null;
			reason = '';
			message = 'Ownership and deadline recorded.';
			await load();
		} catch (e) {
			error = e instanceof Error ? e.message : 'We could not open this. Try again.';
		} finally {
...
<main class="shell workspace">
	...
	<VerifyIdentity />
	<p>
		Policy changes, money corrections, disputes, account recovery and privacy reviews, with owners and deadlines in one
		place. You only see the work your role allows.
	</p>
	<form
		onsubmit={(e) => {
			e.preventDefault();
	...
	{#each items as item (`${item.kind}:${item.id}`)}<article>
			...
			<button
				...
				onclick={() => {
					selected = item;
					due = localInput(item.due_at);
					reason = '';
				}}>Manage ownership and deadline</button
			>
		</article>{:else}{#if !busy && !error}<p>No review matches that.</p>{/if}{/each}
	<button
	...
	{#if selected}<section>
			...
			>
				<label>Review deadline (Lagos)<input type="datetime-local" bind:value={due} required /></label><label
					>Reason<textarea bind:value={reason} required minlength="8" maxlength="2000"></textarea></label
				><button disabled={busy}>Assign to me and save the deadline</button><button
					type="button"
					disabled={busy || reason.trim().length < 8}
					onclick={() => assign(true)}>Put back in the queue</button
				><button type="button" disabled={busy} onclick={() => (selected = null)}>Close</button>
			</form>
		</section>{/if}
</main>
