<script lang="ts">
  ...
  async function checkSession(signal: AbortSignal | undefined = undefined) {
    ...
    catch { /* A background lookup never determines access or blocks sign-in. */ }
  }
...
<main class="shell auth-page">
  <section class="auth-copy"><p class="eyebrow">Your Kredit account</p><h1>Start or sign in.</h1><p class="lede">One account for business trade and personal purchases. Sign in with a code sent to your email or WhatsApp.</p><p class="privacy-note">New here? The same step creates your account. Choose your business workspace or personal purchases after signing in.</p></section>
  <form class="card auth-card" onsubmit={(event) => { event.preventDefault(); void (challengeID ? verifyCode() : requestCode()); }}>
    ...
    {#if !challengeID}
      <fieldset class="channel"><legend>Where should we send your code?</legend><label><input type="radio" value="email" bind:group={channel} disabled={!interactive || busy} />Email</label><label><input type="radio" value="whatsapp" bind:group={channel} disabled={!interactive || busy} />WhatsApp</label></fieldset>
      <label>{channel === 'whatsapp' ? 'Phone number' : 'Email address'}<input bind:value={identifier} type={channel === 'whatsapp' ? 'tel' : 'email'} autocomplete={channel === 'whatsapp' ? 'tel' : 'email'} placeholder={channel === 'whatsapp' ? '0801 234 5678' : 'you@example.com'} disabled={!interactive || busy} required /></label>
    ...
    {:else}
      ...
      <label>Six-digit code<input bind:value={code} inputmode="numeric" autocomplete="one-time-code" pattern={'[0-9]{6}'} minlength="6" maxlength="6" disabled={!interactive || busy || expired} required /></label>
