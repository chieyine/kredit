<script lang="ts">
 ...
 let defaultsScope='', defaultPaymentDays=30, defaultGraceHours=24;
 let scheduleType=$state('one_time'), scheduleCount=$state(2), scheduleCadence=$state('monthly'), monthEndPolicy=$state('cap'), customSchedule=$state('');
 ...
 function clearDraft(){if(busy)return;try{deleteDraft(account.userID,organizationID,sessionStorage);}catch{/* Optional storage. */}principal='';goods='';dueDate='';collectionAt='';invoiceReference='';customSchedule='';invoiceFile=null;reviewed='';draftMessage='Draft cleared.';}
 function useSaved(item:{name:string;amount:string}){goods=item.name;principal=item.amount;reviewed='';saveDraft();}
 function practise(){if(busy)return;
  goods='10 cartons of cooking oil';principal='100,000';
  dueDate=paymentDateAfter(defaultPaymentDays);
  collectionAt='';graceHours=defaultGraceHours;reviewed='';draftMessage='Sample goods and amount filled in. Check all details before saving a real draft.';saveDraft();
 }
 async function loadCustomers(){
  ...
  if(defaultsScope!==scope){defaultPaymentDays=defaults.paymentDays;defaultGraceHours=defaults.graceHours;graceHours=defaults.graceHours;defaultsScope=scope;if(!dueDate)dueDate=defaults.dueDate;}
  ...
  }catch(cause){if(request.current()&&scope===organizationID){customerLoading=false;customerError=cause instanceof Error?cause.message:'Payment settings could not be loaded. Try again.';}}
 }
 async function changeBusiness(){
  defaultsScope='';draftReady=false;keepDraft=false;principal='';goods='';dueDate='';collectionAt='';invoiceReference='';customSchedule='';invoiceFile=null;reviewed='';draftMessage='';
 ...
 async function load(){
  ...
  try{organizationID=requestedWorkspace(organizations);}catch{showError('This business is not available in your account.');return;}
  ...
  const matches=customers.filter(item=>item.buyer_user_id===params.get('customer')&&(!params.get('customer_business')||item.buyer_business_id===params.get('customer_business')));
 ...
 async function uploadInvoice(){
  ...
  const prior=uploadedInvoices.get(uploadScope);if(prior)return prior;
  ...
  try{
   ...
   if(signature!==reviewed){
    const timing=await checkedJSON(`/api/v1/organizations/${encodeURIComponent(organizationID)}/credit-terms/preview`,value=>{
     ...
     return instant;
    },{method:'POST',headers:{'Content-Type':'application/json',...csrfHeader()},body:JSON.stringify({due_date:input.due_date,grace_hours:input.grace_hours,collection_local:input.collection_local,principal_kobo:input.principal_kobo,schedule_type:input.schedule_type,schedule_count:input.schedule_count,schedule_cadence:input.schedule_cadence,month_end_policy:input.month_end_policy,custom_schedule_items:input.custom_schedule_items})});
...
{#if draftMessage}<p class="draft-message" role="status">{draftMessage} <button type="button" disabled={busy} onclick={clearDraft}>Clear</button></p>{/if}{#if error}<p bind:this={errorSummary} class="error" role="alert" tabindex="-1"><strong>Please check:</strong> {error}</p>{/if}{#if success}<p class="success" role="status">{success}</p>{/if}
{#if loading}<p role="status">Checking your businesses…</p>{:else if !organizations.length}<p>Add or reopen your business from <a href="/workspace/today">Home</a> before creating a sale.</p>{/if}<div class="practice"><div><strong>First time here?</strong><p>Let us fill it with sample details so you can see how it works. Nothing reaches anybody until you save and send it yourself.</p></div><button type="button" disabled={busy||customerLoading||!!customerError} onclick={practise}>Show me with samples</button></div>
<form onsubmit={submit} oninput={() => { reviewed=""; saveDraft(); }} onchange={saveDraft}><fieldset disabled={busy||customerLoading} style="border:0;padding:0;margin:0;min-width:0"><section class="form-card"><b>01</b><div><h2>Who is taking the goods?</h2><div class="form-grid"><label>Your business<select bind:value={organizationID} onchange={changeBusiness} required>{#each organizations as org}<option value={org.id}>{org.trading_name||org.legal_name}</option>{/each}</select></label>{#if customerLoading}<p role="status">Checking your customers…</p>{:else if customerError}<div role="alert"><strong>Customer list unavailable</strong><p>{customerError}</p><button type="button" onclick={loadCustomers}>Try again</button></div>{:else if customers.length}<label>Search your customers<input bind:value={customerSearch} type="search" placeholder="Type their name" autocomplete="off" /></label><label>Customer<select bind:value={selectedBuyer} onchange={chooseBuyer} required><option value="">Choose a customer</option>{#each visibleCustomers as customer}<option value={customerKey(customer)}>{customer.trading_name||customer.legal_name}</option>{/each}</select></label><label>Customer name<input value={buyerLegalName} readonly /></label><label>Customer verification<input value={productLabel(selectedCustomer?.state, 'Choose a customer')} readonly /></label>{#if customerWarning}<div class="network-overdue-warning" role="alert"><strong>Overdue record:</strong> {customerWarning}</div>{/if}{:else}<div class="no-customers"><strong>You have not added a customer yet.</strong><p>Add them first. They get a private link and confirm their own details.</p><a class="primary" href="/workspace/partners/customers/new">Add a customer</a></div>{/if}</div></div></section><section class="form-card"><b>02</b><div><h2>What are they taking, and for how much?</h2>{#if savedItems.length}<div class="saved-items"><strong>Goods you sold before</strong>{#each savedItems.slice(0,4) as item}<button type="button" onclick={()=>useSaved(item)}>{item.name} · ₦{item.amount}</button>{/each}</div>{/if}<div class="form-grid"><label>Sale amount (₦)<input bind:value={principal} inputmode="decimal" placeholder="1,200,000" required />{#if verbalPrincipal}<small class="verbal-naira" aria-live="polite">{verbalPrincipal}</small>{/if}</label><label>What goods are they taking?<textarea maxlength="5000" bind:value={goods} rows="4" required></textarea></label><label>Invoice number <small>if you have one</small><input bind:value={invoiceReference} maxlength="120" /></label><div><DocumentUploader accept="application/pdf,image/jpeg,image/png" disabled={busy} onselect={(file)=>{invoiceFile=file;reviewed="";}} />{#if invoiceFile}<small>{invoiceFile.name}</small>{/if}</div><label>First payment date<input type="date" bind:value={dueDate} required /></label><label>Optional later collection time (Nigerian time)<input type="datetime-local" bind:value={collectionAt} /><small>Leave blank to use the end of the payment day plus the extra hours. This time is always interpreted in Africa/Lagos, wherever your device is.</small></label><label>Extra hours you are giving them<input type="number" min="0" max="720" bind:value={graceHours} /></label><label>How will they pay?<select bind:value={scheduleType}><option value="one_time">All at once</option><option value="equal">The same amount, many times</option><option value="custom">Different amounts</option></select></label>{#if scheduleType==='equal'}<label>How many payments?<input type="number" min="2" max="60" bind:value={scheduleCount} required /></label><label>How often?<select bind:value={scheduleCadence}><option value="weekly">Every week</option><option value="fortnightly">Every two weeks</option><option value="monthly">Every month</option></select></label>{#if scheduleCadence==='monthly'}<label>Monthly payment dates<select bind:value={monthEndPolicy}><option value="last_day">Always use the last day (first date must be month end)</option><option value="cap">Keep the agreed day; use month end in shorter months</option></select></label>{/if}{:else if scheduleType==='custom'}<label class="wide">Write out each payment<textarea bind:value={customSchedule} rows="5" placeholder={'600000, 2026-09-30\n600000, 2026-10-30'} required></textarea><small>One payment on each line: the amount, then the date. They must add up to the total money owed.</small></label>{/if}</div></div></section><section class="form-card review"><b>03</b><div><h2 bind:this={reviewHeading} tabindex="-1">Review this sale</h2><p><strong>{buyerLegalName||'Choose a customer'}</strong> will see that they took <strong>{goods||'the goods'}</strong> and must pay <strong>{parseNaira(principal)>0?formatKobo(parseNaira(principal)):'an amount to be entered'}</strong>{dueDate?` by ${dateLabel(dueDate)}`:''}.</p>{#if reviewed}<p><strong>Bank collection may be considered from:</strong> {timeLabel(canonicalAt)}</p>{:else}<p>Check the terms to see the server-verified collection time.</p>{/if}<p>{collectionBoundary}</p><p>Saving creates a draft, not a payment or bank permission.</p></div></section><div class="form-actions"><a href="/workspace/today">Go back</a><button class="primary" disabled={busy||!buyerUserID}>{busy?'Checking…':reviewed?'Save draft sale':'Check terms'}</button></div></fieldset></form><label class="draft-choice"><input type="checkbox" bind:checked={keepDraft} onchange={(event) => { keepDraft = event.currentTarget.checked; saveDraft(); }} /> Keep goods, amount and date on this device for up to 12 hours</label><p>Leave this off on a shared device. Customer, invoice and instalment details are not stored in a browser draft.</p></main>
<style>.draft-choice{display:flex;align-items:center;gap:.7rem;margin-top:1.5rem;font-size:.9rem}.draft-choice input{min-height:1.2rem}.form-grid>label{min-width:0}.form-card{min-width:0}.form-card>div{min-width:0}.verbal-naira{color:var(--color-primary);font-weight:600;font-size:.85rem;margin-top:.25rem}.network-overdue-warning{grid-column:span 2;padding:.85rem 1rem;background:var(--color-surface);border-left:4px solid var(--color-warning);color:var(--color-warning);font-size:.9rem;font-weight:600;margin-top:.75rem}.form-page{max-width:68rem}.page-heading{display:flex;justify-content:space-between;gap:2rem;padding-bottom:2rem;border-bottom:3px solid var(--color-primary)}.page-heading h1{font-family:var(--font-serif);font-size:clamp(3rem,6vw,5.2rem);font-weight:500;line-height:.92;letter-spacing:-.055em;margin:.5rem 0}.draft-badge{height:max-content;padding:.55rem .8rem;color:var(--color-on-primary);background:var(--color-accent);font-weight:750}.draft-message,.practice{display:flex;align-items:center;justify-content:space-between;gap:1rem;padding:1rem;background:var(--color-background);border-left:4px solid var(--color-primary)}.draft-message button,.practice button{padding:.6rem .8rem;border:1px solid var(--color-primary);background:var(--color-surface);font-weight:750}.practice{margin-top:1rem;background:var(--color-surface);border:1px solid var(--color-border)}.practice p{margin:.25rem 0;color:var(--color-muted)}.form-card{display:grid;grid-template-columns:3rem 1fr;gap:1.5rem;margin:1.5rem 0;padding:clamp(1.25rem,4vw,2.3rem);border:1px solid var(--color-border);background:var(--color-surface);box-shadow:8px 8px 0 var(--color-border)}.form-card>b{display:grid;place-items:center;width:2.6rem;height:2.6rem;background:var(--color-primary);color:var(--color-on-primary);--color-muted:rgb(255 255 255 / .72);}.form-card h2{margin:0;font-family:var(--font-serif);font-size:1.8rem;font-weight:500}.form-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:1.15rem;margin-top:1.5rem}.form-grid label{display:grid;gap:.45rem;font-weight:700}.form-grid input,.form-grid select,.form-grid textarea{box-sizing:border-box;width:100%;padding:.8rem;border:1px solid var(--color-border);border-radius:0;background:var(--color-surface);font:inherit}.form-grid input:focus,.form-grid select:focus,.form-grid textarea:focus{border-color:var(--color-primary)}.form-grid input[readonly]{background:var(--color-surface-muted)}.form-grid textarea{min-width:0}.no-customers,.wide{grid-column:span 2}.no-customers{padding:1.4rem;color:var(--color-on-primary);background:var(--color-primary);--color-muted:rgb(255 255 255 / .72);}.no-customers p{color:var(--color-muted)}.saved-items{display:flex;flex-wrap:wrap;gap:.5rem;margin:1rem 0;padding:.8rem;background:var(--color-surface-muted)}.saved-items strong{flex-basis:100%}.saved-items button{padding:.55rem .7rem;border:1px solid var(--color-border);background:var(--color-surface);font:inherit}.review p{font-size:1.05rem;line-height:1.6}.form-actions{display:flex;justify-content:flex-end;align-items:center;gap:1rem;padding-top:1rem}.success{padding:.8rem;background:var(--color-background);color:var(--color-positive)}@media(max-width:720px){.page-heading{display:block}.draft-badge{display:inline-flex;margin-top:1rem}.practice{align-items:start;flex-direction:column}.form-card{grid-template-columns:1fr;padding:1.25rem;box-shadow:5px 5px 0 var(--color-border)}.form-grid{grid-template-columns:1fr}.form-grid textarea,.no-customers,.wide{grid-column:auto}}</style>
