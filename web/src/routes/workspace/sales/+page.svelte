<script lang="ts">
 import { chooseWorkspace, requestedWorkspace } from '$lib/workspace-context';
  ...
  async function load() {
    ...
    try { organizationID = requestedWorkspace(businesses.data); } catch { businesses = {state:"error",scope:"",message:"This business is not available in your account.",status:404}; return; }
