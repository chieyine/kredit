<script lang="ts">
	...
	let principal = $state(''),
		...
		invoiceDocumentHash = $state('');
	let dueDate = $state(''),
		collectionAt = $state('');
	let deliveryMethod: Record<string, string> = $state({}),
		releaseEvidence: Record<string, string> = $state({});
	const money = formatKobo;
	let loading = $state(true),
		pauseReason = $state('');
	let capabilityError = $state(''),
		capabilityLoading = $state(true);
			...
			for (const org of candidates) {
				try {
					const result = await checkedJSON(
						`/api/v1/organizations/${encodeURIComponent(org.id)}/trade-lines/${encodeURIComponent(lineID)}/statement`,
						tradeStatement,
						{ signal: request.signal }
					);
					if (!request.current()) return;
					if (result.line.id !== lineID || result.line.supplier_organization_id !== org.id)
						throw new Error('Wrong customer limit');
					organizationID = org.id;
					statement = result;
					if (!pendingCommand) limit = nairaInput(result.line.approved_limit_kobo);
					return;
				} catch (cause) {
					if (!(cause instanceof RequestError && [403, 404].includes(cause.status))) throw cause;
				}
			}
			throw new Error('Customer limit unavailable');
		} catch (cause) {
			if (request.current()) error = publicError(cause, 'this customer limit');
		} finally {
	...
	async function command(path: string, body: unknown, key: string, method = 'POST') {
		...
		const lineID = statement.line.id,
		...
		try {
			...
			if (!intent) {
				intent = new MutationIntent('customer-limit-command', path);
			...
			await intent.run(
				...
				(value) => {
					const result = record(value),
						line = tradeLine(result.trade_line);
					...
					if (path.includes('/drawdowns')) {
						...
						if (sale.trade_line_id !== lineID || (key !== 'reserve' && sale.id !== key))
							throw new Error('Sale update not confirmed');
					}
					return result;
				},
				method
			...
			if (generation === routeGeneration && lineID === page.params.id) busy = '';
		}
	}
	...
	async function reserve(event: SubmitEvent) {
		...
		if (principalKobo <= 0 || !goods || !dueDate || !collectionAt) {
			error = 'Please enter the money, the goods, the payment day and the bank debit time.';
			return;
		}
		let collection: string;
		try {
			collection = lagosISO(collectionAt);
		} catch {
			error = 'Enter a valid Lagos collection date and time.';
			return;
		}
		const saved = await command(
			`/api/v1/organizations/${encodeURIComponent(organizationID)}/trade-lines/${encodeURIComponent(page.params.id ?? '')}/drawdowns`,
			{
				...
				invoice_document_hash: invoiceDocumentHash,
				due_date: dueDate,
	...
	async function loadCapabilities() {
		const request = capabilities.begin();
		capabilityLoading = true;
		capabilityError = '';
		try {
			const enabled = await checkedJSON(
			...
			if (request.current()) capabilityError = publicError(cause, 'sale availability');
		} finally {
			if (request.current()) capabilityLoading = false;
		}
	}
	$effect(() => {
...
<main class="shell workspace">
	...
	{#if statement}{@const lineID = statement.line.id}
		...
		<section class="summary">
			...
			<article>
				<span>Customer already owes</span><strong>{money(statement.line.current_exposure_kobo)}</strong>
			</article>
			<article><span>Waiting for customer</span><strong>{money(statement.line.reserved_pending_kobo)}</strong></article>
			<article><span>Now</span><strong>{stateLabel(statement.line.state)}</strong></article>
		</section>
		{#if error}<p class="error" role="alert">{error}</p>{/if}{#if notice}<p class="notice" role="status">
				{notice}
			</p>{/if}
		{#if pendingCommand}<section class="card">
				<p>The last change has not been confirmed. Retry its original details before making another change.</p>
				<button
					disabled={!!busy || loading}
					onclick={() => {
						if (pendingCommand)
							void command(pendingCommand.path, pendingCommand.body, pendingCommand.key, pendingCommand.method);
					}}>Retry the original change</button
				>
			</section>{/if}
		<section class="card compact">
			<h2>Pause this limit</h2>
			<p>While a limit is paused, the customer cannot use it for a new sale. Sales already running are not affected.</p>
			{#if statement.line.state === 'ACTIVE'}<label
					>Why are you pausing this limit?<input
				...
				>{:else if statement.line.state === 'SUSPENDED'}<button
					class="primary"
		...
		{#if capabilityLoading}<section class="card">
				<p role="status">Checking whether sales from this limit are available…</p>
			</section>
		{:else if capabilityError}<section class="card">
				<p role="alert">{capabilityError}</p>
				<button onclick={loadCapabilities}>Check sale availability again</button>
			</section>
		{:else if canAddDrawdown}
			<section class="card">
				<h2>Add a sale from this limit</h2>
				<p>Your customer sees the goods, the money and the payment day before you hand anything over.</p>
				<form onsubmit={reserve} class="form-grid">
					<label
						>Money to pay (₦)<input
							disabled={!!busy || !!pendingCommand}
						...
						/></label
					><label
						>What are they buying?<textarea disabled={!!busy || !!pendingCommand} bind:value={goods} rows="3" required
						></textarea></label
					><label
						>Invoice number <small>optional</small><input
							disabled={!!busy || !!pendingCommand}
							bind:value={invoiceReference}
						/></label
					><label
						>Pay before<input disabled={!!busy || !!pendingCommand} type="date" bind:value={dueDate} required /></label
					><label
						>If unpaid, Kredit may debit their bank after (Lagos time)<input
							disabled={!!busy || !!pendingCommand}
							type="datetime-local"
							bind:value={collectionAt}
							required
						/></label
					><button class="primary wide" disabled={!!busy || !!pendingCommand || loading}
						>{busy === 'reserve' ? 'Saving…' : `Add ${money(parseNaira(principal))} sale`}</button
					>
				</form>
			</section>
		{:else}
			<section class="card">
				<h2>Add a sale from this limit</h2>
				<p>
					Selling from a customer limit is switched off for this account, so there is nothing to fill in here. You can
					still record a normal sale, and the balance below stays up to date.
				</p>
				<a class="primary" href="/workspace/sales/new">Record a sale</a>
			</section>
		{/if}
		<section class="card compact">
			<h2>Lower the limit</h2>
			<p>You can only take back what the customer has not used yet. To raise a limit, the customer must agree first.</p>
			<label>New limit (₦)<input disabled={!!busy || !!pendingCommand} bind:value={limit} inputmode="decimal" /></label
			><button disabled={!!busy || !!pendingCommand || loading} onclick={reduce}
				>Change limit to {money(parseNaira(limit))}</button
			>
		</section>
		...
		{#if statement.drawdowns.length}<div class="drawdowns">
				{#each statement.drawdowns as drawdown (drawdown.id)}<article class="drawdown">
						...
						<dl>
							...
							<dd>{drawdown.grace_hours} hours</dd>
							<dt>Invoice number</dt>
							<dd>{drawdown.invoice_reference || 'None'}</dd>
						</dl>
						<details class="hash">
						...
						>
						{#if drawdown.state === 'BUYER_CONFIRMED'}<div class="action">
								<label
									>How will they get the goods?<input
										disabled={!!busy || !!pendingCommand}
										bind:value={deliveryMethod[drawdown.id]}
										placeholder="Delivery or pickup"
									/></label
								><label
									>Delivery or receipt number<input
										disabled={!!busy || !!pendingCommand}
										bind:value={releaseEvidence[drawdown.id]}
								...
								><button
									...
									onclick={() =>
										command(
											...
											drawdown.id
										)}>The goods have left</button
								>
							</div>{/if}
						{#if ['PENDING_BUYER_CONFIRMATION', 'BUYER_CONFIRMED'].includes(drawdown.state)}<button
								class="danger"
								disabled={!!busy || !!pendingCommand || loading}
								onclick={() =>
									command(
							...
							>{/if}
						{#if drawdown.state === 'RECEIPT_ISSUE_REPORTED'}<div class="action">
								<p>
									If the goods have been returned or you agree to cancel without collecting this sale, accept the
									cancellation below. This releases the reserved limit without creating a debt. Otherwise, correct the
									problem and ask the customer to confirm receipt.
								</p>
								<button
									class="danger"
									disabled={!!busy || !!pendingCommand || loading}
						...
						{#if drawdown.release_actor_id}<p>
								How the goods left: {drawdown.delivery_method}{drawdown.release_evidence_reference
									? ` · ${drawdown.release_evidence_reference}`
									: ''}
							</p>{/if}{#if drawdown.receipt_state === 'issue_reported'}<p class="error">
								Customer reported: {drawdown.receipt_issue_reason}
							</p>{/if}
					</article>{/each}
			</div>{:else}<p>No sale has used this limit yet.</p>{/if}
	{:else if error}<h1>We could not open this limit.</h1>
		<p role="alert">{error}</p>
		<button type="button" onclick={() => load()}>Try again</button>{:else}<p role="status">
			Opening customer limit…
		</p>{/if}
</main>
