...
<main class="shell workspace">
	<a href="/workspace/sales/limits">← Customer limits</a><p class="eyebrow">Customer limit</p>
	{#if statement}
  ...
  {:else if canAddDrawdown}
		...
		{:else}
		<section class="card"><h2>Add a sale from this limit</h2><p>Selling from a customer limit is switched off for this account, so there is nothing to fill in here. You can still record a normal sale, and the balance below stays up to date.</p><a class="primary" href="/workspace/sales/new">Record a sale</a></section>
		{/if}
		...
		{#if statement.drawdowns.length}<div class="drawdowns">{#each statement.drawdowns as drawdown}<article class="drawdown">
			...
			{#if drawdown.state === 'RECEIPT_ISSUE_REPORTED'}<div class="action"><p>If the goods have been returned or you agree to cancel without collecting this sale, accept the cancellation below. This releases the reserved limit without creating a debt. Otherwise, correct the problem and ask the customer to confirm receipt.</p><button class="danger" disabled={!!busy || !!pendingCommand || loading} onclick={() => command(`/api/v1/organizations/${organizationID}/trade-lines/${statement.line.id}/drawdowns/${drawdown.id}/cancel`, {}, drawdown.id)}>Accept return and cancel this sale</button></div>{/if}
