 ...
 async function load() {
  ...
  try {
   if (!organizations.length) {
    ...
    const requested = new URLSearchParams(location.search).get('organization'); organizationID = requestedWorkspace(result);
   }
...
<main class="shell workspace"><header><p class="eyebrow">Customer limits</p><h1>Customer credit limits</h1><p class="lede">Set the most one customer may owe you at any time. Every new sale eats into that amount, and paying back frees it up again.</p></header>
