<script lang="ts">
 ...
 function entity(value: unknown, field: string) { const result = record(value); if(!text(record(result[field]).id))throw new Error('Missing record reference'); return result; }
 function checkedSale(value: unknown) {
   const result = entity(value, 'request'), sale = record(result.request);
   text(sale.state); text(sale.buyer_legal_name);text(sale.goods_description);kobo(sale.principal_kobo);text(sale.due_date);
 ...
 function moneyRow(value: unknown, field: string) { const row = record(value); text(row.id); kobo(row[field]); text(row.state); return row; }
	...
	let deliveryMethod = $state('supplier_delivery'), releaseNotes = $state(''), paymentAmount = $state(''), paymentReference = $state(''), paymentPaidAt = $state(''), paymentSource = $state('supplier_recorded_transfer');
 ...
 async function load(requestID = id, selectedOrg = page.url.searchParams.get('organization') ?? organizationID) {
   ...
   try {
     ...
     if (view.request.state === 'DRAFT') {
       draftPrincipal = nairaInput(view.request.principal_kobo); draftGoods = view.request.goods_description;
       draftDueDate = view.request.due_date; draftCollectionAt = localInput(view.request.collection_at); draftGraceHours = view.request.grace_hours;
     }
     if (view.obligation) {
       const [p, s, c, e, a] = await Promise.all([
         ...
         readResource(base, `${base}/schedule`, value => { const data = record(value); const items = rows('items', item => { const row = moneyRow(item, 'principal_due_kobo'); kobo(row.allocated_kobo); if(!Number.isFinite(Date.parse(text(row.due_at))))throw new Error('Invalid payment date'); return row; })(data); return { schedule: data.schedule, items }; }, request.signal, 'payment dates'),
         ...
         readResource(base, `${base}/collection/eligibility`, value => { const data = record(value); if (typeof data.eligible !== 'boolean') throw new Error('Missing eligibility'); if (data.eligible) kobo(data.amount_kobo); if(data.reasons!==undefined&&(!Array.isArray(data.reasons)||!data.reasons.every(value=>typeof value==='string')))throw new Error('Invalid eligibility reasons'); return data; }, request.signal, 'bank debit eligibility'),
 ...
 async function updateDraft() {
   ...
   try {
     ...
     await mutate(`/api/v1/organizations/${organizationID}/credit-requests/${id}`, {
       ...
       due_date: draftDueDate, grace_hours: Number(draftGraceHours), collection_at: lagosISO(draftCollectionAt)
     }, 'request', 'PATCH');
 ...
 async function recordPayment() {
   ...
   try {
     ...
     const confirmed = await command('payments', { amount_kobo: amount, currency: 'NGN', source_type: paymentSource, provider_reference: paymentReference, paid_at: paidAt });
     if (confirmed) {
       paymentNotice = `${formatKobo(amount)} payment saved.`;
       paymentAmount = ''; paymentReference = ''; paymentPaidAt = '';
     }
   } catch { error = 'Check the amount and actual payment date/time before saving this payment.'; }
 }
 ...
 async function openInvoice(){
  if(busy||loading)return;error='';
  try {
   const url=await checkedJSON(`/api/v1/organizations/${encodeURIComponent(organizationID)}/documents/${encodeURIComponent(view.request.invoice_document_id)}/download`,value=>{
    const url=new URL(text(record(value).url),location.origin);
    if(!['https:','http:'].includes(url.protocol)||url.username||url.password||(location.protocol==='https:'&&url.protocol!=='https:'))throw new Error('Invalid invoice link');return url.href;
 ...
 $effect(() => {
   ...
   untrack(() => { review = null; notice = ''; paymentNotice = ''; paymentAmount = ''; paymentReference = ''; paymentPaidAt = ''; claimReviewReason = ''; void load(requestID, selectedOrg); });
   return () => reads.cancel();
 });
</script>
...
<section class="page-shell">
	...
	{#if !view}{#if loading}<p role="status">Opening sale…</p>{:else}<button onclick={() => load()}>Try again</button>{/if}{:else}
		...
		<div class="quick-actions"><a class="repeat" href={`/workspace/sales/new?organization=${encodeURIComponent(organizationID)}&customer=${encodeURIComponent(view.request.buyer_user_id ?? '')}&goods=${encodeURIComponent(view.request.goods_description)}&amount=${encodeURIComponent(nairaInput(view.request.principal_kobo))}`}>Sell these same goods again</a><a class="repeat" href={`/workspace/sales/${id}/deliveries?organization=${encodeURIComponent(organizationID)}`}>Deliveries & Credit Notes →</a><ShareActions compact title="Kredit payment reminder" text={`Hello ${view.request.buyer_legal_name}, this is a reminder that ${formatKobo(view.obligation?.outstanding_kobo ?? view.request.principal_kobo)} is left for ${view.request.goods_description}. Payment day: ${scheduleItems.find((i:any)=>i.state!=='CANCELLED'&&i.principal_due_kobo>i.allocated_kobo)?.due_at?.slice(0,10)??(view.obligation?'Check your current payment schedule':view.request.due_date)}.`} /></div>
		...
		{#if view.request.state === 'READY_TO_RELEASE'}<section class="card action"><h2>Have the goods been dispatched?</h2><label>How did they get the goods?<select disabled={busy||loading} bind:value={deliveryMethod}><option value="supplier_delivery">We delivered them</option><option value="buyer_collection">The customer came and collected</option><option value="third_party_delivery">Somebody else delivered them</option></select></label><label>Delivery note number<textarea disabled={busy||loading} bind:value={releaseNotes}></textarea></label><button class="primary" disabled={busy || loading} onclick={() => command('release',{delivery_method:deliveryMethod,notes:releaseNotes})}>Yes, the goods have left</button></section>{/if}
		{#if view.obligation}
			<section class="card action"><h2>Enter money you have received</h2><p>Only enter money that has actually reached you. Open your bank app and see it first.</p>{#if paymentNotice}<p class="success" role="status">{paymentNotice}</p>{/if}<label>How did they pay you?<select disabled={busy||loading} bind:value={paymentSource}><option value="supplier_recorded_transfer">Bank transfer or POS</option><option value="cash_recorded">Cash</option></select></label><label>When did the money reach you? (Nigerian time)<input type="datetime-local" disabled={busy||loading} bind:value={paymentPaidAt} required /></label><label>How much did you receive? (₦)<input disabled={busy||loading} bind:value={paymentAmount} inputmode="decimal" /></label><label>{paymentSource === 'cash_recorded' ? 'Receipt or note' : 'Transfer or POS number'} <small>optional</small><input disabled={busy||loading} bind:value={paymentReference} /></label>{#if paymentSource === 'cash_recorded'}<p class="warning">If they paid cash, give them a receipt before you save this.</p>{/if}<button class="primary" disabled={busy || loading} onclick={recordPayment}>Save this payment</button><h3>Payments so far</h3>{#if loading}<p role="status">Checking payment history…</p>{:else if paymentError}<p role="alert">{paymentError}</p><button onclick={() => load()}>Check payment history again</button>{:else if payments.length}<ul class="plain-list">{#each payments as payment}<li><span><Money amountKobo={payment.amount_kobo} /> · {productLabel(payment.source_type)}</span><strong>{productLabel(payment.state)}</strong></li>{/each}</ul>{:else}<p>No payment yet.</p>{/if}</section>

			<details class="card action"><summary><strong>Payment days</strong> <span class="hint">the agreed dates</span></summary>{#if loading}<p>Checking the payment schedule…</p>{:else if scheduleError}<p role="alert">{scheduleError}</p>{:else if scheduleItems.length}<div class="table-wrap"><table><thead><tr><th>Pay before</th><th>Expected</th><th>Paid</th><th>Now</th></tr></thead><tbody>{#each scheduleItems as item}<tr><td>{dateLabel(item.due_at.slice(0,10))}</td><td><Money amountKobo={item.principal_due_kobo} /></td><td><Money amountKobo={item.allocated_kobo} /></td><td>{productLabel(item.state)}</td></tr>{/each}</tbody></table></div>{:else}<p>No scheduled payments are present in this checked record.</p>{/if}<p class="muted">These are the payment days as they stand. To change a date, somebody at Kredit must review it and your customer must agree to it. Contact support to ask.</p></details>

			{#if claimError}<p role="alert">{claimError}</p>{/if}
			{#if paymentClaims.length}<section class="card action"><h2>Transfers your customer says they sent</h2><p>Open your bank and verify the amount, reference and payment date first. Reject an incorrect report so the customer can correct it. The moment you accept it, the balance drops.</p>{#if paymentClaims.length}<div class="claim-list">{#each paymentClaims as claim}<article><div><strong><Money amountKobo={claim.amount_kobo} /></strong><span>{productLabel(claim.state)} · {claim.transfer_reference}</span><small>Customer’s payment date: {timeLabel(claim.paid_at)}</small></div>{#if ['pending','expired'].includes(claim.state?.toLowerCase())}<label>Why are you accepting or rejecting this?<input disabled={busy||loading} bind:value={claimReviewReason} placeholder="For example: Seen in our bank account" /></label><div class="button-row"><button class="primary" disabled={busy || loading} onclick={()=>decidePaymentClaim(claim,'confirmed')}>Yes, this money reached me</button><button disabled={busy || loading} onclick={()=>decidePaymentClaim(claim,'rejected')}>No, I never received it</button></div>{:else if claim.review_reason}<p>{claim.review_reason}</p>{/if}</article>{/each}</div>{:else}<p>Your customer has not reported any transfer.</p>{/if}</section>{/if}

			<details class="card action" open={eligibility?.eligible}><summary><strong>Ask their bank for the money</strong> <span class="hint">{eligibility?.eligible ? 'available now' : eligibilityError || loading ? 'status unavailable' : 'not available yet'}</span></summary>{#if loading}<p>Checking bank debit eligibility…</p>{:else if eligibilityError}<p role="alert">{eligibilityError}</p>{:else if eligibility?.eligible}<p>The currently eligible amount is <strong><Money amountKobo={eligibility.amount_kobo} /></strong> under the customer’s bank permission. Collection is not guaranteed.</p><button class="primary" disabled={busy || loading} onclick={startCollection}>Ask the bank now</button>{:else}<p>Bank debit cannot start yet.</p>{#if eligibility?.reasons?.length}<ul>{#each eligibility.reasons as reason}<li>{productLabel(reason)}</li>{/each}</ul>{/if}{/if}{#if collectionError}<p role="alert">{collectionError}</p>{/if}{#if collectionAttempts.length}<h3>What happened with bank debits</h3><ul class="plain-list">{#each collectionAttempts as attempt}<li><span><Money amountKobo={attempt.requested_amount_kobo} /> · {productLabel(attempt.state)}<small>{timeLabel(attempt.requested_at)}</small></span><span class="button-row">{#if attempt.state==='FAILED'}<button disabled={busy || loading} onclick={()=>collectionAction(attempt,'retry')}>Try again</button>{/if}{#if attempt.state==='SUBMITTED'||attempt.state==='UNKNOWN'}<button disabled={busy || loading} onclick={()=>collectionAction(attempt,'reconcile')}>Check what happened</button>{/if}</span></li>{/each}</ul>{/if}</details>

			<details class="card action"><summary><strong>Fix a mistake</strong></summary><p>Only use this when something you saved is genuinely wrong. Every change you make here is recorded.</p><h3>Remove a payment entered by mistake</h3><label>Payment<select disabled={busy||loading} bind:value={reversePaymentID}><option value="">Choose a payment</option>{#each payments.filter((payment)=>payment.state?.toUpperCase()==='RECOGNIZED') as payment}<option value={payment.id}>{formatKobo(payment.amount_kobo)} · {payment.provider_reference||productLabel(payment.source_type)}</option>{/each}</select></label><label>Why is it wrong?<textarea disabled={busy||loading} bind:value={reverseReason} rows="3"></textarea></label><button disabled={busy || loading} onclick={reversePayment}>Remove this payment</button><hr/><h3>Reduce what is owed on this sale</h3><label>What do you want to reduce?<select disabled={busy||loading} bind:value={operationType}><option value="write-off">The money the customer owes</option><option value="fee-waiver">A Kredit fee</option></select></label><label>Amount (₦)<input disabled={busy||loading} bind:value={operationAmount} inputmode="decimal" /></label><label>Reason<textarea disabled={busy||loading} bind:value={operationReason} rows="3"></textarea></label><p>Larger corrections are proposed and recorded in Admin, under Financial changes. This form handles the smaller ones.</p><button disabled={busy || loading} onclick={changeMoney}>Save this change</button></details>

			<details class="card action"><summary><strong>Report a problem</strong> <span class="hint">goods or amount wrong</span></summary><p>Say how much money is affected, and what went wrong. Only the disputed amount is held. A debit already submitted to the bank may still complete.</p><form onsubmit={openDispute}><label>How much money is affected? (₦)<input disabled={busy||loading} bind:value={disputeAmount} inputmode="decimal" required /></label><label>In one line, what is wrong?<input disabled={busy||loading} bind:value={disputeReason} required /></label><label>Tell us the full story<textarea disabled={busy||loading} bind:value={disputeExplanation} rows="4"></textarea></label><label>While this is sorted out, what should happen to bank debit?<select disabled={busy||loading} bind:value={disputeEffect}><option value="CONTESTED_ONLY">Hold only the money in question</option></select></label><button class="primary" disabled={busy || loading}>Report this problem</button></form></details>
		{/if}
	{/if}
	</section>
{#if review}<PaymentReview amount={review.claim.amount_kobo} reference={review.claim.transfer_reference} decision={review.decision} busy={busy} error={error} onconfirm={confirmClaim} oncancel={() => { if(!busy){review = null; error = '';} }} />{/if}
<style>details.action>summary{cursor:pointer;list-style:none;display:flex;align-items:center;justify-content:space-between;gap:1rem;min-height:2.75rem;font-size:1rem}details.action>summary::-webkit-details-marker{display:none}details.action>summary::after{content:"+";font-weight:700;color:var(--color-accent,var(--color-primary))}details.action[open]>summary::after{content:"\2013"}details.action>summary .hint{font-size:.78rem;font-weight:600;color:var(--color-muted,var(--color-foreground))}details.action[open]>summary{margin-bottom:1rem;padding-bottom:.75rem;border-bottom:1px solid var(--color-border,var(--color-border))}.detail-grid{display:grid;grid-template-columns:2fr 1fr;gap:1rem;margin:1.5rem 0}.card{padding:1.5rem}.notice{padding:.8rem 1rem;background:var(--color-background)}.quick-actions{display:flex;flex-wrap:wrap;align-items:center;gap:.7rem}.repeat{display:inline-flex;align-items:center;min-height:2.5rem;padding:.35rem .75rem;border:1px solid var(--color-border);font-weight:750;text-decoration:none}dl div{display:flex;justify-content:space-between;gap:2rem;border-bottom:1px solid var(--color-border);padding:.7rem 0}dd{margin:0;text-align:right;font-weight:700}.action,.action form{display:grid;gap:.8rem;margin:1rem 0}.action form{margin:0}.action label{display:grid;gap:.35rem;font-weight:700}.action input,.action select,.action textarea{padding:.75rem;border:1px solid var(--color-border);border-radius:.7rem;font:inherit}.button-row{display:flex;flex-wrap:wrap;gap:.7rem}.danger{color:var(--color-destructive);border-color:var(--color-destructive);background:var(--color-surface)}.warning{padding:.75rem;border-left:4px solid var(--color-warning);background:var(--color-surface)}.success{color:var(--color-positive);font-weight:750}.table-wrap{overflow-x:auto}table{width:100%;border-collapse:collapse}th,td{text-align:left;padding:.75rem;border-bottom:1px solid var(--color-border);white-space:nowrap}.plain-list,.claim-list{display:grid;gap:.65rem;padding:0;list-style:none}.plain-list li,.claim-list article{display:flex;align-items:center;justify-content:space-between;gap:1rem;padding:.8rem;border:1px solid var(--color-border);border-radius:.75rem}.plain-list span,.claim-list article>div{display:grid;gap:.2rem}.claim-list article{align-items:stretch;display:grid}.claim-list small,.plain-list small{display:block;color:var(--color-muted)}details summary{cursor:pointer}hr{width:100%;border:0;border-top:1px solid var(--color-border)}code{overflow-wrap:anywhere}@media(max-width:720px){.detail-grid{grid-template-columns:1fr}.plain-list li{align-items:flex-start;flex-direction:column}}
</style>
