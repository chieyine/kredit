<script lang="ts">
  ...
  // Dispatch form state
  let newCarrier = $state(''), newTracking = $state(''), dispatchQuantities = $state<Record<string, number>>({});

  // Credit note form state
  let creditAmount = $state(''), creditReason = $state('');
  ...
  async function load() {
    ...
    try {
      ...
      const res = await checkedJSON(`${base}/deliveries`, val => {
        const r = record(val);
        return {
...
<main class="shell workspace deliveries-page">
  ...
  <p class="lede">Track partial shipments, signed delivery receipts, and approved credit adjustments with dual-control governance.</p>
  ...
  {:else}
    ...
    <section class="card">
      ...
      {#if !lineItems.length}
        <p class="empty-note">This order is tracked at the whole-contract level, or line items have not yet been broken down.</p>
      {:else}
    ...
    <section class="card">
      ...
      <form onsubmit={(e) => { e.preventDefault(); void dispatchShipment(); }} class="dispatch-form">
        <h3>Dispatch New Shipment</h3>
        <div class="form-row">
          <label>Carrier<input bind:value={newCarrier} placeholder="e.g. Kredit Logistics Fleet" required disabled={busy} /></label>
    ...
    <section class="card">
      ...
      <p class="muted">Credit notes reduce outstanding receivable balances. Under maker-checker dual control, the issuer cannot approve their own credit note.</p>
      ...
      <form onsubmit={(e) => { e.preventDefault(); void createCreditNote(); }} class="credit-form">
        <h3>Issue Credit Note Draft</h3>
        <div class="form-row">
          <label>Amount (₦)<input bind:value={creditAmount} placeholder="e.g. 50,000.00" inputmode="decimal" required disabled={busy} /></label>
          <label>Reason<input bind:value={creditReason} placeholder="e.g. Volume discount / goods return" required disabled={busy} /></label>
        </div>
...
<style>
  ...
  .shipment-list, .credit-notes-list {
    display: flex;
    flex-direction: column;
    gap: 0.75rem;
    margin: 1rem 0;
  }
  .shipment-item, .credit-note-item {
    display: flex;
