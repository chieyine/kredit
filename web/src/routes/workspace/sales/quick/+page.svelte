<script lang="ts">
	...
	const reads = new LatestRequest(),
		businessReads = new LatestRequest();
	...
	const buyer = $derived(
		customers.state === 'ready' ? customers.data.find((item) => customerKey(item) === selectedBuyer) : undefined
	);
	function restoreForBusiness() {
		...
		try {
			...
			draftWarning = 'This browser cannot keep a draft. Keep this page open until you finish.';
		}
		draftReady = true;
	}
	$effect(() => {
		...
		try {
			...
			else if (!saveDraft(account.userID, organizationID, { goods, principal, dueDate }, sessionStorage))
				draftWarning = 'Your draft could not be kept on this device. Keep this page open until you finish.';
		} catch {
			draftWarning = 'Your draft could not be kept on this device. Keep this page open until you finish.';
		}
	});
	async function loadCustomers(resetDraft = false) {
		...
		try {
			...
			if (request.current() && organizationID === scope)
				customers = {
					state: 'error',
					scope,
					status: 0,
					message: cause instanceof Error ? cause.message : 'Payment settings could not be loaded. Try again.'
				};
		}
	}
	...
	async function next() {
		...
		error = '';
		if (step === 1 && !buyer) {
			error = 'Choose the customer for this sale.';
			return;
		}
		if (step === 2 && (!goods.trim() || goods.trim().length > 5000 || amount <= 0)) {
			error = 'Enter the goods and a valid amount, with no more than two decimal places.';
			return;
		}
		if (step === 3) {
			if (!/^\d{4}-\d{2}-\d{2}$/.test(dueDate)) {
				error = 'Choose the agreed payment date.';
				return;
			}
			busy = true;
			timing = null;
			try {
				...
				const result = await checkedJSON(
					`/api/v1/organizations/${encodeURIComponent(scope)}/credit-terms/preview`,
					(value) => {
						const result = record(value),
							collectionAt = text(result.collection_at);
						if (
							result.due_date !== date ||
							result.grace_hours !== graceHours ||
							result.timezone !== 'Africa/Lagos' ||
							result.timing_mode !== 'lagos_end_of_day' ||
							!Number.isFinite(Date.parse(collectionAt))
						)
					...
					},
					{
						...
						body: JSON.stringify({ due_date: date, grace_hours: graceHours })
					}
				);
	...
	async function submit() {
		...
		try {
			if (
				...
				amount <= 0 ||
				!goods.trim() ||
				timing?.dueDate !== dueDate ||
				timing.organizationID !== organizationID
			)
				throw new Error('Go back and check the customer, amount and payment date.');
			...
			const id = await creation.run(
				{
					...
					collection_at: timing.collectionAt,
			...
			error = cause instanceof Error ? cause.message : 'We could not confirm that this sale was saved.';
		} finally {
			busy = false;
		}
	}
...
<main class="shell quick-sale">
	<header class="task-heading">
		<div>
			<p class="eyebrow">New credit sale</p>
			<h1>Add a sale</h1>
			<p class="lede">Choose the customer, goods, amount and payment date.</p>
		</div>
		<a href={`/workspace/sales/new?advanced=1&organization=${encodeURIComponent(organizationID)}`}
			>Instalments or an invoice? Use the full form</a
		>
	</header>
	<ResourceNotice resource={organizations} label="Businesses" retry={load} />
	{#if organizations.state === 'ready' && !organizations.data.length}<div class="empty-state">
			<h2>Add your business first</h2>
			<a class="primary" href="/workspace/today">Add business details</a>
		</div>
	{:else if organizationID}
		{#if recoveredDraft}<div class="inline-notice" role="status">
				<p>Your draft for this business was restored. Choose the customer again.</p>
				<button
					type="button"
					disabled={busy}
					onclick={() => {
		...
		<section class="sale-card" aria-busy={busy}>
			<h2 bind:this={heading} tabindex="-1">
				{['Who is buying?', 'What are you supplying?', 'When will they pay?', 'Check your sale'][step - 1]}
			</h2>
			{#if step === 1}
				...
				{#if customers.state === 'ready'}{#if customers.data.length}<label
							>Customer<select bind:value={selectedBuyer} disabled={busy}
								><option value="">Choose a customer</option>{#each customers.data as item}<option
									...
									>{/each}</select
							></label
						>{#if buyer}<div class="inline-notice">
								<strong>{buyer.trading_name || buyer.legal_name}</strong
								>{#if buyer.trading_name && buyer.trading_name !== buyer.legal_name}<p>
										Contracting party: {buyer.legal_name}
									</p>{/if}{#if buyer.overdue}<p>
										This customer has an overdue record. Review it before offering more credit.
									</p>{/if}
							</div>{/if}{:else}<div class="empty-state">
							<h3>No customers yet</h3>
							<p>Invite a customer to confirm their own details.</p>
						</div>{/if}<a href={`/workspace/partners/customers/new?organization=${encodeURIComponent(organizationID)}`}
						>Add a customer</a
					>{/if}
			{:else if step === 2}
				<label
					>Goods and quantity<textarea
						bind:value={goods}
						rows="4"
						maxlength="5000"
						placeholder="For example: 40 cartons of 5L cooking oil"
						disabled={busy}
					></textarea></label
				>
				<label
			...
			{:else if step === 3}
				...
				<div class="inline-notice">
					...
					<p>
						The day ends at 11:59 pm Nigerian time. This sale allows {graceHours} extra hours before a bank debit can be considered.
					</p>
			...
			{:else}
				<dl class="sale-summary">
					<div>
						<dt>Customer</dt>
						<dd>{buyer?.trading_name || buyer?.legal_name}</dd>
					</div>
					{#if buyer?.trading_name && buyer.trading_name !== buyer.legal_name}<div>
							<dt>Contracting party</dt>
					...
					<div>
						<dt>Sale amount</dt>
						<dd class="amount"><Money amountKobo={amount} /></dd>
					</div>
					<div>
						<dt>Pay by</dt>
						<dd>{dateLabel(dueDate)}</dd>
					</div>
					<div>
						<dt>Bank debit may be considered from</dt>
						<dd>{timing ? timeLabel(timing.collectionAt) : 'Not yet verified'}</dd>
					</div>
				</dl>
				<p class="field-help">
					This saves a draft. Review the complete terms and fees on the next screen before sending it. Your customer
					must accept the agreement and complete the required bank permission.
				</p>
			{/if}
		</section>
		<footer class="form-actions">
			<div>
				{#if step > 1}<button class="secondary" type="button" disabled={busy} onclick={back}>Back</button>{/if}
			</div>
			{#if step < 4}<button class="primary" type="button" onclick={next} disabled={busy || (step === 1 && !buyer)}
					>{busy ? 'Checking date…' : 'Continue'}</button
				>{:else}<button class="primary" type="button" onclick={submit} disabled={busy || !timing}
					>{busy ? 'Saving…' : 'Save draft sale'}</button
				>{/if}
		</footer>
		<label class="draft-choice"
			><input type="checkbox" bind:checked={keepDraft} />Keep this draft on this device for up to 12 hours</label
		>
		<p class="field-help">
			Draft saving is off until you choose it. Avoid using it on a shared device. The selected customer is never saved
			in a browser draft.
		</p>
		{#if draftWarning}<p role="status">{draftWarning}</p>{/if}
	{/if}
</main>
