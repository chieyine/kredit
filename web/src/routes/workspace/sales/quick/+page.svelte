<script lang="ts">
 import { requestedWorkspace } from '$lib/workspace-context';
  ...
  async function load() {
    ...
    try { organizationID = requestedWorkspace(result.data); } catch { organizations = {state:'error',scope:account.userID,status:404,message:'This business is not available in your account.'}; return; }
    ...
    if (customers.state === 'ready') {
      const matches = customers.data.filter(item => item.buyer_user_id === params.get('customer') && (!params.get('customer_business') || item.buyer_business_id === params.get('customer_business')));
...
<main class="shell quick-sale">
  <header class="task-heading"><div><p class="eyebrow">New credit sale</p><h1>Add a sale</h1><p class="lede">Choose the customer, goods, amount and payment date.</p></div><a href={`/workspace/sales/new?advanced=1&organization=${encodeURIComponent(organizationID)}`}>Instalments or an invoice? Use the full form</a></header>
  <ResourceNotice resource={organizations} label="Businesses" retry={load} />
  {#if organizations.state === 'ready' && !organizations.data.length}<div class="empty-state"><h2>Add your business first</h2><a class="primary" href="/workspace/today">Add business details</a></div>
