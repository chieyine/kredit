<script lang="ts">
 ...
 async function initialize() {
  ...
  try {
   const result = await checkedJSON('/api/v1/organizations', rows('organizations', value => ({ id: text(record(value).id), legal_name: text(record(value).legal_name) })), { signal: request.signal });
   if (!request.current()) return;
   organizations = result; organizationID = requestedWorkspace(result); await load();
