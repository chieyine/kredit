<script lang="ts">
 ...
 import { organization, kobo, paymentRow } from '$lib/records';
	...
	type PortfolioHealth = { on_time_collection_bps: number; disputed_ratio_bps: number; top_concentration_bps: number; health_rating: string };
	type BranchExposure = { branch_id: string; branch_name: string; territory: string; total_outstanding_kobo: KoboValue; ageing_buckets: Record<string, KoboValue>; customer_count: number; overdue_count: number };
	type EnterpriseReport = { organization_id: string; generated_at: string; portfolio_health: PortfolioHealth; branch_exposures: BranchExposure[]; integrity_hash: string };
 ...
 async function load() {
  ...
  loading=true;error='';summary=null;buckets={};fees=null;payments=[];enterprise=null;
  ...
  try{
   const [nextSummary,nextBuckets,nextFees,nextPayments,nextEnterprise]=await Promise.all([
    ...
    checkedJSON(`${root}/reports/enterprise`,(value)=>{
      ...
      const branches:BranchExposure[]=rawB.map((b:any)=>({
        ...
        ageing_buckets:Object.fromEntries(Object.entries(record(b.ageing_buckets??{})).map(([k,v])=>[k,kobo(v)])),
    ...
    },{signal:request.signal}).catch(()=>null)
   ]);
   if(!request.current()||organizationID!==scope)return;
   summary=nextSummary;buckets=nextBuckets;fees=nextFees;payments=nextPayments;enterprise=nextEnterprise;
 ...
 async function initialize(){
  ...
  try{
   ...
   organizationID=requestedWorkspace(next);await load();
	...
	async function exportEnterpriseCSV() {
		...
		try {
			...
			if (!response.ok) { error = 'Could not generate enterprise report. Please try again.'; return; }
...
<main class="shell workspace reports"><header><p class="eyebrow">Business performance</p><h1>Business reports</h1><p class="lede">What you are owed, what is late, what has come in, how old the debts are and what Kredit has charged you.</p></header>
	<div class="toolbar">
		<label>Business<select disabled={exporting || exportingEnterprise} bind:value={organizationID} onchange={()=>chooseWorkspace(organizationID)}>{#each organizations as organization}<option value={organization.id}>{organization.trading_name || organization.legal_name}</option>{/each}</select></label>
		<div class="actions">
			<button onclick={exportCSV} disabled={!summary || exporting || loading || Boolean(error)}>{exporting ? 'Preparing…' : 'Download Receivables CSV'}</button>
			<button class="secondary-btn" onclick={exportEnterpriseCSV} disabled={!enterprise || exportingEnterprise || loading || Boolean(error)}>{exportingEnterprise ? 'Signing…' : 'Enterprise Report CSV'}</button>
		</div>
	</div>
	{#if error}<p class="error" role="alert">{error}</p><button onclick={initialize}>Try again</button>{:else if loading}<div class="loading" aria-live="polite"><Skeleton rows={4} tall /></div>{:else if !organizations.length}<p>Add your business to see its reports. <a href="/workspace/today">Set up your business →</a></p>{:else if summary}
			...
			<section class="card enterprise-health">
				...
				<div class="health-grid">
					...
					<article>
						...
						<strong class="hash-code">{enterprise.integrity_hash ? `${enterprise.integrity_hash.slice(0, 8)}…` : 'Verified'}</strong>
...
<style>.reports>header{max-width:62rem}.reports h1{max-width:15ch;font-family:var(--font-serif);font-size:clamp(3rem,6vw,5.4rem);font-weight:500;line-height:.92;letter-spacing:-.055em}.toolbar{display:flex;justify-content:space-between;gap:1rem;align-items:end;margin:2rem 0;padding:1rem;flex-wrap:wrap;color:var(--color-on-primary);background:var(--color-primary);--color-muted:rgb(255 255 255 / .72);}.toolbar label{display:grid;gap:.4rem;font-weight:700}.toolbar select,.toolbar button{min-height:3rem;padding:.75rem;border:1px solid var(--color-border-strong);border-radius:0;background:var(--color-surface);color:var(--color-primary)}.toolbar .actions{display:flex;gap:.75rem;flex-wrap:wrap;}.toolbar button{background:var(--color-primary);color:var(--color-on-primary);border-color:var(--color-primary);font-weight:750;--color-muted:rgb(255 255 255 / .72);}.toolbar .secondary-btn{background:var(--color-surface);color:var(--color-primary);border-color:var(--color-border-strong);}.stats{display:grid;grid-template-columns:1.2fr repeat(4,1fr);border-top:1px solid var(--color-border);border-left:1px solid var(--color-border)}.stats article,.card{padding:1.25rem;border:0;border-right:1px solid var(--color-border);border-bottom:1px solid var(--color-border);border-radius:0;background:var(--color-surface)}.stats article.focus{color:var(--color-on-primary);background:var(--color-primary);--color-muted:rgb(255 255 255 / .72);}.stats span,.stats small{display:block;color:var(--color-muted)}.stats .focus span,.stats .focus small{color:var(--color-surface-muted)}.stats strong{display:block;margin:.45rem 0;font-family:var(--font-serif);font-size:clamp(1.5rem,2.5vw,2.2rem);font-weight:500}.performance{display:grid;grid-template-columns:1fr 1fr;gap:clamp(2rem,7vw,7rem);align-items:center;margin:2rem 0;padding:2rem;border:1px solid var(--color-border);background:var(--color-primary);color:var(--color-on-primary);--color-muted:rgb(255 255 255 / .72);}.performance h2{max-width:13ch;margin:.5rem 0;font-family:var(--font-serif);font-size:clamp(2rem,4vw,3.5rem);font-weight:500;line-height:.98}.performance>div>p:last-child{color:var(--color-muted);line-height:1.65}.meter>div{height:1rem;background:var(--color-foreground);--color-muted:rgb(255 255 255 / .72);}.meter>div span{display:block;height:100%;background:var(--color-accent)}.meter p{display:flex;flex-wrap:wrap;gap:.45rem;color:var(--color-muted)}.meter b{color:var(--color-on-primary)}.card{margin-top:1rem;border:1px solid var(--color-border)}.ageing>p{color:var(--color-muted)}.card dl{display:grid;gap:.7rem}.card dl div{display:flex;justify-content:space-between;gap:2rem;border-bottom:1px solid var(--color-border);padding-bottom:.7rem}.card dd{font-weight:700}.ageing>a{display:inline-block;margin-top:1rem;font-weight:750}.enterprise-health{margin:2rem 0;border:1px solid var(--color-border);}.health-header{display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:1rem;margin-bottom:1.5rem;}.health-header h2{font-family:var(--font-serif);font-size:clamp(1.75rem,3vw,2.2rem);font-weight:500;margin:.25rem 0;}.health-badge{display:inline-block;padding:.35rem .85rem;font-size:.85rem;font-weight:750;letter-spacing:.05em;text-transform:uppercase;border-radius:2px;}.badge-excellent{background:#0d5e36;color:#ffffff;}.badge-healthy{background:#004b87;color:#ffffff;}.badge-watch{background:#b25e00;color:#ffffff;}.badge-stressed{background:#9b1c1c;color:#ffffff;}.health-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:1rem;border-top:1px solid var(--color-border);padding-top:1.25rem;}.health-grid article span{display:block;font-size:.85rem;color:var(--color-muted);}.health-grid article strong{display:block;font-family:var(--font-serif);font-size:clamp(1.4rem,2vw,1.8rem);font-weight:500;margin:.3rem 0;}.health-grid article small{display:block;font-size:.8rem;color:var(--color-muted);}.hash-code{font-family:monospace;font-size:1.1rem !important;letter-spacing:-0.05em;}.branch-section{margin:2rem 0;border:1px solid var(--color-border);}.branch-section h2{font-family:var(--font-serif);font-size:clamp(1.5rem,2.5vw,1.8rem);font-weight:500;margin:0 0 .25rem;}.branch-section p{color:var(--color-muted);margin-bottom:1.25rem;}.table-wrap{overflow-x:auto;}.branch-table{width:100%;border-collapse:collapse;text-align:left;font-size:.9rem;}.branch-table th{background:var(--color-primary);color:var(--color-on-primary);padding:.6rem .8rem;font-weight:700;}.branch-table td{padding:.7rem .8rem;border-bottom:1px solid var(--color-border);}.branch-table .total-col{font-weight:750;text-align:right;}.share-card{display:grid;grid-template-columns:1fr auto;gap:2rem;align-items:center;margin-top:2rem;background:var(--color-background)}.share-card h2,.share-card p{margin:.25rem 0}.share-card label{display:grid;gap:.3rem;width:max-content;margin-top:.8rem;font-weight:750}.share-card select{min-height:2.7rem;padding:.55rem;border:1px solid var(--color-border);background:var(--color-surface);font:inherit}.share-card>div:last-child>strong,.share-card>div:last-child>small{display:block}.share-card>div:last-child>strong{font-family:var(--font-serif);font-size:2rem;font-weight:500}.error{color:var(--color-overdue)}@media(max-width:1050px){.stats{grid-template-columns:1fr 1fr}.stats .focus{grid-column:1/-1}.health-grid{grid-template-columns:1fr 1fr;}}@media(max-width:700px){.stats,.performance{grid-template-columns:1fr}.stats .focus{grid-column:auto}.toolbar{align-items:stretch;flex-direction:column}.toolbar .actions{flex-direction:column;}.toolbar button{width:100%}.share-card{grid-template-columns:1fr}.card dl div{align-items:flex-start;flex-direction:column;gap:.25rem}.health-grid{grid-template-columns:1fr;}}
</style>
