<script lang="ts">
 ...
 async function save(){if(busy||loading||loadError||!orgID||!intent||!reviewed)return;busy=true;message='';try{profile=await intent.run({expected_version:profile.version,bank_code:bankCode,account_number:accountNumber},value=>settingsProfile(record(value).profile),'PUT');accountNumber='';reviewed=false;message='Bank account saved. We need to confirm who owns it before you can receive payments.';}catch(cause){message=publicError(cause,'The registration is unconfirmed. Check your saved account or contact support before trying again.')}finally{busy=false}}
 onMount(load);
</script>
<svelte:head><title>Bank account — Kredit</title></svelte:head>
<main class="shell workspace form-page"><p class="eyebrow">Settings / Bank account</p><h1>Receiving account</h1><p>Choose your bank and enter the account number where you want to receive payments. If your business has no CAC registration, use an account in the verified owner’s full name.</p>
...
<p>We will check the account name and confirm it belongs to your business or its verified owner. Changing this account only affects future payments.</p>
<label class="confirm"><input type="checkbox" bind:checked={reviewed} required />I checked the bank and account number. This account belongs to my business or, if it has no CAC registration, its verified owner.</label>
<button disabled={!reviewed||!bankCode||!/^\d{10}$/.test(accountNumber)}>{busy?'Saving…':'Save bank account'}</button>
</fieldset></form>{/if}
{:else}<p>Only the owner or a person with financial access can change this account.</p>{/if}</section>{/if}
<a href="/workspace/settings">All settings</a></main>
<style>.form-page{max-width:52rem}.card,fieldset{display:grid;gap:1rem}.card{padding:1.4rem;border:1px solid var(--color-border);border-radius:1rem;background:var(--color-surface)}fieldset{border:0;padding:0;min-width:0}label{display:grid;gap:.4rem}input,select,button{padding:.75rem;border:1px solid var(--color-border);border-radius:.65rem;font:inherit}button{background:var(--color-primary);color:var(--color-on-primary)}.confirm{display:flex;align-items:start}.notice{padding:1rem;background:var(--color-background);border-radius:.7rem}</style>
