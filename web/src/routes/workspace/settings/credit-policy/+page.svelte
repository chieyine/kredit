	...
	async function save() {
		...
		try {
			const result = await adminPost(
				`/api/v1/organizations/${encodeURIComponent(orgID)}/onboarding/credit-policy`,
				{
					expected_version: profile.version,
					credit_limit_kobo: parseNaira(limitNaira),
					payment_days: Number(days),
					grace_hours: Number(grace)
				},
				'PUT'
			);
			profile = settingsProfile(result.profile);
			message = 'Saved. We will use these on your next sale.';
		} catch (cause) {
...
<svelte:head><title>Normal sale settings — Kredit</title></svelte:head>
<main class="shell workspace form-page">
	<p class="eyebrow">Settings / Normal sales</p>
	<h1>Default credit terms</h1>
	<p class="lede">
		We put these in automatically each time you add a sale, so you type less. You can still change them for any
		customer.
	</p>
	{#if message}<p class="notice" role="status">{message}</p>{/if}{#if loading}<p role="status">
		...
		<section class="card">
			{#if permissions.credit_policy}<label
					...
					/></label
				><label
					>How many days you normally give<input
						disabled={busy}
						type="number"
						min="1"
						max="365"
					...
					/></label
				><label
					>Extra hours you normally give after that<input
						disabled={busy}
						type="number"
						min="0"
						max="720"
