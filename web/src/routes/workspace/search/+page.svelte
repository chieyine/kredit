<script lang="ts">
	...
	const saleResults = $derived(
		needle
			? sales
					.filter((item) =>
						[item.id, item.buyer_legal_name, item.goods_description, item.invoice_reference, item.state].some((value) =>
							String(value ?? '')
								.toLowerCase()
								.includes(needle)
						)
	...
	function decodeItem<T>(value: unknown, id: string, strings: string[]): T {
		...
		if (!text(item[id])) throw new Error('Missing record identity');
	...
	async function loadBusiness() {
		...
		try {
			...
			const [customerData, saleData, paymentData] = await Promise.all([
				checkedJSON(
					...
					rows('customers', (value) =>
						decodeItem<Customer>(value, 'buyer_user_id', [
							'buyer_business_id',
							'legal_name',
							'trading_name',
							'phone',
				...
				checkedJSON(
					...
					rows('requests', (value) => {
						saleView(value);
						return decodeItem<CreditRequest>(record(value).request, 'id', [
							'buyer_legal_name',
							'goods_description',
							'invoice_reference',
							'state'
			...
			if (request.current()) error = publicError(cause, 'your records');
		} finally {
	...
	async function initialize() {
		...
		try {
			const result = await checkedJSON(
				...
				rows('organizations', (value) => decodeItem<Organization>(value, 'id', ['legal_name', 'trading_name'])),
				{ signal: request.signal }
			);
...
<main class="shell workspace search-page">
	<header>
		...
		<h1>Search your business</h1>
		<p class="lede">
			Search names, goods, invoice numbers, Kredit numbers and transfer references, all inside your own business.
		</p>
	</header>
	<div class="search-bar">
		...
		><label class="query"
			>Search<input
				bind:value={query}
				type="search"
				placeholder="Name, invoice, sale or transfer reference"
				autocomplete="off"
			/></label
		>
	</div>
	{#if error}<p class="error" role="alert">
			{error}
			<button type="button" onclick={() => (organizations.length ? loadBusiness() : initialize())}>Try again</button>
		</p>{:else if loading}<p role="status">Opening your records…</p>{:else if !organizationID}<section class="empty">
			<strong>No business selected.</strong>
			<p>Create a business account first. Then you can search its customers, its sales and every payment.</p>
		</section>{:else if !needle}<section class="empty">
			<strong>Start typing.</strong>
			<p>You only ever search records your own account is allowed to see.</p>
		</section>{:else if count === 0}<section class="empty">
			<strong>Nothing matched that.</strong>
			<p>Try a customer name, an invoice number, the goods, a sale number or a transfer reference.</p>
		</section>{:else}
							...
							><div>
								<strong>{customer.trading_name || customer.legal_name || 'Customer'}</strong><small
									>{customer.legal_name && customer.trading_name
										? customer.legal_name
										: customer.email || customer.phone || customer.buyer_user_id}</small
								>
							</div>
					...
					{#each saleResults as sale}<a
							...
							><div>
								<strong>{sale.buyer_legal_name || 'Credit sale'}</strong><small
									>{sale.goods_description || sale.invoice_reference || sale.id}</small
								>
							</div>
							<div class="money"><b>{formatKobo(sale.principal_kobo)}</b><span>{sale.state || 'Sale'} →</span></div></a
						>{/each}
				</div>
			</section>{/if}
		{#if paymentResults.length}<section>
				...
				<div class="results">
					{#each paymentResults as payment}<a
							...
							><div>
								<strong
									>{payment.reference || payment.provider_reference || payment.external_reference || payment.id}</strong
								><small>{payment.request_id ? `Sale ${payment.request_id}` : 'Payment record'}</small>
							</div>
							<div class="money">
								<b>{formatKobo(payment.amount_kobo)}</b><span>{payment.state || 'Payment'} →</span>
							</div></a
						>{/each}
				</div>
			</section>{/if}
