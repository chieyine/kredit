...
<main class="shell workspace search-page">
	<header><p class="eyebrow">Find anything</p><h1>Search your business</h1><p class="lede">Search names, goods, invoice numbers, Kredit numbers and transfer references, all inside your own business.</p></header>
	<div class="search-bar"><label>Business<select bind:value={organizationID} onchange={()=>chooseWorkspace(organizationID)}>{#each organizations as organization}<option value={organization.id}>{organization.trading_name || organization.legal_name}</option>{/each}</select></label><label class="query">Search<input bind:value={query} type="search" placeholder="Name, invoice, sale or transfer reference" autocomplete="off" /></label></div>
	{#if error}<p class="error" role="alert">{error} <button type="button" onclick={() => organizations.length ? loadBusiness() : initialize()}>Try again</button></p>{:else if loading}<p role="status">Opening your records…</p>{:else if !organizationID}<section class="empty"><strong>No business selected.</strong><p>Create a business account first. Then you can search its customers, its sales and every payment.</p></section>{:else if !needle}<section class="empty"><strong>Start typing.</strong><p>You only ever search records your own account is allowed to see.</p></section>{:else if count === 0}<section class="empty"><strong>Nothing matched that.</strong><p>Try a customer name, an invoice number, the goods, a sale number or a transfer reference.</p></section>{:else}
		<p class="count">{count} match{count === 1 ? '' : 'es'} shown</p>
		{#if customerResults.length}<section><h2>Customers</h2><div class="results">{#each customerResults as customer}<a href={`/workspace/partners/customers/${encodeURIComponent(customer.buyer_user_id)}?organization=${encodeURIComponent(organizationID)}`}><div><strong>{customer.trading_name || customer.legal_name || 'Customer'}</strong><small>{customer.legal_name && customer.trading_name ? customer.legal_name : customer.email || customer.phone || customer.buyer_user_id}</small></div><span>Customer →</span></a>{/each}</div></section>{/if}
		{#if saleResults.length}<section><h2>Sales</h2><div class="results">{#each saleResults as sale}<a href={`/workspace/sales/${encodeURIComponent(sale.id)}?organization=${encodeURIComponent(organizationID)}`}><div><strong>{sale.buyer_legal_name || 'Credit sale'}</strong><small>{sale.goods_description || sale.invoice_reference || sale.id}</small></div><div class="money"><b>{formatKobo(sale.principal_kobo)}</b><span>{sale.state || 'Sale'} →</span></div></a>{/each}</div></section>{/if}
		{#if paymentResults.length}<section><h2>Payments</h2><div class="results">{#each paymentResults as payment}<a href={`/workspace/money/received?organization=${organizationID}&q=${encodeURIComponent(payment.reference || payment.provider_reference || payment.external_reference || payment.id)}`}><div><strong>{payment.reference || payment.provider_reference || payment.external_reference || payment.id}</strong><small>{payment.request_id ? `Sale ${payment.request_id}` : 'Payment record'}</small></div><div class="money"><b>{formatKobo(payment.amount_kobo)}</b><span>{payment.state || 'Payment'} →</span></div></a>{/each}</div></section>{/if}
	{/if}
</main>

<style>
	.search-page header{max-width:55rem;padding:2.5rem 0 2rem;border-bottom:3px solid var(--color-primary)}.search-page h1{margin:.5rem 0;font-family:var(--font-serif);font-size:clamp(3rem,7vw,5.5rem);font-weight:500;line-height:.92;letter-spacing:-.055em}.search-bar{display:grid;grid-template-columns:minmax(12rem,20rem) 1fr;gap:1rem;margin:2rem 0;padding:1rem;background:var(--color-primary)}.search-bar label{display:grid;gap:.35rem;color:var(--color-on-primary);font-size:.78rem;font-weight:800}.search-bar input,.search-bar select{box-sizing:border-box;width:100%;min-height:3rem;padding:.75rem;border:0;background:var(--color-surface);color:var(--color-primary);font:inherit}.empty{padding:2rem;border:1px solid var(--color-border);background:var(--color-surface)}.empty strong{font-family:var(--font-serif);font-size:1.8rem;font-weight:500}.empty p{color:var(--color-muted)}.count{font-weight:800}.search-page section>h2{margin:2rem 0 .75rem;font-family:var(--font-serif);font-size:1.7rem;font-weight:500}.results{border-top:1px solid var(--color-border)}.results a{display:flex;justify-content:space-between;gap:1.5rem;padding:1rem .25rem;border-bottom:1px solid var(--color-border);color:inherit;text-decoration:none}.results a:hover{background:var(--color-surface)}.results div{display:grid;gap:.3rem}.results small{color:var(--color-muted)}.results span{color:var(--color-primary);font-size:.78rem;font-weight:800;text-transform:capitalize}.money{text-align:right}.money b{font-family:var(--font-serif);font-size:1.2rem;font-weight:500}.error{color:var(--color-overdue)}@media(max-width:680px){.search-bar{grid-template-columns:1fr}.results a{align-items:flex-start;flex-direction:column}.money{text-align:left}}
</style>
