 ...
 import { chooseWorkspace, requestedWorkspace } from '$lib/workspace-context';
  ...
  async function confirmDecision() {
    ...
    try {
      ...
      await intent.run({ decision: selected.decision, reason: selected.decision === 'confirmed' ? 'Supplier checked the receiving account and confirmed the transfer.' : 'Supplier checked the receiving account and could not locate the transfer.' }, value => { const updated = claimRow(record(value).payment_claim); if (updated.id !== selected.claim.id || updated.state !== selected.decision) throw new Error('Payment decision not confirmed'); return updated; });
      review = null;
  ...
  async function start() {
    ...
    try {
      ...
      const requested = params.get('organization'); query = params.get('q') ?? '';
...
<main class="shell workspace payments-page">
	<header class="page-heading"><div><p class="eyebrow">Payments</p><h1>Payments received</h1><p class="lede">See what has actually entered your account, and check the payments customers say they made.</p></div>{#if organizations.length > 1}<label>Business<select bind:value={organizationID} disabled={!!busy || !!review} onchange={()=>chooseWorkspace(organizationID)}>{#each organizations as organization}<option value={organization.id}>{organization.trading_name || organization.legal_name}</option>{/each}</select></label>{/if}</header>
	...
	{#if loading}<div class="loading" role="status"><span class="sr-only">Opening your payments</span><Skeleton rows={5} tall /></div>{:else if !error && !organizationID}<section class="empty-state"><h2>Add your business first</h2><a href="/workspace/today">Add business details</a></section>{:else if !error}
		...
		{#if visiblePayments.length}<div class="payment-table" role="table" aria-label="Payments received"><div class="table-head" role="row"><span role="columnheader">Customer</span><span role="columnheader">Amount</span><span role="columnheader">How</span><span role="columnheader">Date</span><span role="columnheader">Status</span><span aria-hidden="true"></span></div>{#each visiblePayments as payment}<div class="payment-row" role="row"><div role="cell"><strong>{payment.buyer_legal_name || 'Customer'}</strong><small>{payment.description || payment.reference || 'Sale payment'}</small></div><div role="cell"><strong><Money amountKobo={payment.amount_kobo} /></strong></div><span role="cell">{source(payment.source_type)}</span><span role="cell">{date(payment.paid_at)}</span><span role="cell" class:reversed={payment.state === 'reversed'} class="payment-state">{stateLabel(payment.state)}</span><a role="cell" href={`/workspace/sales/${encodeURIComponent(payment.id)}?organization=${encodeURIComponent(organizationID)}`}>Open sale →</a></div>{/each}</div>
		{:else if payments.length}<div class="empty-history"><h3>No payment matches that.</h3><p>Try a different customer name, transfer number or status.</p></div>{:else}<div class="empty-history"><span aria-hidden="true">₦</span><h3>No confirmed payments yet.</h3><p>Verified payments appear here. A reported transfer stays separate until it is confirmed.</p><a class="primary" href="/workspace/sales/new">Add a sale</a></div>{/if}</section>
	{/if}
</main>
...
<style>
	.payments-page{max-width:76rem;padding-bottom:5rem}.page-heading{display:flex;justify-content:space-between;align-items:end;gap:2rem;padding-bottom:2.2rem;border-bottom:3px solid var(--color-primary)}.page-heading>div{max-width:48rem}.page-heading h1{max-width:11ch;margin:.45rem 0;font-family:var(--font-serif);font-size:clamp(3.2rem,7vw,6rem);font-weight:500;line-height:.9;letter-spacing:-.06em}.page-heading label,.filters label{display:grid;gap:.35rem;font-size:.82rem;font-weight:750}.page-heading select,.filters input,.filters select{box-sizing:border-box;min-height:3rem;padding:.65rem .75rem;border:1px solid var(--color-border);border-radius:0;background:var(--color-surface);color:var(--color-primary);font:inherit}.error-box{display:flex;justify-content:space-between;gap:1rem;align-items:center;margin:1.5rem 0;padding:1rem;color:var(--color-overdue);background:var(--color-surface-muted);border-left:2px solid var(--color-overdue)}.error-box p{margin:.25rem 0}.error-box button{padding:.65rem 1rem;border:1px solid var(--color-overdue);border-radius:0;background:transparent;color:var(--color-overdue);font-weight:700}.loading{padding:2rem 0}.money-summary{display:grid;grid-template-columns:1.4fr 1fr 1fr;margin:2rem 0 4rem;border-top:1px solid var(--color-border);border-left:1px solid var(--color-border)}.money-summary article{display:grid;align-content:start;min-height:9rem;padding:1.35rem;border-right:1px solid var(--color-border);border-bottom:1px solid var(--color-border);background:var(--color-surface)}.money-summary article.total{color:var(--color-on-primary);background:var(--color-primary);--color-muted:rgb(255 255 255 / .72);}.money-summary article.needs-action{box-shadow:inset 0 .35rem var(--color-accent)}.money-summary span,.money-summary small{font-size:.82rem}.money-summary strong{margin:.6rem 0;font-family:var(--font-serif);font-size:clamp(2rem,4vw,3.3rem);font-weight:500;letter-spacing:-.04em}.money-summary .total>span,.money-summary .total strong,.money-summary .total strong :global(span){color:var(--color-on-primary)}.money-summary .total small{color:var(--color-surface-muted)}.review-section{margin-bottom:5rem}.review-section>header,.history>header{display:flex;justify-content:space-between;align-items:end;gap:2rem;padding-bottom:1rem;border-bottom:1px solid var(--color-border)}.review-section h2,.history h2{margin:.2rem 0;font-family:var(--font-serif);font-size:clamp(2rem,4vw,3.4rem);font-weight:500;letter-spacing:-.04em}.review-section>header>span{display:grid;place-items:center;width:2.6rem;height:2.6rem;background:var(--color-accent);color:var(--color-on-primary);font-weight:850}.claim-list{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:1rem;margin-top:1rem}.claim-list article{padding:1.4rem;border:1px solid var(--color-border);background:var(--color-surface);box-shadow:6px 6px 0 var(--color-border)}.claim-amount{display:flex;justify-content:space-between;gap:1rem;padding-bottom:1rem;border-bottom:2px solid var(--color-primary)}.claim-amount span{max-width:10rem}.claim-amount strong{font-family:var(--font-serif);font-size:1.7rem;font-weight:500}.claim-list dl{display:grid;gap:.6rem}.claim-list dl div{display:flex;justify-content:space-between;gap:1rem}.claim-list dt{color:var(--color-muted)}.claim-list dd{margin:0;text-align:right;font-weight:700}.claim-list>article>p{padding:.7rem;background:var(--color-background)}.claim-actions{display:grid;grid-template-columns:1fr 1fr;gap:.6rem}.claim-actions button{min-height:3rem;padding:.65rem;border:1px solid var(--color-primary);border-radius:0;background:var(--color-primary);color:var(--color-on-primary);font:inherit;font-weight:750;--color-muted:rgb(255 255 255 / .72);}.claim-actions .secondary{border-color:var(--color-primary);background:var(--color-surface);color:var(--color-primary)}.all-clear{display:flex;gap:1rem;align-items:center;padding:2rem 0}.all-clear>span{display:grid;place-items:center;width:3.2rem;height:3.2rem;background:var(--color-background);color:var(--color-positive);font-size:1.4rem;font-weight:900}.all-clear h3,.all-clear p{margin:.2rem 0}.filters{display:flex;gap:.7rem}.filters label:first-child{min-width:min(19rem,50vw)}.payment-table{margin-top:1rem;border-top:1px solid var(--color-border)}.table-head,.payment-row{display:grid;grid-template-columns:minmax(11rem,1.5fr) minmax(8rem,.8fr) minmax(9rem,1fr) minmax(7rem,.8fr) minmax(6rem,.7fr) auto;gap:1rem;align-items:center;padding:.85rem;border-right:1px solid var(--color-border);border-bottom:1px solid var(--color-border);border-left:1px solid var(--color-border)}.table-head{background:var(--color-primary);color:var(--color-on-primary);font-size:.75rem;font-weight:750;text-transform:uppercase;letter-spacing:.06em;--color-muted:rgb(255 255 255 / .72);}.payment-row{background:var(--color-surface)}.payment-row>div{display:grid;gap:.2rem}.payment-row small{color:var(--color-muted)}.payment-row a{color:var(--color-primary);font-weight:750;text-decoration:none;white-space:nowrap}.payment-state{width:max-content;padding:.3rem .5rem;background:var(--color-background);color:var(--color-positive);font-size:.78rem;font-weight:800}.payment-state.reversed{background:var(--color-background);color:var(--color-overdue)}.empty-history{margin-top:1rem;padding:clamp(2rem,6vw,4rem);border:1px solid var(--color-border);background:var(--color-background)}.empty-history>span{font-family:var(--font-serif);font-size:3rem;color:var(--color-primary)}.empty-history h3{margin:.5rem 0;font-family:var(--font-serif);font-size:2rem;font-weight:500}.empty-history p{max-width:34rem;color:var(--color-muted)}@media(max-width:800px){.page-heading,.review-section>header,.history>header{display:block}.page-heading>label{margin-top:1rem}.money-summary{grid-template-columns:1fr}.claim-list{grid-template-columns:1fr}.filters{display:grid;margin-top:1rem}.filters label:first-child{min-width:0}.payment-table{border:0}.table-head{display:none}.payment-row{grid-template-columns:1fr auto;gap:.65rem}.payment-row>[role='cell']{grid-column:1}.payment-row>[role='cell']:nth-child(2){grid-column:2;grid-row:1}.payment-row>[role='cell']:nth-child(3),.payment-row>[role='cell']:nth-child(4){display:inline}.payment-row>[role='cell']:nth-child(5){grid-column:1}.payment-row>a[role='cell']{grid-column:2;grid-row:3}.claim-actions{grid-template-columns:1fr}.error-box{align-items:stretch;flex-direction:column}.error-box button{width:100%}}
	/* The most important number uses the strongest contrast in the product. */
	.money-summary article.total{color:var(--color-on-primary);background:var(--color-primary);box-shadow:inset 0 .4rem var(--color-accent);--color-muted:rgb(255 255 255 / .72);}
	.money-summary .total>span,.money-summary .total strong,.money-summary .total strong :global(span){color:var(--color-on-primary)!important}
	.money-summary .total small{color:var(--color-muted)}
</style>
