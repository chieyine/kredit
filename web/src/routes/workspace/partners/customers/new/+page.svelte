<script lang="ts">
 import { requestedWorkspace } from '$lib/workspace-context';
	...
	async function load(){
		...
		try{const items=await checkedJSON('/api/v1/organizations',rows('organizations',value=>{const row=record(value);if(!text(row.id))throw new Error('Missing business');text(row.legal_name);return row;}),{signal:request.signal});if(request.current()){organizations=items;const requested=new URLSearchParams(location.search).get('organization');organizationID=requestedWorkspace(items as {id:string}[]);}}
