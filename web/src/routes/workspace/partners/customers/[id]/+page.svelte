<script lang="ts">
 ...
 let businessName=$state(''),buyerUserID=$state('');
 ...
 async function loadCustomer(customerID=page.params.id,selected=page.url.searchParams.get('organization')){
  const request=requests.begin();error='';history=null;statement=null;organizationID='';businessName='';buyerUserID='';note='';noteMessage='';
  try{
   ...
   const customers=await checkedJSON(`/api/v1/organizations/${encodeURIComponent(org.id)}/customers`,rows('customers',value=>{const item=record(value);text(item.buyer_user_id);text(item.buyer_business_id);text(item.legal_name);return item}),{signal:request.signal});
   ...
   const [h,statementResult]=await Promise.all([checkedJSON(`${base}/history${query}`,decodeHistory,{signal:request.signal}),checkedJSON(`${base}/statement${query}`,decodeStatement,{signal:request.signal})]);
   if(!request.current())return;
   if(statementResult.buyer_id!==customer.buyer_user_id)throw new Error('Wrong customer statement');
   organizationID=org.id;buyerUserID=String(customer.buyer_user_id);businessName=String(customer.trading_name||customer.legal_name);history=h;statement=statementResult;
 ...
 function customerName(){return businessName||'Customer relationship';}
 function saveNote(){
