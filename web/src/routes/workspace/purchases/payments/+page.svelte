	...
	async function load() {
		...
		try {
			const result = await checkedJSON(
				buyerEndpoint('/api/v1/buyer/payment-claims'),
...
<main class="shell workspace reported">
	<p class="eyebrow">Transfers I reported</p>
	<h1>Reported supplier payments</h1>
	<p class="lede">
		A seller has to see the money in their bank before your balance can drop. That is what protects both of you.
	</p>
	{#if loading}<p role="status">Opening your transfers…</p>{:else if error}<section role="alert">
			...
			{#each claims as claim (claim.id)}<article>
					...
					<dl>
						...
						<div>
							<dt>Payment day</dt>
							<dd>{readableDate(claim.paid_at)}</dd>
						</div>
						{#if claim.review_reason}<div>
								<dt>What the seller said</dt>
								<dd>{claim.review_reason}</dd>
							</div>{/if}
					</dl>
					{#if claim.state === 'pending'}<p>
							Awaiting seller review. Temporary collection-pause deadline: {readableDateTime(claim.hold_expires_at)}.
							After this deadline, collection may resume.
						</p>{/if}
				</article>{/each}
		</section>{:else}<section class="empty-state">
			<h2>You have not reported any transfer</h2>
			<p>When you tell a seller you have sent money, you can follow their answer here.</p>
			<a href={workspaceHref('/workspace/purchases/obligations', page.url)}>See what I owe →</a>
		</section>{/if}
</main>
