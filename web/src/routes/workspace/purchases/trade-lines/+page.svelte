<script lang="ts">
 ...
 import {feeDisclosure,validFeeTerms} from "$lib/fee-terms";
	...
	async function load() {
		...
		try {
			const lines = await checkedJSON(buyerEndpoint('/api/v1/buyer/trade-lines'), rows('trade_lines', tradeLine), {signal: read.signal});
			const result = [];
...
<main class="shell workspace">
	<p class="eyebrow">Your limits</p><h1>Your buying limits</h1><p class="lede">Read every sale before the seller sends anything. Payment only starts after you confirm the goods reached you.</p>
	...
	{#if loading}<p role="status">Opening your customer limits…</p>{:else if statements.length}{#each statements as statement}<section class="line">
		...
		{#if statement.drawdowns.length}<div class="drawdowns">{#each statement.drawdowns as drawdown}<article>
			...
			{#if drawdown.state === 'RECEIPT_ISSUE_REPORTED'}<div class="receipt"><p>Only confirm after the seller has corrected the problem and you have received the agreed goods. If the goods were returned, ask the seller to accept the return and cancel this purchase.</p><button class="primary" disabled={busy !== '' || loading} onclick={() => command(`/api/v1/buyer/trade-lines/${encodeURIComponent(statement.line.id)}/drawdowns/${encodeURIComponent(drawdown.id)}/receipt`, { state: 'no_issue' }, drawdown.id)}>The problem is resolved; I received the goods</button></div>{/if}
			{#if drawdown.receipt_state === 'issue_reported'}<p class="error">Problem reported: {drawdown.receipt_issue_reason}</p>{/if}{#if drawdown.obligation_id}<a href={`/workspace/purchases/obligations/${encodeURIComponent(drawdown.obligation_id)}`}>Open payment details →</a>{/if}
