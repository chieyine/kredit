<script lang="ts">
 ...
 async function load() {
  ...
  try {
   const result = await checkedJSON(buyerEndpoint(`/api/v1/buyer/amendments?offset=${offset}`), rows('changes', changeRecord), {signal: read.signal});
