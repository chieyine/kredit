<script lang="ts">
	...
	function changeRecord(value: unknown): DateChange {
		...
		if (
			!Number.isFinite(Date.parse(String(change.expires_at))) ||
		...
		for (const value of change.items) {
			const item = record(value),
				id = text(item.id);
			if (!id || items.has(id) || !Number.isFinite(Date.parse(text(item.due_at))))
				throw new Error('Invalid original payment day');
		...
		const dates = change.dates.map((value): ProposedDay => {
			const date = record(value),
				...
				item = items.get(id);
			if (!item || seen.has(id) || !Number.isFinite(Date.parse(text(date.due_at))))
				throw new Error('Invalid proposed payment day');
			seen.add(id);
	...
	async function load() {
		...
		try {
			const result = await checkedJSON(
				buyerEndpoint(`/api/v1/buyer/amendments?offset=${offset}`),
				rows('changes', changeRecord),
				{ signal: read.signal }
			);
			if (read.current()) {
				changes = result.slice(0, 100);
				more = result.length > 100;
			}
		} catch (cause) {
			if (read.current()) error = publicError(cause, 'your proposed payment dates');
		} finally {
	...
	async function decide(change: DateChange, action: 'accept' | 'reject') {
		...
		try {
			...
			await intents.get(path)!.run(
				{
					action,
					reason:
						action === 'accept' ? 'I accept the exact dates shown on this page.' : 'I do not accept the new dates.'
				},
				(value) => {
					if (record(value).recorded !== true) throw new Error('Decision not confirmed');
					return true;
				}
			);
			message =
				action === 'accept'
					...
					: 'Your rejection was confirmed. The old payment days still stand.';
			await load();
		} catch (cause) {
			error = cause instanceof Error ? cause.message : 'We could not confirm your decision.';
		} finally {
			busy = false;
		}
	}
...
<svelte:head><title>Payment date changes — Kredit</title></svelte:head>
<main class="shell workspace">
	<h1>Payment date changes</h1>
	<VerifyIdentity />
	<p>
		Read every new date before you decide. Until you accept, your old payment days still stand. The money you owe and
		the agreed fee do not change.
	</p>
	{#if error}<p role="alert">{error}</p>{/if}{#if message}<p role="status">
			...
			{#if c.state === 'awaiting_buyer'}<label
					><input type="checkbox" bind:checked={consents[c.id]} disabled={busy || loading} /> I have read these exact dates
					and I agree to them.</label
				><button disabled={busy || loading || !consents[c.id]} onclick={() => decide(c, 'accept')}
					>Yes, I accept these dates</button
				><button disabled={busy || loading} onclick={() => decide(c, 'reject')}>No, keep my old dates</button>{/if}
		</article>{:else}<p>
			{loading
				? 'Loading date changes…'
				: error
