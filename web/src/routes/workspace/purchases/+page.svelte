 ...
 async function load(query=businessQuery){
  ...
  try{
   const [account,sales,days]=await Promise.allSettled([
    ...
    checkedJSON(buyerEndpoint('/api/v1/buyer/credit-requests'),rows('requests',saleView),{signal:request.signal}),
   ...
   if(sales.status==='fulfilled'){requests=sales.value.filter(item=>item.request?.buyer_business_id===portal.business.id);balancesUnavailable=false; if(days.status==='fulfilled'){const ids=new Set(requests.map(item=>item.obligation?.id).filter(Boolean));paymentDays=days.value.filter(item=>ids.has(item.obligation_id));datesUnavailable=false;}}
...
<main class="shell buyer-home">
	...
	{#if portal}
		<header class="buyer-head"><div><p class="eyebrow">Your business · Purchases</p><h1>Supplier purchases</h1><p>Signed in as <strong>{portal.person.full_name}</strong> for {portal.business.legal_name}.</p></div>{#if pending.length}<a class="primary" href={workspaceHref("/workspace/purchases/orders",page.url)}>Read {pending.length} sale{pending.length===1?'':'s'} waiting for you</a>{/if}</header>
        ...
        {#if error}<p role="alert">{error}</p><button onclick={()=>void load()} disabled={loading}>Try again</button>{/if}{#if loading}<p role="status">Opening your balances…</p>{:else}
			<section class="owe-hero" aria-label="What you owe">
				...
				{#if outstanding===null||datesUnavailable}<p class="due-strip calm">Payment dates could not be confirmed.</p>{:else if overdueCount}<a class="due-strip late" href={workspaceHref("/workspace/purchases/obligations",page.url)}><span>{overdueCount} payment day{overdueCount===1?' has':'s have'} passed</span><em>See what is late →</em></a>{:else if nextPayment}<a class="due-strip" href={workspaceHref("/workspace/purchases/obligations",page.url)}><span>Next: {formatKobo(nextPayment.next_due_kobo)}</span><em>by {new Date(nextPayment.next_due_at).toLocaleDateString('en-NG',{timeZone:'Africa/Lagos'})} →</em></a>{:else if outstanding>0n}<a class="due-strip" href={workspaceHref("/workspace/purchases/obligations",page.url)}>Open your sales to check payment days →</a>{:else}<p class="due-strip calm">Nothing is due right now.</p>{/if}
			</section>
			<section class="next-actions">
				<div><p class="eyebrow">What needs you</p><h2>{pending.length ? 'Review your new purchase offers.' : overdueCount ? 'A payment day has passed.' : outstanding === null ? 'We could not confirm your total.' : outstanding > 0n ? 'See your payment schedule.' : 'No outstanding supplier balance.'}</h2></div>
				<div class="action-copy">{#if pending.length}<p>Your supplier has proposed a purchase. Review the goods, total and payment dates before accepting.</p><a href={workspaceHref("/workspace/purchases/orders",page.url)}>Read the sale →</a>{:else if overdueCount}<p>Open your balances and see what is late, what you have already paid and what is still left.</p><a href={workspaceHref("/workspace/purchases/obligations",page.url)}>See what is late →</a>{:else if outstanding === null}<p>We could not confirm one or more of your balances. Open each sale instead of trusting the total above.</p><a href={workspaceHref("/workspace/purchases/obligations",page.url)}>Check my balances →</a>{:else if outstanding > 0n}<p>You can open your balances any time and see what you agreed to, what you have paid and what is left.</p><a href={workspaceHref("/workspace/purchases/obligations",page.url)}>See my balances →</a>{:else}<p>If a seller sends you a new sale, it will show up right here.</p><a href={workspaceHref("/workspace/purchases/history",page.url)}>View payment history →</a>{/if}</div>
			</section>
		{/if}
...
<style>.owe-hero{margin:1.5rem 0}.owe-hero .owed{background:var(--color-primary);color:var(--color-on-primary);padding:1.6rem 1.4rem 1.5rem;--color-muted:rgb(255 255 255 / .72);}.owe-hero .owed span{display:block;font-size:.7rem;font-weight:800;letter-spacing:.1em;text-transform:uppercase;color:var(--color-muted)}.owe-hero .owed strong{display:block;font-family:var(--font-serif);font-size:clamp(2.6rem,7vw,4rem);font-weight:500;line-height:1;margin:.5rem 0 .4rem;letter-spacing:-.03em;font-variant-numeric:tabular-nums}.owe-hero .owed small{color:var(--color-muted);font-size:.82rem}.due-strip{display:flex;align-items:center;justify-content:space-between;gap:1rem;margin:0;padding:.85rem 1.4rem;background:var(--color-background);color:var(--color-primary);font-weight:750;font-size:.9rem;text-decoration:none}.due-strip em{font-style:normal;font-variant-numeric:tabular-nums}.due-strip.late{background:var(--color-overdue);color:var(--color-on-primary)}.due-strip.calm{color:var(--color-foreground);background:var(--color-surface-muted)}.buyer-home{padding-bottom:6rem}.eyebrow{color:var(--color-primary);font-weight:800;text-transform:uppercase;letter-spacing:.1em;font-size:.72rem}.buyer-head{display:flex;justify-content:space-between;align-items:end;gap:2rem;padding:3rem 0 2rem;border-bottom:3px solid var(--color-primary)}.buyer-head h1,.buyer-home>h1{max-width:13ch;margin:.5rem 0;font-family:var(--font-serif);font-size:clamp(3rem,7vw,5.8rem);font-weight:500;line-height:.92;letter-spacing:-.06em}.buyer-head p{color:var(--color-foreground)}.next-actions{display:grid;grid-template-columns:1fr 1fr;gap:clamp(2rem,8vw,8rem);margin:3rem 0;padding:2rem 0;border-top:1px solid var(--color-border);border-bottom:1px solid var(--color-border)}.next-actions h2{max-width:12ch;margin:.5rem 0;font-family:var(--font-serif);font-size:clamp(2.2rem,4vw,3.8rem);font-weight:500;line-height:.96;letter-spacing:-.04em}.action-copy{padding-top:.5rem}.action-copy p{max-width:36rem;color:var(--color-foreground);line-height:1.7}.action-copy a{color:var(--color-primary);font-weight:800}.error{color:var(--color-overdue)}.help{max-width:34rem;color:var(--color-muted);line-height:1.65}@media(max-width:900px){}@media(max-width:720px){.buyer-head{display:block}.buyer-head .primary{margin-top:1rem}.next-actions{grid-template-columns:1fr}.next-actions{gap:1rem}}
</style>
