<script lang="ts">
 ...
 import SaleCosts from "$lib/components/SaleCosts.svelte";
  ...
  const allowed = (action:string) => view?.purchasing_actions ? view.purchasing_actions.includes(action) : Boolean(view?.request.buyer_user_id && view.request.buyer_user_id===account.userID);
  const mayAccept = $derived(allowed('accept') && !!view && ['SENT','BUYER_REVIEWING'].includes(view.request.state) && view.agreement?.id && /^[0-9a-f]{64}$/i.test(view.agreement.document_hash) && validFeeTerms(view.request.fee_terms) && !!view.request.supplier_legal_name.trim() && !!view.request.buyer_legal_name.trim() && !!view.request.goods_description.trim() && dateLabel(view.request.due_date) !== 'Date unavailable' && timeLabel(view.request.collection_at) !== 'Time unavailable');
  ...
  const blocked = (action: string) => Boolean(busy || (uncertain && uncertain !== action) || !allowed(({accept:'accept',decline:'review',receipt:'receive',mandate:'bank','payment-claims':'payments','payment-link':'payments',disputes:'disputes'} as Record<string,string>)[action] ?? 'read'));
  ...
  async function perform<T>(action: string, payload: unknown, decode: Decoder<T>, success: (result: T) => void) {
    ...
    const scope=requestID;busy = action; actionError = ''; message = '';
    try {
      const result = await intent(action).run(payload, decode);
      if(requestID!==scope)return;uncertain = ''; success(result); await load();
    } catch (cause) {
...
<main class="shell buyer-sale">
  ...
  {#if view}
<SaleProgress view={view} audience="buyer"/>{#if view.request.buyer_user_id!==account.userID}<p class="inline-notice">You are acting for this business. Your current purchasing permissions and acceptance limit control the actions below. Bank authorizations and payment-management actions remain with the purchasing owner.</p>{/if}    {#if view.request.system_acceptance_id}<p class="inline-notice" role="status">The waiting period after your delivery notice ended without a response. Kredit activated this sale under its agreed terms using a system record. This is not recorded as your receipt confirmation. You can still report a problem below.</p>{/if}
    ...
    {#if ['SENT','BUYER_REVIEWING'].includes(view.request.state)}
      <SaleCosts view={view}/><section class="consent-panel"><h2>Your decision</h2><p><a href={`/workspace/purchases?business_id=${encodeURIComponent(view.request.buyer_business_id)}`}>Review identity checks for this business</a></p><p>Accepting records your agreement to this sale. Bank-debit permission is a separate step. Ask the seller to correct anything that is wrong before you accept.</p>
    ...
    {#if accepted && !['PAID', 'COMPLETED'].includes(view.request.state)}
      <section class="bank-panel" aria-labelledby="bank-heading"><h2 id="bank-heading">Bank-debit permission</h2>
        ...
        {:else}<p>The sale is accepted, but bank permission is not ready. The seller must not release goods until Kredit confirms readiness.</p><p>{view.mandate ? `Permission status: ${productLabel(view.mandate.status)}` : 'No bank permission is attached yet.'}</p>
          {#if bankURL}<p>Review your bank account, permission limit and duration before completing authorization.</p><a class="primary" href={bankURL} rel="noreferrer noopener" onclick={rememberReturn}>Continue to bank authorization <span aria-hidden="true">↗</span></a><p class="field-help">Returning from the provider is not confirmation. Use “Check permission status” after completing that step.</p>
    ...
    {#if view.obligation}
      <section class="payments-panel"><div class="section-heading"><h2>Payments on this sale</h2><a href={`/workspace/purchases/obligations/${encodeURIComponent(view.obligation.id)}`}>View schedule</a></div><p class="balance-label">Left to pay</p><p class="outstanding"><Money amountKobo={view.obligation.outstanding_kobo} /></p>
