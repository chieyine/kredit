<script lang="ts">
 ...
 async function initialize() {
  ...
  try {
   ...
   organizations = result; organizationID = requestedWorkspace(result); await loadCases();
...
<main class="shell workspace help"><p class="eyebrow">Get help</p><h1>Help and support</h1><p class="lede">Tell us what happened in your own words. A real person will look at your Kredit records and reply to you safely.</p>
...
<style>.help{max-width:54rem}.help h1{font-family:var(--font-serif);font-size:clamp(3rem,7vw,5rem);line-height:.95}.card,form,label,.cases{display:grid;gap:.75rem}textarea,select{box-sizing:border-box;width:100%;padding:.8rem;border:1px solid var(--color-border);font:inherit}.cases article{display:grid;grid-template-columns:1fr auto;gap:.35rem;padding:.9rem 0;border-bottom:1px solid var(--color-border)}.cases small{grid-column:1/-1}.cases button{grid-column:1/-1;width:max-content;padding:.55rem .7rem;border:1px solid var(--color-border);background:var(--color-surface);font:inherit;font-weight:750}.notice{padding:1rem;border-left:4px solid var(--color-positive);background:var(--color-background)}.error{color:var(--color-destructive)}</style>
