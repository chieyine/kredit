<script lang="ts">
	...
	const stages = [
		...
		{ role: 'Customer', title: 'Set up bank permission', action: 'Complete sample bank permission' },
		...
		<div class="shell experience-shell">
			...
			<div class="demo-stage">
				...
				<div class="stage-copy">
					...
					{:else if stage === 3}<p>The seller marks when the goods left, and keeps the delivery note with the sale.</p>
					{:else if stage === 4}<p>The customer confirms what arrived. If something is wrong, they can say so before paying.</p>
					{:else if stage === 5}<p>When money lands, the seller enters it once. The balance changes for both sides at the same time.</p>
					{:else}<p>Seller and customer now give the same answer about the goods, the payment and the money left. The shared record helps them check any disagreement.</p>{/if}
				</div>
				...
				<div class="stage-footer">{#if stage > 0}<button class="restart" onclick={restart}>Start again</button>{/if}{#if stage === 6}<a class="start-real" href="/app">Now add my first real sale <span>↗</span></a>{:else}<span>Your choices stay on this device.</span>{/if}</div>
			</div>
		</div>
	</section>
