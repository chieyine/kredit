	...
	const urls = [
		...fixed.map(
			({ path, priority, changeFrequency }) =>
				`<url><loc>${SITE_URL}${path}</loc><changefreq>${changeFrequency}</changefreq><priority>${priority}</priority></url>`
		),
		...
		...articles.map(
			(article) =>
				`<url><loc>${SITE_URL}/blog/${article.slug}</loc><lastmod>${article.modified}</lastmod><changefreq>monthly</changefreq><priority>0.7</priority></url>`
		)
	].join('');
	return new Response(
