<script lang="ts">
	...
	async function load() {
		...
		try {
			const result = await checkedJSON(
				'/api/v1/buyer/relationships/consents',
				(value) => {
					...
					if (new Set(suppliers.map((item) => item.id)).size !== suppliers.length)
						throw new Error('Duplicate seller directory');
					return { permissions, suppliers };
				},
				{ signal: read.signal }
			);
			if (read.current()) {
				sellers = result.suppliers;
				consents = result.permissions;
			}
		} catch (cause) {
			if (read.current()) error = publicError(cause, 'your seller permissions');
		} finally {
	...
	async function save(seller: Seller, granted: boolean) {
		...
		try {
			...
			if (!intents.has(seller.id))
				intents.set(
					seller.id,
					new MutationIntent(
						`${account.userID}:buyer-seller-permission:${seller.id}`,
						'/api/v1/buyer/relationships/consents'
					)
				);
			await intents.get(seller.id)!.run(
				{
					supplier_organization_id: seller.id,
				...
				(value) => {
					...
					if (
						consent.supplier_organization_id !== seller.id ||
						consent.consent_type !== 'payment_reminders' ||
						consent.granted !== granted
					)
						throw new Error('Permission not confirmed');
					return consent;
				}
			);
			notice = granted
				? `${seller.name} may send payment reminders.`
				: `${seller.name} may no longer send optional payment reminders.`;
			await load();
		} catch (cause) {
			notice = cause instanceof Error ? cause.message : 'We could not confirm this permission.';
		} finally {
			busy = '';
		}
	}
...
<main class="shell workspace permissions">
	<p class="eyebrow">Seller permissions</p>
	<h1>Supplier reminder preferences</h1>
	<p class="lede">
		This only covers extra reminders a seller may want to send. Important sale, payment and safety messages always come
		from Kredit, and you cannot switch those off.
	</p>
			...
			{#each sellers as seller (seller.id)}{@const allowed = current(seller.id)?.granted === true}
				<article>
					...
					<button disabled={busy !== ''} onclick={() => save(seller, !allowed)}
						>{busy === seller.id ? 'Saving…' : allowed ? 'Stop optional reminders' : 'Allow payment reminders'}</button
					>
				</article>{/each}
		</section>{:else}<section class="empty-state">
			<h2>No sellers yet</h2>
			<p>This list fills up once you start buying from a seller on Kredit.</p>
		</section>{/if}
</main>
...
<style>
	...
	.permissions article div,
	.permissions article span {
		display: block;
	}
	.permissions article span {
		margin-top: 0.3rem;
