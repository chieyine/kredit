<script lang="ts">
	...
	let enabled = $state(false),
		message = $state('');
	onMount(() => {
		enabled = readLocal('kredit:low-data', false);
	});
	function save() {
		setLowData(enabled);
		message = enabled ? 'Effects are now reduced on this device.' : 'Done. The normal display is back on this phone.';
	}
</script>

<svelte:head><title>Data use — Kredit</title></svelte:head>
<main class="shell workspace data-page">
	<a href="/workspace/settings">← Settings</a>
	<p class="eyebrow">Data use</p>
	<h1>Display and data usage</h1>
	<p class="lede">
		Turn this on if your network is slow, or data is expensive. Your sales and records stay exactly the same. Only the
		display gets lighter.
	</p>
	<section class="card">
		<label
			><input type="checkbox" bind:checked={enabled} /><span
				><strong>Use less data on this phone</strong><small
					>Reduces movement and visual effects; your records stay the same.</small
				></span
			></label
