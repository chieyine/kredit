<script lang="ts">
	...
	async function load() {
		...
		loadError = '';
		try {
			enrolled = await checkedJSON('/api/v1/me', (value) => {
				const body = record(value);
				if (typeof body.mfa_enrolled !== 'boolean') throw new Error('Missing safety status');
				return body.mfa_enrolled;
			});
		} catch (cause) {
			loadError = publicError(cause, 'your sign-in safety');
		} finally {
			loading = false;
		}
	}
	async function beginEnrollment() {
		...
		try {
			...
			message = cause instanceof Error ? cause.message : 'Setup could not be confirmed.';
		} finally {
			busy = false;
		}
	}
	...
	async function verify() {
		...
		try {
			...
			if (body.authentication_level !== 'AAL2')
				throw new Error('We could not confirm verification. Refresh your safety settings.');
			...
			message = cause instanceof Error ? cause.message : 'Verification could not be confirmed.';
		} finally {
			busy = false;
		}
	}
	async function regenerateCodes() {
		...
		try {
			...
			if (!Array.isArray(body.recovery_codes) || !body.recovery_codes.length)
				throw new Error('We could not retrieve your new backup codes. Make a new set before relying on them.');
			recoveryCodes = body.recovery_codes.map(text);
			message = 'Your new backup codes are ready. The old ones no longer work.';
		} catch (cause) {
			message = cause instanceof Error ? cause.message : 'New backup codes could not be confirmed.';
		} finally {
			busy = false;
		}
	}
...
<main class="shell workspace form-page">
	...
	<p class="lede">
		Ask for a six-digit code from an app on your phone before anybody can change money, bank or staff details. This adds
		protection if somebody gains access to your phone number.
	</p>
	{#if message}<p class="notice" role="status">{message}</p>{/if}
	{#if loading}<p role="status">Checking sign-in safety…</p>{:else if loadError}<p class="error" role="alert">
			{loadError}
		</p>
		...
		<section class="card">
			...
			{#if enrolled}
				...
				<label class="field"
					...
					/></label
				>
				<button class="primary" type="button" disabled={busy || !/^\d{6}$/.test(code)} onclick={verify}
					>{busy ? 'Checking…' : 'Check this code'}</button
				>
				<button type="button" disabled={busy} onclick={regenerateCodes}>Make new backup codes</button>
			{:else if !secret}
				<p>
					Use an app like Google Authenticator, Microsoft Authenticator, or 1Password. Never send these codes to
					anybody, not even to somebody who says they are from Kredit.
				</p>
				<button class="primary" type="button" disabled={busy} onclick={beginEnrollment}
					>{busy ? 'Starting…' : 'Start extra safety'}</button
				>
			{:else}
				...
				<button class="primary" type="button" disabled={busy || !/^\d{6}$/.test(code)} onclick={verify}
					>{busy ? 'Checking…' : 'Check code and turn it on'}</button
				>
			{/if}
		</section>
	{/if}
	{#if recoveryCodes.length}<section class="card">
