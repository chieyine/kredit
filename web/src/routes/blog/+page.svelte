<script lang="ts">
	import { articleCategoryDetails, articleCategories } from '$lib/blog/articles';
 ...
 let query=$state(''),category=$state('All guides');
	const posts = $derived(articles.map((article,index)=>({href:`/blog/${article.slug}`,title:article.title,excerpt:article.description,date:new Date(`${article.modified}T12:00:00`).toLocaleDateString('en-NG',{day:'numeric',month:'long',year:'numeric'}),minutes:article.readingMinutes,issue:String(index+1).padStart(3,'0'),category:article.category})));
...
<style>
	...
	.playbook-head > p { margin: 0; padding-top: 1.2rem; border-top:1px solid var(--color-foreground); color:var(--color-muted); font-size: 1.05rem; line-height: 1.7; }
	.featured { display: grid; grid-template-columns: .75fr 1.25fr; min-height: 33rem; margin-top: clamp(4rem, 8vw, 7rem); color:var(--color-on-primary); background:var(--color-primary);--color-muted:var(--color-on-primary);}.feature-mark { position: relative; display: flex; justify-content: space-between; padding: 1.5rem; overflow: hidden; border-right: 1px solid rgb(255 255 255 / .3); }.feature-mark span { font-size: .65rem; font-weight: 800; letter-spacing: .14em; }.feature-mark strong { font-family: var(--font-serif); font-size: 1.4rem; font-weight: 500; }.feature-mark i { position: absolute; left: 50%; bottom: -4rem; transform: translateX(-50%); color:var(--color-accent-ink); font-family: var(--font-serif); font-size: 24rem; font-style: normal; line-height: .8; }
	.featured > a { display: flex; flex-direction: column; padding: clamp(2rem, 5vw, 4rem); color: var(--color-on-primary); text-decoration: none; }.meta { display: flex; justify-content: space-between; gap: 1rem; color:var(--color-muted); font-size: .7rem; }.featured h2 { max-width: 13ch; margin: auto 0 1.2rem; font-family: var(--font-serif); font-size: clamp(2.7rem, 5vw, 5rem); font-weight: 500; line-height: .93; letter-spacing: -.05em; }.featured p { max-width: 36rem; margin: 0 0 2rem; color:var(--color-muted); line-height: 1.65; }.read { display: flex; justify-content: space-between; padding-top: 1rem; border-top: 1px solid rgb(255 255 255 / .35); font-size: .78rem; font-weight: 760; }.read b { color:var(--color-accent-ink); font-size: 1.1rem; }
	.topic-nav{margin-top:5rem}.topic-nav>div{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));margin-top:1rem;border-top:1px solid var(--color-muted);border-left:1px solid var(--color-muted)}.topic-nav a{display:grid;gap:.7rem;min-height:7rem;padding:1rem;border-right:1px solid var(--color-muted);border-bottom:1px solid var(--color-muted);color:var(--color-foreground);text-decoration:none}.topic-nav a span{color:var(--color-foreground);font-size:.65rem;text-transform:uppercase}.topic-nav a strong{font-family:var(--font-serif);font-size:1.05rem;font-weight:500}.topic-nav a i{align-self:end;color:var(--color-primary);font-style:normal}.topic-nav a:hover{background:var(--color-surface-muted)}
	.archive { margin-top: 7rem; }.archive-head { display: flex; justify-content: space-between; gap: 2rem; padding-bottom: 1rem; border-bottom:3px solid var(--color-foreground); font-size: .68rem; font-weight: 780; letter-spacing: .1em; text-transform: uppercase; }.archive-head p { margin: 0; }.archive-head span { color:var(--color-muted); }.post-row { display: grid; grid-template-columns: 3rem 1fr 8rem 2rem; gap: clamp(1rem, 4vw, 4rem); align-items: start; padding: 2.4rem 0; border-bottom:1px solid var(--color-border); color:var(--color-foreground); text-decoration: none; }.issue { color:var(--color-primary); font-size: .68rem; font-weight: 800; }.post-row h2 { max-width: 25ch; margin: 0 0 .7rem; font-family: var(--font-serif); font-size: clamp(1.6rem, 3vw, 2.5rem); font-weight: 500; line-height: 1.05; letter-spacing: -.035em; }.post-row p { max-width: 45rem; margin: 0; color:var(--color-muted); line-height: 1.65; }.post-meta { color:var(--color-muted); font-size: .68rem; line-height: 1.6; }.post-row i { color:var(--color-primary); font-size: 1.1rem; font-style: normal; }.post-row:hover h2 { color: var(--color-primary); }
	.library{display:grid;grid-template-columns:1fr 1fr;gap:2rem;align-items:end;margin-top:7rem;padding-bottom:1.5rem;border-bottom:3px solid var(--color-foreground)}.library h2{margin:.4rem 0;font-family:var(--font-serif);font-size:clamp(2rem,5vw,3.6rem);font-weight:500}.filters{display:grid;grid-template-columns:1.5fr 1fr;gap:.7rem}.filters label{display:grid;gap:.35rem;font-size:.75rem;font-weight:750}.filters input,.filters select{box-sizing:border-box;width:100%;min-height:3rem;padding:.7rem;border:1px solid var(--color-border);background:var(--color-surface);font:inherit}.archive{margin-top:1.5rem}.post-row small{display:block;margin-bottom:.6rem;color:var(--color-primary);font-size:.67rem;font-weight:800;text-transform:uppercase;letter-spacing:.08em}.no-results{padding:3rem;text-align:center;background:var(--color-border)}.no-results button{padding:.7rem 1rem;border:0;background:var(--color-primary);color:var(--color-on-primary);font-weight:750;--color-muted:var(--color-on-primary);}
	.playbook-end { display: flex; justify-content: space-between; gap: 2rem; align-items: center; margin-top: 5rem; padding-top: 1.3rem; border-top:3px solid var(--color-foreground); }.playbook-end p { margin: 0; font-family: var(--font-serif); font-size: 1.3rem; }.playbook-end a { display: inline-flex; gap: 2rem; color:var(--color-primary); font-weight: 760; text-decoration: none; }
