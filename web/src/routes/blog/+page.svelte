<script lang="ts">
	...
	let query = $state(''),
		category = $state('All guides');
	const posts = $derived(
		articles.map((article, index) => ({
			href: `/blog/${article.slug}`,
			title: article.title,
			excerpt: article.description,
			date: new Date(`${article.modified}T12:00:00`).toLocaleDateString('en-NG', {
				day: 'numeric',
				month: 'long',
				year: 'numeric'
			}),
	...
	const topics = $derived(
		articleCategories.map((category) => ({
			category,
			...articleCategoryDetails[category],
	...
	const visiblePosts = $derived(
		posts.filter(
			(post) =>
				(category === 'All guides' || post.category === category) &&
				`${post.title} ${post.excerpt} ${post.category}`.toLowerCase().includes(query.trim().toLowerCase())
		)
	);
	const listSchema = $derived({
		'@context': 'https://schema.org',
		'@type': 'ItemList',
		name: 'Kredit guides for Nigerian businesses',
		numberOfItems: articles.length,
		itemListElement: articles.map((article, index) => ({
			'@type': 'ListItem',
			position: index + 1,
			name: article.title,
			url: `https://kredit.ng/blog/${article.slug}`
		}))
	});
</script>
...
<header class="playbook-head">
	...
	<p>
		...
		what to do when somebody stops paying.
	</p>
</header>
...
<section class="library" aria-labelledby="library-title">
	<div>
		<p class="eyebrow">{articles.length} guides</p>
		<h2 id="library-title">What do you need to know?</h2>
	</div>
	<div class="filters">
		<label
			>Search guides<input
				...
				placeholder="For example: late payment"
			/></label
		><label
...
<section class="archive" aria-labelledby="archive-title">
	...
	{:else}<div class="no-results">
			...
			<p>Try a shorter word, like “payment”, “customer” or “credit”.</p>
			<button
				onclick={() => {
					query = '';
					category = 'All guides';
				}}>Show me every guide</button
			>
		</div>
	{/each}
</section>
...
<style>
	...
	.post-row {
		display: grid;
		grid-template-columns: 3rem 1fr 8rem 2rem;
