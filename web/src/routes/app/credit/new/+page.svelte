...
{#if loading}<p role="status">Checking your businesses…</p>{:else if !organizations.length}<p>Add or reopen your business from <a href="/app/overview">Home</a> before creating a sale.</p>{/if}<div class="practice"><div><strong>First time here?</strong><p>Let us fill it with sample details so you can see how it works. Nothing reaches anybody until you save and send it yourself.</p></div><button type="button" disabled={busy} onclick={practise}>Show me with samples</button></div>
