<script lang="ts">
	...
	async function requestCode() {
		...
		if (!response.ok) { error = body.detail ?? 'We could not send your code. Check the email or number and try again.'; return; }
	...
	async function verifyCode() {
		...
		const body = await response.json().catch(() => ({})); busy = false;
		if (!response.ok) { error = body.detail ?? 'That code did not work. Check the six digits and try again.'; return; }
		await goto(destination());
	}
</script>
<svelte:head><title>Sign in — Kredit</title></svelte:head>
<main class="shell auth-page">
	<section class="auth-copy"><p class="eyebrow">Seller account</p><h1>Welcome back.</h1><p class="lede">See who owes you, what has been paid and what needs you today. There is no password to remember, we just send you a code.</p></section>
	<form class="card auth-card" onsubmit={(event) => { event.preventDefault(); challengeID ? verifyCode() : requestCode(); }}>
		<h2>{challengeID ? 'Enter your six-digit code' : 'Sign in'}</h2>
		{#if !challengeID}
			<label>Your email or phone number<input bind:value={identifier} type={channel === 'email' ? 'email' : 'tel'} autocomplete={channel === 'email' ? 'email' : 'tel'} required /></label>
			<label>Where should we send the code?<select bind:value={channel}><option value="email">Email</option><option value="sms">SMS</option></select></label>
		{:else}
			<p>We sent a six-digit code to <strong>{identifier}</strong>. It only works once, and only for a few minutes.</p>
			<label>The six-digit code<input bind:value={code} inputmode="numeric" autocomplete="one-time-code" minlength="6" maxlength="6" required /></label>
			{#if developmentCode}<p class="notice">Development code: <strong>{developmentCode}</strong></p>{/if}
		{/if}
		{#if error}<p class="error" role="alert">{error}</p>{/if}
		<button class="primary" disabled={busy || (challengeID ? code.length !== 6 : !identifier)}>{busy ? 'Please wait…' : challengeID ? 'Open my account' : 'Send me my code'}</button>
		{#if challengeID}<button class="secondary" type="button" onclick={() => { challengeID = ''; code = ''; developmentCode = ''; }}>Use a different email or number</button>{/if}
	</form>
</main>
