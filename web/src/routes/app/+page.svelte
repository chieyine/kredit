<script lang="ts">
	...
	async function requestCode() {
		...
		if (!response.ok) { error = body.detail ?? 'We could not send your sign-in code. Check your details and try again.'; return; }
	...
	async function verifyCode() {
		...
		const body = await response.json().catch(() => ({})); busy = false;
		if (!response.ok) { error = body.detail ?? 'That code did not work. Check it and try again.'; return; }
		await goto(destination());
	}
</script>
<svelte:head><title>Sign in — Kredit</title></svelte:head>
<main class="shell auth-page">
	<section class="auth-copy"><p class="eyebrow">Seller account</p><h1>Your sales.<br />Your money.</h1><p class="lede">Sign in to see what customers owe, what has been paid and what needs your attention.</p></section>
	<form class="card auth-card" onsubmit={(event) => { event.preventDefault(); challengeID ? verifyCode() : requestCode(); }}>
		<h2>{challengeID ? 'Enter your sign-in code' : 'Sign in securely'}</h2>
		{#if !challengeID}
			...
			<label>Send my code by<select bind:value={channel}><option value="email">Email</option><option value="sms">SMS</option></select></label>
		{:else}
			<p>We sent a six-digit sign-in code to <strong>{identifier}</strong>.</p>
		...
		<button class="primary" disabled={busy || (challengeID ? code.length !== 6 : !identifier)}>{busy ? 'Please wait…' : challengeID ? 'Open my account' : 'Send sign-in code'}</button>
		{#if challengeID}<button class="secondary" type="button" onclick={() => { challengeID = ''; code = ''; developmentCode = ''; }}>Use a different email or phone</button>{/if}
	</form>
</main>
