<script lang="ts">
 ...
 import { settingsProfile } from '$lib/api/onboarding-settings';
	...
	let businessType='', orgID='', profile:any={}, readiness:any={requirements:[],missing:[]}, permissions:any={}, terms='',privacy='', loading=true,busy=false,message='';
	let repName='',repTitle='',contact='',channel='email',challengeID='',code='';
 let loadError='';
 ...
 async function load(){
  ...
  try{
   const orgs=await checkedJSON('/api/v1/organizations',rows('organizations',value=>{const row=record(value),id=text(row.id);if(!id)throw new Error('Missing business');return {id,type:text(row.business_type)};}),{signal:request.signal});
   if(!request.current())return;const selected=new URLSearchParams(location.search).get('organization');orgID=selected?(orgs.some(o=>o.id===selected)?selected:''):orgs[0]?.id??'';if(!orgID){if(selected)throw new Error('Business access could not be verified');return;}
...
<main class="shell workspace onboarding"><header><p class="eyebrow">Set up your account</p><h1>Set up your business.</h1><p class="lede">You can write down your first sale before any of this is finished. Do the important parts now, and come back for the money settings later.</p><p class="unregistered"><strong>No CAC? Choose “Not registered yet”.</strong> Confirm your identity and add a bank account in the same full name. You can sell to businesses or individuals once the setup checks are complete.</p></header>
