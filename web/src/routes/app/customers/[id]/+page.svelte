<script lang="ts">
	...
	async function loadCustomer() {
		...
		try {
			...
			if (!response.ok) throw new Error('We could not open your businesses. Please try again.');
			...
			for (const organization of candidates) {
				...
				if (!historyResponse.ok || !statementResponse.ok) throw new Error('We could not open this customer. Please try again.');
			...
			error = cause instanceof Error ? cause.message : 'We could not open this customer. Check your network and try again.';
		}
	}
	onMount(() => { void loadCustomer(); });
	function saveNote(){writeLocal(`kredit:customer-note:${organizationID}:${page.params.id}`,note.trim());noteMessage='Note saved on this phone only.';setTimeout(()=>noteMessage='',1800)}
</script>
<svelte:head><title>Customer — Kredit</title></svelte:head>
<main class="shell workspace"><a href="/app/customers">← Customers</a><p class="eyebrow">Customer</p><h1>How this customer has paid you.</h1><p class="lede">You only see sales this customer made with you. No other seller can see your records, and you cannot see theirs.</p>
	{#if error}<p class="error" role="alert">{error} <button type="button" onclick={loadCustomer}>Try again</button></p>{:else if !history}<p>Opening this customer…</p>{:else}<div class="customer-actions"><a class="primary-link" href={`/app/credit/new?customer=${encodeURIComponent(page.params.id ?? '')}&organization=${encodeURIComponent(organizationID)}`}>Sell to them again</a><button type="button" onclick={()=>window.print()}>Print this page</button></div><ShareActions title="Kredit customer statement" text={`Kredit statement: ${money(history.current_active_principal_kobo)} is still owed across ${history.active_obligations ?? 0} open sale(s). ${history.completed_obligations ?? 0} sale(s) fully paid.`}/><section class="stats"><article><span>Sales still open</span><strong>{history.active_obligations ?? 0}</strong></article><article><span>Money they still owe you</span><strong>{money(history.current_active_principal_kobo)}</strong></article><article><span>Sales fully paid</span><strong>{history.completed_obligations ?? 0}</strong></article><article><span>Paid on time</span><strong>{Number(history.on_time_percentage ?? 0).toFixed(0)}%</strong></article></section><section class="card"><h2>Your private note</h2><p>Use it for directions to their shop, their usual order, or anything you must remember. This note never leaves this phone. Not even Kredit can see it.</p><label>Note about this customer<textarea bind:value={note} rows="3" placeholder="For example: delivers to the second shop on Mondays"></textarea></label><button type="button" onclick={saveNote}>Save note</button>{#if noteMessage}<small role="status">{noteMessage}</small>{/if}</section><section class="card"><h2>Sales with you</h2>{#if statement?.obligations?.length}<div class="table">{#each statement.obligations as obligation}<a href={`/app/credit/${obligation.credit_request_id}?organization=${organizationID}`}><span>{obligation.buyer_name || 'Credit sale'}</span><strong>{money(obligation.outstanding_kobo)}</strong><small>{productLabel(obligation.payment_status)}</small></a>{/each}</div>{:else}<p>This customer has no open sale with you right now.</p>{/if}</section>{/if}
</main>
