<script lang="ts">
	...
	function summaryText(){const value=paymentSummary();return `Kredit ${sharePeriod==='today'?'today':'last 7 days'}: ${value.count} payment(s) received, ${money(value.total)} in total. ${money(summary?.outstanding_kobo)} remains outstanding; ${money(summary?.overdue_kobo)} is overdue.`}
...
<main class="shell workspace reports"><header><p class="eyebrow">Business performance</p><h1>See where the money is.<br />See what needs fixing.</h1><p class="lede">Outstanding balances, overdue exposure, payments received, ageing and Kredit fees in one place.</p></header>
	...
	{#if error}<p class="error" role="alert">{error}</p>{:else if loading}<div class="loading" aria-live="polite"><Skeleton rows={4} tall /></div>{:else if summary}
		<section class="stats"><article class="focus"><span>Outstanding now</span><strong>{money(summary.outstanding_kobo)}</strong><small>{summary.obligation_count} open balance{summary.obligation_count===1?'':'s'}</small></article><article><span>Overdue now</span><strong>{money(summary.overdue_kobo)}</strong><small>{overdueShare}% of outstanding value</small></article><article><span>Received</span><strong>{money(paidTotal)}</strong><small>{receivedRate}% of tracked paid + outstanding value</small></article><article><span>Average payment</span><strong>{money(averagePayment)}</strong><small>{recognizedPayments.length} recorded payment{recognizedPayments.length===1?'':'s'}</small></article><article><span>Kredit fees</span><strong>{money(fees?.total_fees_kobo)}</strong><small>Accrued after recorded waivers</small></article></section>
		<section class="performance"><div><p class="eyebrow">Collection picture</p><h2>{receivedRate}% of tracked value has been received.</h2><p>This compares payments already recorded with payments plus current outstanding balances. Use it as a directional operating view, not an accounting statement.</p></div><div class="meter" aria-label={`${receivedRate}% received`}><div><span style={`width:${receivedRate}%`}></span></div><p><b>{money(paidTotal)}</b> received <span>·</span> <b>{money(summary.outstanding_kobo)}</b> outstanding</p></div></section>
		<section class="card ageing"><h2>How late is the outstanding money?</h2><p>The older a balance becomes, the more attention it usually deserves.</p><dl>{#each Object.entries(buckets) as [bucket, amount]}<div><dt>{bucketName(bucket)}</dt><dd>{money(amount)}</dd></div>{/each}</dl><a href="/app/overdue">Review overdue balances →</a></section>
		<section class="card share-card"><div><h2>Share a money update</h2><p>Send a short payment summary to a partner or staff member without exporting the full ledger.</p><label>Period<select bind:value={sharePeriod}><option value="today">Today</option><option value="week">Last 7 days</option></select></label></div><div><strong>{money(paymentSummary().total)}</strong><small>{paymentSummary().count} payment{paymentSummary().count===1?'':'s'} received</small><ShareActions compact title="Kredit money update" text={summaryText()}/></div></section>
	{/if}
</main>
